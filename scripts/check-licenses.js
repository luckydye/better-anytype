...
const remoteConfigUrl =
	"https://raw.githubusercontent.com/anyproto/open/main/compliance/licenses-config.json";
...
function processLicenses(licenses, allowedLicenses) {
	const disallowedPackages = Object.keys(licenses).filter((pkg) => {
		...
		if (
			licenses[pkg].publisher == "Anytype" ||
			licenses[pkg].publisher == "Any" ||
			licenses[pkg].publisher == "Any Association"
		) {
			return false;
		}
		...
		if (pkgLicenses.includes(" AND ")) {
			...
			return !licenseNames.every((name) =>
				allowedLicenses.includes(name),
			);
		}
