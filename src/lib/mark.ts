...
class Mark {
			...
			switch (overlap) {
				...
				case I.MarkOverlap.Inner:
					map[type].push({
						type: el.type,
						param: el.param,
						range: { from: mark.range.to, to: el.range.to },
					});
				...
				case I.MarkOverlap.Right:
					if (
						![I.MarkType.Emoji].includes(el.type) &&
						el.param == mark.param
					) {
	...
	getInRange(marks: I.Mark[], type: I.MarkType, range: I.TextRange): any {
		...
		const overlaps = [
			I.MarkOverlap.Inner,
			I.MarkOverlap.InnerLeft,
			I.MarkOverlap.InnerRight,
			I.MarkOverlap.Equal,
		];
	...
	toHtml(text: string, marks: I.Mark[]) {
		...
		const render = (mark: I.Mark) => {
			...
			if (mark.type == I.MarkType.Mention) {
				prefix =
					'<smile></smile><img src="./img/space.svg" class="space" /><name>';
				suffix = "</name>";
			}
			...
			if (r[mark.range.from] && r[mark.range.to - 1]) {
				r[mark.range.from] =
					`<${tag} ${attr} ${data.join(" ")}>${prefix}${r[mark.range.from]}`;
	...
	cleanHtml(html: string) {
		...
		obj.find(this.getTag(I.MarkType.Mention))
			.removeAttr("class")
	...
	fromHtml(
		html: string,
		restricted: I.MarkType[],
	): { marks: I.Mark[]; text: string; adjustMarks: boolean } {
		const tags = this.getTags();
		const rh = new RegExp(
			`<(\/)?(${Object.values(tags).join("|")})(?:([^>]*)>|>)`,
			"ig",
		);
		const rp = /data-param="([^"]*)"/i;
	...
	fromMarkdown(
		...
		const checked = marks.filter((it) =>
			[I.MarkType.Code].includes(it.type),
		);
		const overlaps = [
			I.MarkOverlap.Left,
			I.MarkOverlap.Right,
		...
		html.replace(
			/\[([^\[\]]+)\]\(([^\(\)]+)\)(\s|$)/g,
			(s: string, p1: string, p2: string, p3: string) => {
				p1 = String(p1 || "");
				p2 = String(p2 || "");
				p3 = String(p3 || "");
				...
				for (let i = 0; i < marks.length; ++i) {
					const mark = marks[i];
					if (
						mark.range.from >= from &&
						mark.range.to <= from + p1.length + p2.length + 4
					) {
						if (
							[I.MarkType.Link, I.MarkType.Object].includes(
								mark.type,
							)
						) {
							marks.splice(i, 1);
	...
	fromUnicode(
		...
		const keys = Object.keys(Patterns).map((it) =>
			U.Common.regexEscape(it),
		);
		...
		const checked = marks.filter((it) =>
			[I.MarkType.Code, I.MarkType.Link].includes(it.type),
		);
	...
	toggleLink(newMark: I.Mark, marks: I.Mark[]) {
		for (let i = 0; i < marks.length; ++i) {
			const mark = marks[i];
			if (
				[I.MarkType.Link, I.MarkType.Object].includes(mark.type) &&
	...
	overlap(a: I.TextRange, b: I.TextRange): I.MarkOverlap {
		...
		} else if (a.from < b.from && a.to >= b.from) {
