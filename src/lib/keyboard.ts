...
class Keyboard {
	...
	unbind() {
		$(window).off(
			"keyup.common keydown.common mousedown.common scroll.common mousemove.common blur.common online.common offline.common",
		);
	}
	...
	onKeyDown(e: any) {
		...
		if (!this.isNavigationDisabled) {
			this.shortcut(isMac ? "cmd+[" : "alt+arrowleft", e, () =>
				this.onBack(),
			);
			this.shortcut(isMac ? "cmd+]" : "alt+arrowright", e, () =>
				this.onForward(),
			);
		...
		if (isMain) {
			...
			this.shortcut("ctrl+tab", e, () => {
				const items = U.Menu.getVaultItems().filter(
					(it) => !it.isButton,
				);
			...
			this.shortcut(`${cmd}+s, ${cmd}+k`, e, (pressed: string) => {
				if (
					S.Popup.isOpen("search") ||
					!this.isPinChecked ||
					(pressed == `${cmd}+k` && this.checkSelection())
				) {
					return;
				}
			...
			this.shortcut(`${cmd}+shift+\\`, e, () => {
				S.Popup.open("confirm", {
					...
					data: {
						...
						canCancel: true,
						onConfirm: () =>
							U.Common.copyToast("ID", this.getRootId()),
					},
				});
			});
	...
	checkSelection() {
		...
		if (
			...
			(selection && selection.get(I.SelectType.Block).length)
		) {
			return true;
		}
	...
	pageCreate(details: any, route: string) {
		if (this.isMain()) {
			...
			U.Object.create(
				...
				flags,
				route,
				(message) => U.Object.openConfig(message.details),
			);
		}
	}
	...
	onBack() {
		...
		if (
			S.Auth.accountIsDeleted() ||
			S.Auth.accountIsPending() ||
			!this.checkBack()
		) {
			return;
		}
		...
		switch (cmd) {
			...
			case "save": {
				S.Popup.open("export", {
					data: {
						objectIds: [rootId],
						route: analytics.route.menuSystem,
						allowHtml: true,
					},
				});
				break;
			...
			case "debugSpace": {
				C.DebugSpaceSummary(S.Common.space, (message: any) => {
					if (!message.error.code) {
						U.Common.getElectron().fileWrite(
							"debug-space-summary.json",
	...
	onContactUrl() {
		...
		C.AppGetVersion((message: any) => {
			...
			url = url.replace(
				/\%25version\%25/g,
				U.Common.getElectron().version.app,
			);
	...
	onMembershipUpgrade() {
		...
		Renderer.send(
			"urlOpen",
			J.Url.membershipUpgrade.replace(/\%25name\%25/g, name),
		);
	}
	...
	onTechInfo() {
		...
		C.AppGetVersion((message: any) => {
			let data = [
				[
					translate("libKeyboardOSVersion"),
					U.Common.getElectron().version.os,
				],
				[
					translate("libKeyboardAppVersion"),
					U.Common.getElectron().version.app,
				],
			...
			if (account) {
				data = data.concat([
					...
					[
						translate("libKeyboardAnalyticsId"),
						account.info.analyticsId,
					],
			...
			S.Popup.open("confirm", {
				...
				data: {
					...
					onConfirm: () => {
						U.Common.copyToast(
							translate("libKeyboardTechInformation"),
							data.map((it) => `${it[0]}: ${it[1]}`).join("\n"),
						);
					},
				},
			});
	...
	onSearchMenu(value: string, route?: string) {
		...
		if (!isPopup) {
			isDisabled =
				this.isMainSet() || this.isMainStore() || this.isMainGraph();
		} else {
			isDisabled = ["set", "store", "graph"].includes(
				popupMatch.params.action,
			);
		}
	...
	onQuickCapture(shortcut: boolean, param?: Partial<I.MenuParam>) {
		...
		if (
			S.Common.navigationMenu != I.NavigationMenuMode.Hover &&
			S.Menu.isOpen("quickCapture")
		) {
	...
	getMatch() {
		const ret = (this.isPopup() ? this.getPopupMatch() : this.match) || {
			params: {},
		};
	...
	restoreSource() {
		...
		switch (type) {
			case I.Source.Popup:
				window.setTimeout(
					() => S.Popup.open(data.id, data.param),
					S.Popup.getTimeout(),
				);
				break;
		}
	...
	isArrow(e: any): boolean {
		return [Key.up, Key.down, Key.left, Key.right].includes(
			this.eventKey(e),
		);
	}
