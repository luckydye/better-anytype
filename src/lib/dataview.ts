...
class Dataview {
	viewGetRelations(
		...
		let relations = U.Common.objectCopy(
			S.Record.getObjectRelations(rootId, blockId),
		).filter((it) => it);
		let o = 0;

		if (!config.debug.hiddenObject) {
			relations = relations.filter(
				(it) => it.relationKey == "name" || !it.isHidden,
			);
		}
		...
		const ret = relations
			...
			.map((relation) => {
				const vr =
					(view.relations || [])
						.filter((it) => it)
						.find((it) => it.relationKey == relation.relationKey) ||
					{};
		...
		C.BlockDataviewRelationAdd(
			...
			(message: any) => {
				...
				C.BlockDataviewViewRelationReplace(
					...
					(message: any) => {
						if (index >= 0) {
							const newView = S.Record.getView(
								rootId,
								blockId,
								view.id,
							);
							const oldIndex = (
								newView.relations || []
							).findIndex((it) => it.relationKey == relationKey);

							let keys = newView.relations.map(
								(it) => it.relationKey,
							);
							if (oldIndex < 0) {
							...
							C.BlockDataviewViewRelationSort(
								rootId,
								blockId,
								view.id,
								keys,
	...
	getData(param: any, callBack?: (message: any) => void) {
		...
		const filters = U.Common.objectCopy(view.filters).concat(
			param.filters || [],
		);
		...
		if (block) {
			const el = block.content.objectOrder.find(
				(it) => it.viewId == view.id && it.groupId == "",
			);
	...
	getView(rootId: string, blockId: string, viewId?: string): I.View {
		...
		if (!viewId) {
			viewId = S.Record.getMeta(
				S.Record.getSubId(rootId, blockId),
				"",
			).viewId;
		}
	...
	isCollection(rootId: string, blockId: string): boolean {
		...
		const target = targetObjectId
			? S.Detail.get(rootId, targetObjectId, ["layout"], true)
			: null;

		return target
			? U.Object.isCollectionLayout(target.layout)
			: isCollection;
	}
	...
	loadGroupList(
		...
		C.ObjectGroupsSubscribe(
			S.Common.space,
			subId,
			view.groupRelationKey,
			view.filters,
			object.setOf || [],
			isCollection ? object.id : "",
			(message: any) => {
				if (message.error.code) {
					return;
				}
				...
				S.Record.groupsSet(
					rootId,
					block.id,
					this.applyGroupOrder(rootId, block.id, view.id, groups),
				);
			},
		);
	}
	...
	getGroupFilter(relation: any, value: any): I.Filter {
		...
		switch (relation.format) {
			...
			case I.RelationType.MultiSelect: {
				value = Relation.getArrayValue(value);
				filter.condition = value.length
					? I.FilterCondition.ExactIn
					: I.FilterCondition.Empty;
	...
	applyObjectOrder(
		...
		const el = block.content.objectOrder.find(
			(it) =>
				it.viewId == viewId && (groupId ? it.groupId == groupId : true),
		);
		if (!el) {
			return records;
		}
	...
	getDetails(
		...
		if (relations.length) {
			relations.forEach((it) => {
				details[it.relationKey] = Relation.formatValue(
					it,
					details[it.relationKey] || null,
					true,
				);
			});
		}
		...
		if (
			view.groupRelationKey &&
			"undefined" == typeof details[view.groupRelationKey]
		) {
			if (groupId) {
		...
		for (const filter of view.filters) {
			...
			const value = Relation.getTimestampForQuickOption(
				filter.value,
				filter.quickOption,
			);
			if (!value) {
			...
			if (relation && !relation.isReadonlyValue) {
				details[filter.relationKey] = Relation.formatValue(
					relation,
					value,
					true,
				);
	...
	getTypeId(
		...
		const isAllowedDefaultType =
			this.isCollection(rootId, blockId) || !!relations.length;
		...
		if (types.length) {
			...
			for (const item of relations) {
				...
				if (
					first &&
					!U.Object.isInFileOrSystemLayouts(first.recommendedLayout)
				) {
					typeId = first.id;
					break;
				}
	...
	getCreateTooltip(
		...
		if (isCollection) {
			...
			if (type) {
				return U.Common.sprintf(
					translate("blockDataviewCreateNewTooltipType"),
					type.name,
				);
			}
		}
	...
	viewUpdate(
		...
		blockId: string,
		viewId: string,
		param: Partial<I.View>,
		callBack?: (message: any) => void,
	) {
		const view = U.Common.objectCopy(
			S.Record.getView(rootId, blockId, viewId),
		);
		if (view) {
			C.BlockDataviewViewUpdate(
				rootId,
				blockId,
				view.id,
				Object.assign(view, param),
				callBack,
			);
		}
	}
