...
class Relation {
	...
	public getCoverOptions(rootId: string, blockId: string) {
		...
		const options: any[] = U.Common.objectCopy(
	...
	public getGroupOptions(rootId: string, blockId: string, type: I.ViewType) {
		...
		let options: any[] = S.Record.getObjectRelations(
			rootId,
			blockId,
		).filter((it: any) => {
			return (
	...
	public getStringValue(value: any): string {
		if (
			typeof value === "object" &&
			value &&
			U.Common.hasProperty(value, "length")
		) {
		...
		if (objectTypes.length) {
			const allowedTypes = objectTypes
				...
				.filter(
					(it) =>
						it &&
						!U.Object.isInFileOrSystemLayouts(it.recommendedLayout),
				);
	...
	systemKeys() {
		return require("@anytype/proto/lib/json/systemRelations.json");
	}
