...
class Preview {
	...
	tooltipShow(param: Partial<TooltipParam>) {
		...
		this.timeout.tooltip = window.setTimeout(() => {
			...
			const node = $(
				`<div class="tooltip anim"><div class="txt">${U.Common.lbBr(text)}</div></div>`,
			);
			...
			this.timeout.delay = window.setTimeout(
				() => (this.delayTooltip = delay),
				500,
			);
	...
	previewShow(param: I.Preview) {
		...
		passThrough
			? obj.addClass("passThrough")
			: obj.removeClass("passThrough");
		obj.off("mouseleave.preview").on("mouseleave.preview", () =>
			this.previewHide(true),
		);
