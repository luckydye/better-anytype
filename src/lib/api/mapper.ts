...
import { Encode, Decode, Commands, Events, Model, Rpc } from "@anytype/proto";
...
export const Mapper = {
	...
	NotificationPayload(v: Model.Notification.PayloadCase) {
		...
		if (v == V.PARTICIPANTPERMISSIONSCHANGE)
			t = "participantPermissionsChange";
	...
	From: {
		...
		ObjectView: (obj: Model.ObjectView) => {
			return {
				...
				participants: (obj.getBlockparticipantsList() || []).map(
					(it) => ({
		...
		MetaList: (obj: Model.Search.Meta): any => {
			return {
				...
				ranges: (obj.getHighlightrangesList() || []).map(
					Mapper.From.Range,
				),
			};
		},
	...
	Event: {
		Type(v: number): string {
			...
			if (v == V.BLOCKDATAVIEWTARGETOBJECTIDSET)
				t = "BlockDataviewTargetObjectIdSet";
			if (v == V.BLOCKDATAVIEWISCOLLECTIONSET)
				t = "BlockDataviewIsCollectionSet";

			if (v == V.BLOCKDATAVIEWRELATIONSET) t = "BlockDataviewRelationSet";
			if (v == V.BLOCKDATAVIEWRELATIONDELETE)
				t = "BlockDataviewRelationDelete";
			if (v == V.BLOCKDATAVIEWGROUPORDERUPDATE)
				t = "BlockDataviewGroupOrderUpdate";
			if (v == V.BLOCKDATAVIEWOBJECTORDERUPDATE)
				t = "BlockDataviewObjectOrderUpdate";
		...
		ObjectRelationsAmend: (obj: Events.Event.Object.Relations.Amend) => {
			return {
				id: obj.getId(),
				relations: (obj.getRelationlinksList() || []).map(
					Mapper.From.RelationLink,
				),
			};
		},
		...
		BlockSetLink: (obj: Events.Event.Block.Set.Link) => {
			return {
				id: obj.getId(),
				targetBlockId: obj.hasTargetblockid()
					? obj.getTargetblockid().getValue()
					: null,
				cardStyle: obj.hasCardstyle()
					? obj.getCardstyle().getValue()
					: null,
				iconSize: obj.hasIconsize()
					? obj.getIconsize().getValue()
					: null,
				description: obj.hasDescription()
					? obj.getDescription().getValue()
					: null,
				relations: obj.hasRelations()
					? obj.getRelations().getValueList() || []
					: null,
		...
		BlockSetText: (obj: Events.Event.Block.Set.Text) => {
			return {
				...
				iconEmoji: obj.hasIconemoji()
					? obj.getIconemoji().getValue()
					: null,
				iconImage: obj.hasIconimage()
					? obj.getIconimage().getValue()
					: null,
				marks: obj.hasMarks()
					? (obj.getMarks().getValue().getMarksList() || []).map(
							Mapper.From.Mark,
						)
					: null,
			};
		...
		BlockDataviewTargetObjectIdSet: (
			obj: Events.Event.Block.Dataview.TargetObjectIdSet,
		) => {
			return {
				id: obj.getId(),
				targetObjectId: obj.getTargetobjectid(),
		...
		BlockDataviewIsCollectionSet: (
			obj: Events.Event.Block.Dataview.IsCollectionSet,
		) => {
			return {
				id: obj.getId(),
		...
		BlockSetFile: (obj: Events.Event.Block.Set.File) => {
			return {
				id: obj.getId(),
				targetObjectId: obj.hasTargetobjectid()
					? obj.getTargetobjectid().getValue()
					: null,
		...
		BlockSetBackgroundColor: (
			obj: Events.Event.Block.Set.BackgroundColor,
		) => {
			return {
				id: obj.getId(),
				bgColor: obj.getBackgroundcolor(),
		...
		BlockSetTableRow: (obj: Events.Event.Block.Set.TableRow) => {
			return {
				id: obj.getId(),
				isHeader: obj.hasIsheader()
					? obj.getIsheader().getValue()
					: null,
			};
		},
		...
		BlockDataviewViewUpdate: (
			obj: Events.Event.Block.Dataview.ViewUpdate,
		) => {
			const ret = {
				id: obj.getId(),
				viewId: obj.getViewid(),
				fields: obj.hasFields()
					? Mapper.From.ViewFields(obj.getFields())
					: null,
			};
			...
			keys.forEach((key) => {
				const items =
					obj[U.Common.toCamelCase(`get-${key.id}-list`)]() || [];
				...
				items.forEach((item) => {
					if (item.hasAdd()) {
						...
						const items = (op.getItemsList() || []).map(
							Mapper.From[key.mapper],
						);
		...
		BlockDataviewViewDelete: (
			obj: Events.Event.Block.Dataview.ViewDelete,
		) => {
			return {
				id: obj.getId(),
				viewId: obj.getViewid(),
		...
		BlockDataviewViewOrder: (
			obj: Events.Event.Block.Dataview.ViewOrder,
		) => {
			return {
				id: obj.getId(),
		...
		BlockDataviewRelationDelete: (
			obj: Events.Event.Block.Dataview.RelationDelete,
		) => {
			return {
				id: obj.getId(),
				relationKeys: obj.getRelationkeysList() || [],
		...
		BlockDataviewRelationSet: (
			obj: Events.Event.Block.Dataview.RelationSet,
		) => {
			return {
				id: obj.getId(),
				relations: (obj.getRelationlinksList() || []).map(
		...
		BlockDataviewGroupOrderUpdate: (
			obj: Events.Event.Block.Dataview.GroupOrderUpdate,
		) => {
			return {
				id: obj.getId(),
				groupOrder: obj.hasGrouporder()
					? Mapper.From.GroupOrder(obj.getGrouporder())
					: null,
			};
		},

		BlockDataviewObjectOrderUpdate: (
			obj: Events.Event.Block.Dataview.ObjectOrderUpdate,
		) => {
			return {
				id: obj.getId(),
				groupId: obj.getGroupid(),
		...
		SubscriptionPosition: (
			obj: Events.Event.Object.Subscription.Position,
		) => {
			return {
				id: obj.getId(),
				afterId: obj.getAfterid(),
		...
		SubscriptionCounters: (
			obj: Events.Event.Object.Subscription.Counters,
		) => {
			return {
				total: obj.getTotal(),
				subId: obj.getSubid(),
