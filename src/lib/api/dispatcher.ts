...
import { Commands, type Events, Service } from "@anytype/proto";
import {
	I,
	M,
	S,
...
class Dispatcher {
		...
		for (const message of messages) {
			...
			switch (type) {
				...
				case "AccountConfigUpdate": {
					...
					Renderer.send(
						"setConfig",
						U.Common.objectCopy(S.Common.config),
					);
					break;
				}
				...
				case "FileLimitReached": {
					const { bytesLimit, localUsage, spaces } =
						S.Common.spaceStorage;
					const bytesUsed = spaces.reduce(
						(res, current) => (res += current.bytesUsage),
						0,
					);
					const percentageUsed = Math.floor(
						U.Common.getPercent(bytesUsed, bytesLimit),
					);

					if (percentageUsed >= 99) {
						Preview.toastShow({
							action: I.ToastAction.StorageFull,
						});
					} else if (localUsage > bytesLimit) {
						Preview.toastShow({
							text: translate("toastFileLimitReached"),
						});
					}
					break;
				}
				...
				case "BlockAdd": {
					...
					for (const block of blocks) {
						if (block.type == I.BlockType.Dataview) {
							S.Record.relationsSet(
								rootId,
								block.id,
								block.content.relationLinks,
							);
							S.Record.viewsSet(
								rootId,
								block.id,
						...
						elements.forEach((element) => {
							...
							if (element.move) {
								...
								ids.forEach((id: string, i: number) => {
									const oidx = list.findIndex(
										(it) => it[key.idField] == id,
									);
									if (oidx >= 0) {
							...
							if (element.remove) {
								...
								list = list.filter(
									(it) => !ids.includes(it[key.idField]),
								);
				...
				case "BlockDataviewObjectOrderUpdate": {
					...
					const cb = (it) =>
						it.viewId == viewId && it.groupId == groupId;
					...
					if (!el) {
						el = {
							viewId,
							groupId,
							objectIds: observable.array([]),
						};
						block.content.objectOrder.push(el);
					}

					changes.forEach((it) => {
						const idx = it.afterId
							? el.objectIds.indexOf(it.afterId) + 1
							: 0;
						...
						switch (it.operation) {
							...
							case I.SliceOperation.Remove:
								el.objectIds = el.objectIds.filter(
									(id) => !it.ids.includes(id),
								);
								break;
					...
					S.Block.updateContent(rootId, id, {
						objectOrder: block.content.objectOrder,
					});
					S.Block.updateWidgetData(rootId);
					break;
				}
				...
				case "ObjectDetailsUnset": {
					...
					// Subscriptions
					this.getUniqueSubIds(subIds).forEach((subId) =>
						S.Detail.delete(subId, id, keys),
					);
				...
				case "NotificationSend": {
					...
					if (isMainWindow && !electron.isFocused()) {
						new window.Notification(
							U.Common.stripTags(item.title),
							{
								body: U.Common.stripTags(item.text),
							},
						).onclick = () => electron.focus();
					}
					break;
				}
				...
				case "PayloadBroadcast": {
					...
					let payload: any = {};
					try {
						payload = JSON.parse(mapped.payload);
					} catch (e) {
						/**/
					}
				...
				case "ProcessDone": {
					...
					switch (state) {
						case I.ProgressState.Running: {
							...
							if (
								[
									I.ProgressType.Recover,
									I.ProgressType.Migration,
								].includes(type)
							) {
								canCancel = false;
								isUnlocked = false;
	...
	getUniqueSubIds(subIds: string[]) {
		return U.Common.arrayUnique(
			(subIds || []).map((it) => it.split("/")[0]),
		);
	}
	...
	detailsUpdate(
		...
		this.getUniqueSubIds(subIds).forEach((subId) =>
			S.Detail.update(subId, { id, details }, clear),
		);

		if (
			[I.SpaceStatus.Deleted, I.SpaceStatus.Removing].includes(
				details.spaceAccountStatus,
			)
		) {
	...
	subscriptionPosition(
		...
		if (oldIndex !== newIndex) {
			S.Record.recordsSet(
				sid,
				"",
				arrayMove(records, oldIndex, newIndex),
			);
		}
	}
	...
	onObjectView(rootId: string, traceId: string, objectView: any) {
		const { details, restrictions, relationLinks, participants } =
			objectView;
		...
		if (root && root.fields.analyticsContext) {
			analytics.setContext(
				root.fields.analyticsContext,
				root.fields.analyticsOriginalId,
			);
		} else {
		...
		try {
			this.service[ct](
				...
				(error: any, response: any) => {
					...
					message.event = response.getEvent
						? response.getEvent()
						: null;
					...
					if (message.error.code) {
						...
						if (!SKIP_SENTRY_ERRORS.includes(type)) {
							Sentry.captureMessage(
								`${type}: code: ${code} msg: ${message.error.description}`,
							);
						...
						message.error.description = U.Common.translateError(
							type,
							message.error,
						);
					}
