import type { Rpc } from "@anytype/proto/lib/commands_pb";
import { Decode } from "@anytype/proto";
...
export const AccountLocalLinkNewChallenge = (
	response: Rpc.Account.LocalLink.NewChallenge.Response,
) => {
	return {
...
export const AccountLocalLinkSolveChallenge = (
	response: Rpc.Account.LocalLink.SolveChallenge.Response,
) => {
	return {
...
export const DebugStat = (response: Rpc.Debug.Stat.Response) => {
	let res = {};
	try {
		res = JSON.parse(response.getJsonstat());
	} catch (e) {
		/**/
	}
	return res;
...
export const ObjectCreateFromUrl = (
	response: Rpc.Object.CreateFromUrl.Response,
) => {
	return {
		objectId: response.getObjectid(),
		details: Decode.struct(response.getDetails()),
...
export const ObjectSearchWithMeta = (
	response: Rpc.Object.SearchWithMeta.Response,
) => {
	return {
		records: (response.getResultsList() || []).map(
			Mapper.From.ObjectSearchWithMeta,
		),
	};
};
...
export const HistoryDiffVersions = (
	response: Rpc.History.DiffVersions.Response,
) => {
	return {
...
export const GalleryDownloadIndex = (
	response: Rpc.Gallery.DownloadIndex.Response,
) => {
	return {
		categories: (response.getCategoriesList() || []).map(
			(it: Rpc.Gallery.DownloadIndex.Response.Category) => {
				return {
					id: it.getId(),
					icon: it.getIcon(),
...
export const GalleryDownloadManifest = (
	response: Rpc.Gallery.DownloadManifest.Response,
) => {
	return {
...
export const NameServiceResolveName = (
	response: Rpc.NameService.ResolveName.Response,
) => {
	return {
...
export const MembershipGetStatus = (
	response: Rpc.Membership.GetStatus.Response,
) => {
	return {
...
export const MembershipGetTiers = (
	response: Rpc.Membership.GetTiers.Response,
) => {
	return {
		tiers: (response.getTiersList() || []).map((it) =>
			Mapper.From.MembershipTierData(it),
		),
	};
};

export const MembershipRegisterPaymentRequest = (
	response: Rpc.Membership.RegisterPaymentRequest.Response,
) => {
	return {
...
export const MembershipGetPortalLinkUrl = (
	response: Rpc.Membership.GetPortalLinkUrl.Response,
) => {
	return {
...
export const SpaceInviteGenerate = (
	response: Rpc.Space.InviteGenerate.Response,
) => {
	return {
...
export const SpaceInviteGetCurrent = (
	response: Rpc.Space.InviteGetCurrent.Response,
) => {
	return {
		inviteCid: response.getInvitecid(),
		inviteKey: response.getInvitefilekey(),
...
export const DeviceList = (response: Rpc.Device.List.Response) => {
	return {
		devices: (response.getDevicesList() || []).map((it) =>
			Mapper.From.DeviceInfo(it),
		),
	};
};
