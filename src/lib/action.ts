...
class Action {
	...
	download(block: I.Block, route: string) {
		...
		const url = block.isFileImage()
			? S.Common.imageUrl(targetObjectId, 1000000)
			: S.Common.fileUrl(targetObjectId);
	...
	duplicate(
		...
		C.BlockListDuplicate(
			...
			(message: any) => {
				...
				const lastId =
					message.blockIds && message.blockIds.length
						? message.blockIds[message.blockIds.length - 1]
						: "";
				...
				analytics.event("DuplicateBlock", {
	...
	move(
		...
		C.BlockListMoveToExistingObject(
			contextId,
			targetContextId,
			targetId,
			blockIds,
			...
			(message: any) => {
				...
				analytics.event(
					contextId != targetContextId ? "MoveBlock" : "ReorderBlock",
					{
						count,
					},
				);
	...
	openFileDialog(extensions: string[], callBack?: (paths: string[]) => void) {
		...
		if (extensions && extensions.length) {
			options.filters = [{ name: "Filtered extensions", extensions }];
		}

		U.Common.getElectron()
		...
		C.WorkspaceObjectAdd(S.Common.space, object.id, (message: any) => {
			...
			switch (object.layout) {
				case I.ObjectLayout.Type: {
					toast = U.Common.sprintf(
						translate("toastObjectTypeAdded"),
						object.name,
					);
	...
	uninstall(
		...
		switch (object.layout) {
			case I.ObjectLayout.Type: {
				title = U.Common.sprintf(
					translate("libActionUninstallTypeTitle"),
					object.name,
				);
				text = translate("libActionUninstallTypeText");
				toast = U.Common.sprintf(
					translate("toastObjectTypeRemoved"),
					object.name,
				);
			...
			case I.ObjectLayout.Relation: {
				title = U.Common.sprintf(
					translate("libActionUninstallRelationTitle"),
					object.name,
				);
				text = translate("libActionUninstallRelationText");
				toast = U.Common.sprintf(
					translate("toastRelationRemoved"),
					object.name,
				);
	...
	delete(ids: string[], route: string, callBack?: () => void): void {
		...
		S.Popup.open("confirm", {
			data: {
				title: U.Common.sprintf(
					translate("popupConfirmDeleteWarningTitle"),
					count,
					U.Common.plural(count, translate("pluralObject")),
				),
	...
	restoreFromBackup(
		onError: (error: { code: number; description: string }) => boolean,
	) {
		...
		this.openFileDialog(["zip"], (paths) => {
			C.AccountRecoverFromLegacyExport(
				paths[0],
				dataPath,
				...
				(message: any) => {
					...
					C.ObjectImport(
						spaceId,
						{ paths, noCollection: true },
						[],
						false,
						I.ImportType.Protobuf,
						I.ImportMode.AllOrNothing,
						false,
						true,
						false,
						false,
	...
	import(
		...
		const fileOptions: any = {
			properties: ["openFile"],
			filters: [{ name: "Filtered extensions", extensions }],
		};
	...
	export(
		...
		this.openDirectoryDialog(
			{ buttonLabel: translate("commonExport") },
			(paths) => {
				if (onSelectPath) {
					onSelectPath();
				}
	...
	copyBlocks(rootId: string, ids: string[], isCut: boolean) {
		...
		let next = null;
		let blocks = S.Block.unwrapTree(tree).filter((it) =>
			ids.includes(it.id),
		);
		...
		ids.forEach((id: string) => {
			...
			if (block && block.isTable()) {
				blocks = blocks.concat(
					S.Block.unwrapTree([S.Block.wrapTree(rootId, block.id)]),
				);
			}
		});
		...
		if (isCut) {
			next = S.Block.getNextBlock(rootId, focused, -1, (it) =>
				it.isFocusable(),
			);
		}
	...
	removeSpace(id: string, route: string, callBack?: (message: any) => void) {
		...
		const title = U.Common.sprintf(
			translate(`space${suffix}WarningTitle`),
			name,
		);
		const text = U.Common.sprintf(
			translate(`space${suffix}WarningText`),
			name,
		);
		...
		S.Popup.open("confirm", {
			data: {
				...
				textConfirm: confirm,
				onConfirm: () => {
					analytics.event(`Click${suffix}SpaceWarning`, {
						type: suffix,
						route,
					});
					...
					const cb = () => {
						C.SpaceDelete(id, (message: any) => {
							...
							if (!message.error.code) {
								Preview.toastShow({ text: toast });
								analytics.event(`${suffix}Space`, {
									type: deleted.spaceAccessType,
									route,
								});
							}
						});
			...
			if (!message.error.code) {
				Preview.toastShow({
					text: U.Common.sprintf(
						translate("toastApproveLeaveRequest"),
						name,
					),
				});
	...
	setIsFavorite(
		...
		C.ObjectListSetIsFavorite(objectIds, v, (message: any) => {
			...
			analytics.event(v ? "AddToFavorites" : "RemoveFromFavorites", {
	...
	createWidgetFromObject(
		...
		if (object && !object._empty_) {
			...
			} else if (U.Object.isInSetLayouts(object.layout)) {
				layout = I.WidgetLayout.Compact;
			} else if (U.Object.isInPageLayouts(object.layout)) {
			...
			data: {
				...
				onConfirm: () => {
					C.SpaceInviteRevoke(spaceId, () => {
						...
						Preview.toastShow({
							text: translate("toastInviteRevoke"),
						});
