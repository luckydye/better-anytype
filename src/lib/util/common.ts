...
class UtilCommon {
	...
	sprintf(...args: any[]) {
		...
		const doFormat = (
			...
			switch (type) {
				...
				case "d": {
					let number = +value;
					number = Number.parseInt(String(number));
	...
	fromCamelCase(str: string, symbol: string) {
		...
		return String(str || "").replace(
			/([A-Z]{1})/g,
			(_, char) => symbol + char.toLowerCase(),
		);
	}
		...
		for (const k in o) {
			if (typeof o[k] === "object") {
				...
				if (!this.objectLength(o[k])) {
					delete o[k];
				} else if (this.hasProperty(o[k], "fieldsMap")) {
					o[k] = this.fieldsMap(o[k]["fieldsMap"]);
				}
			} else if (typeof o[k] === "undefined" || o[k] === null) {
				delete o[k];
			}
		}
		return o;
	...
	stringCut(haystack: string, start: number, end: number): string {
		return (
			String(haystack || "").substring(0, start) + haystack.substring(end)
		);
	}
	...
	copyToast(label: string, text: string, toast?: string) {
		...
		Preview.toastShow({
			text: this.sprintf(toast || translate("toastCopy"), label),
		});
	}
	...
	formatNumber(v: number): string {
		...
		let ret = String(v || "");
		let parts = new Intl.NumberFormat("en-GB", {
	...
	renderLinks(obj: any) {
		...
		links.click((e: any) => {
			...
			e.preventDefault();
			el.hasClass("path")
				? this.onPath(el.attr("href"))
				: this.onUrl(el.attr("href"));
		});
	}
	...
	getPlatform(): I.Platform {
		return (
			J.Constant.platforms[this.getElectron().platform] || I.Platform.None
		);
	}
	...
	checkErrorCommon(code: number): boolean {
		...
		if (code == J.Error.Code.ANOTHER_ANYTYPE_PROCESS_IS_RUNNING) {
			alert(
				"You have another instance of anytype running on this machine. Closing...",
			);
		...
		if (
			[
				J.Error.Code.ANYTYPE_NEEDS_UPGRADE,
				J.Error.Code.PROTOCOL_NEEDS_UPGRADE,
			].includes(code)
		) {
			this.onErrorUpdate();
			return false;
	...
	checkErrorOnOpen(rootId: string, code: number, context: any): boolean {
		...
		if (
			[J.Error.Code.NOT_FOUND, J.Error.Code.OBJECT_DELETED].includes(code)
		) {
			if (context) {
	...
	intercept(obj: any, change: any) {
		return this.compareJSON(change.newValue, obj[change.name])
			? null
			: change;
	}
	...
	getScrollContainer(isPopup: boolean) {
		return (
			isPopup ? $("#popupPage-innerWrap") : $(window)
	...
	addBodyClass(prefix: string, v: string) {
		...
		const c = String(obj.attr("class") || "")
			.split(" ")
			.filter((it) => !it.match(reg));
	...
	matchLocalPath(s: string): string {
		...
		const rw = new RegExp(
			/^(?:[a-zA-Z]:|[\\\/]{2}[^\\\/]+[\\\/]+[^\\\/]+)\\(?:[\p{L}\p{N}\s\._-]+\\)*[\p{L}\p{N}\s\._-]+(?:\.[\p{L}\p{N}\s_-]+)?$/giu,
		);
		...
		for (const item of items) {
			if (item.path) {
				...
				reader.onload = () => {
					ret.push({
						name: item.name,
						path: this.getElectron().fileWrite(
							item.name,
							reader.result,
							{
								encoding: "binary",
	...
	pauseMedia() {
		$("audio, video").each((i: number, item: any) => {
			item.pause();
		});
	}
	...
	translateError(command: string, error: any) {
		...
		const Text = require("@anytype/locales/default.json");
	...
	sanitize(s: string): string {
		...
		const tags = [
			...
			"smile",
			"img",
		].concat(Object.values(Mark.getTags()));
	...
	addScript(id: string, src: string) {
		const body = $("body");
		const element = $(
			`<script id="${id}" type="text/javascript" src="${src}"></script>`,
		);
