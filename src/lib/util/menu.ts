...
class UtilMenu {
	mapperBlock(it: any) {
		...
		if (it.lang) {
			...
			if (S.Common.interfaceLang != J.Constant.default.interfaceLang) {
				it.aliases.push(
					translate(nameKey, J.Constant.default.interfaceLang),
				);
				it.aliases.push(
					translate(descriptionKey, J.Constant.default.interfaceLang),
				);
	...
	getBlockObject() {
		const items = U.Data.getObjectTypesForNewObject({
			withSet: true,
			withCollection: true,
		});
		const ret: any[] = [
		...
		items.sort((c1, c2) =>
			U.Data.sortByNumericKey("lastUsedDate", c1, c2, I.SortType.Desc),
		);
	...
	getActions(param: any) {
		...
		if (hasBookmark) {
			items.push({
				id: "copyUrl",
				icon: "copy",
				name: translate("libMenuCopyUrl"),
			});
		}

		if (hasDataview) {
			const isCollection = Dataview.isCollection(rootId, blockId);
			const sourceName = isCollection
				? translate("commonCollection")
				: translate("commonSet");
		...
		if (id) {
			if (!isCollection) {
				...
				const treeSkipLayouts = setLayouts
					...
					.concat([I.ObjectLayout.Participant]);
	...
	getInterfaceLanguages() {
		const ret: any[] = [];
		const Locale = require("@anytype/locales/locale.json");
	...
	spaceContext(space: any, param: any) {
		...
		if (isOwner && space.isShared && !isLocalNetwork && isOnline) {
			options.push({
				id: "revoke",
				name: translate("popupSettingsSpaceShareRevokeInvite"),
			});
		}
		...
		} else if (space.isAccountJoining) {
			options.push({
				id: "cancel",
				color: "red",
				name: translate("popupSettingsSpacesCancelRequest"),
			});
		} else {
			options.push({
				id: "remove",
		...
		S.Menu.open("select", {
			...
			data: {
				...
				onSelect: (e: any, item: any) => {
					switch (item.id) {
						case "qr": {
							S.Popup.open("inviteQr", {
								data: { link: U.Space.getInviteLink(cid, key) },
							});
							analytics.event("ClickSettingsSpaceShare", {
								type: "Qr",
							});
							break;
						}
	...
	getVaultItems() {
		...
		if (U.Space.canCreateSpace()) {
			items.push({
				id: "add",
				name: translate("commonCreateNew"),
				isButton: true,
			});
		}
	...
	getFixedWidgets() {
		return [
			{
				id: J.Constant.widgetId.favorite,
				name: translate("menuWidgetFavorites"),
				iconEmoji: ":star:",
			},
			{
				id: J.Constant.widgetId.set,
				name: translate("menuWidgetSets"),
				iconEmoji: ":mag:",
			},
			{
				id: J.Constant.widgetId.collection,
				name: translate("menuWidgetCollections"),
				iconEmoji: ":card_index_dividers:",
			},
			{
				id: J.Constant.widgetId.recentEdit,
				name: translate("menuWidgetRecentEdit"),
				iconEmoji: ":memo:",
			},
			{
				id: J.Constant.widgetId.recentOpen,
				name: translate("menuWidgetRecentOpen"),
				iconEmoji: ":date:",
				caption: translate("menuWidgetRecentOpenCaption"),
			},
		];
	}
}
