...
const fileIcons = import.meta.glob("/src/assets/**/icon/file/**/*.svg", {
	eager: true,
	query: "?url",
});

function getIcon(key: string) {
	return Object.entries(fileIcons).find(([k]) => k.match(key))?.[1]?.default;
}
...
class UtilFile {
	size(v: number): string {
		...
		return (
			U.Common.formatNumber(Number(U.Common.sprintf(`%0.2f`, ret))) + unit
		);
	}
	...
	icon(object: any): string {
		...
		const n = name.split(".");
		const mime = String(
			object.mime || object.mimeType || object.fileMimeType || "",
		).toLowerCase();
		...
		if (
			["csv", "json", "txt", "doc", "docx", "md", "tsx", "scss"].indexOf(
				e,
			) >= 0
		) {
			icon = "text";
	...
	iconImage(object: any): string {
		const tp = S.Common.getThemePath();
		return getIcon(`img/${tp}icon/file/${this.icon(object)}`);
	}
	...
	loadPreviewBase64(
		file: any,
		param: any,
		success?: (image: string, param: any) => void,
	...
	name(object: any) {
		...
		if (
			!fileExt ||
			new RegExp(`\\.${U.Common.regexEscape(fileExt)}$`).test(name)
		) {
			return name;
		}
