...
class UtilData {
	...
	blockClass(block: any) {
		...
		switch (block.type) {
			...
			default: {
				...
				switch (block.type) {
					case I.BlockType.Text:
						c.push(this.blockTextClass(style));
						break;
					case I.BlockType.Layout:
						c.push(this.blockLayoutClass(style));
						break;
					case I.BlockType.Div:
						c.push(this.blockDivClass(style));
						break;
				}
				break;
			}
	...
	layoutClass(id: string, layout: I.ObjectLayout) {
		...
		switch (layout) {
			default:
				c = U.Common.toCamelCase(`is-${I.ObjectLayout[layout]}`);
				break;
	...
	diffClass(v: I.DiffType): string {
		let c = "";
		switch (v) {
			case I.DiffType.None:
				c = "diffNone";
				break;
			case I.DiffType.Add:
				c = "diffAdd";
				break;
			case I.DiffType.Change:
				c = "diffChange";
				break;
			case I.DiffType.Remove:
				c = "diffRemove";
				break;
		}
		return c;
		...
		C.ObjectOpen(root, "", space, (message: any) => {
			...
			C.ObjectOpen(widgets, "", space, (message: any) => {
				...
				this.createSpaceSubscriptions(() => {
					...
					[I.SurveyType.Register, I.SurveyType.Object].forEach((it) =>
						Survey.check(it),
					);
				...
				filters: [
					{
						...
						relationKey: "layout",
						condition: I.FilterCondition.Equal,
						value: I.ObjectLayout.SpaceView,
					},
				],
				sorts: [{ relationKey: "createdDate", type: I.SortType.Desc }],
				ignoreWorkspace: true,
				ignoreHidden: false,
			},
		];
				...
				filters: [
					{
						...
						relationKey: "layout",
						condition: I.FilterCondition.Equal,
						value: I.ObjectLayout.Participant,
					},
					{
						operator: I.FilterOperator.And,
	...
	createSpaceSubscriptions(callBack?: () => void): void {
		...
		const list: any[] = [
			...
			},
			{
				...
				filters: [
					{
						...
						value: [J.Constant.storeSpaceId, space],
					},
					{
						operator: I.FilterOperator.And,
						relationKey: "layout",
				...
				ignoreHidden: false,
				onSubscribe: () => {
					S.Record.getTypes().forEach((it) =>
						S.Record.typeKeyMapSet(it.spaceId, it.uniqueKey, it.id),
					);
				},
			},
			{
				...
				ignoreHidden: false,
				onSubscribe: () => {
					S.Record.getRelations().forEach((it) =>
						S.Record.relationKeyMapSet(
							it.spaceId,
							it.relationKey,
							it.id,
						),
				...
				filters: [
					{
						...
						relationKey: "layout",
						condition: I.FilterCondition.Equal,
						value: I.ObjectLayout.Option,
					},
				],
				sorts: [{ relationKey: "name", type: I.SortType.Asc }],
				noDeps: true,
				ignoreDeleted: true,
			},
			{
	...
	spaceRelationKeys() {
		return J.Relation.default
			.concat(J.Relation.space)
			.concat(J.Relation.participant);
	}
	...
	blockInsertText(
		...
		const text = U.Common.stringInsert(
			block.content.text,
			needle,
			from,
			to,
	...
	getObjectTypesForNewObject(param?: any) {
		...
		items = items.concat(
			S.Record.getTypes().filter((it) => {
				return (
					pageLayouts.includes(it.recommendedLayout) &&
	...
	getTemplatesByTypeId(typeId: string, callBack: (message: any) => void) {
		...
		const sorts = [{ relationKey: "name", type: I.SortType.Asc }];
	...
	checkDetails(rootId: string, blockId?: string) {
		...
		if (!object._empty_) {
			...
			className = [
				this.layoutClass(object.id, object.layout),
				"align" + object.layoutAlign,
			];
		}
		...
		} else if (ret.withIcon) {
			className.push("withIcon");
		} else if (ret.withCover) {
	...
	checkLinkSettings(content: I.ContentLink, layout: I.ObjectLayout) {
		...
		content.relations = (content.relations || []).filter((it) =>
			relationKeys.includes(it),
		);

		if (U.Object.isTaskLayout(layout)) {
			content.iconSize = I.LinkIconSize.Small;
		} else if (U.Object.isNoteLayout(layout)) {
			const filter = ["type"];
	...
	onSubscribe(subId: string, idField: string, keys: string[], message: any) {
		...
		S.Record.recordsSet(
			subId,
			"",
			message.records.map((it) => it[idField]).filter((it) => it),
		);
	}

	searchSubscribe(
		param: SearchSubscribeParams,
		callBack?: (message: any) => void,
	) {
		...
		C.ObjectSearchSubscribe(
			subId,
			filters,
			sorts.map(this.sortMapper),
			keys,
			sources,
			offset,
			limit,
	...
	search(
		param: SearchSubscribeParams & { fullText?: string },
		callBack?: (message: any) => void,
	) {
		const { config, space } = S.Common;
		...
		C.ObjectSearch(
			...
			(message: any) => {
				if (message.records) {
					message.records = message.records.map((it) =>
						S.Detail.mapper(it),
					);
				}
	...
	setWindowTitle(rootId: string, objectId: string) {
		this.setWindowTitleText(
			U.Object.name(S.Detail.get(rootId, objectId, [])),
		);
	}
	...
	graphFilters() {
		...
		const filters = [
			...
			},
			{
				...
				value: U.Object.getFileAndSystemLayouts(),
			},
			{
				operator: I.FilterOperator.And,
				relationKey: "id",
				condition: I.FilterCondition.NotEqual,
				value: J.Constant.anytypeProfileId,
			},
			{
				operator: I.FilterOperator.And,
				relationKey: "spaceId",
	...
	moveToPage(rootId: string, ids: string[], typeId: string, route: string) {
		...
		C.BlockListConvertToObjects(
			rootId,
			ids,
			type.uniqueKey,
			type.defaultTemplateId,
			this.getLinkBlockParam("", type.recommendedLayout),
			(message: any) => {
				if (!message.error.code) {
	...
	getMembershipTiers(noCache: boolean) {
		...
		C.MembershipGetTiers(noCache, interfaceLang, (message) => {
			...
			const tiers = message.tiers.filter(
				(it) =>
					it.id == I.TierType.Explorer || it.isTest == !!testPayment,
			);
	...
	getMembershipTier(id: I.TierType): I.MembershipTier {
		return (
			S.Common.membershipTiers.find((it) => it.id == id) ||
			new M.MembershipTier({ id: I.TierType.None })
		);
	}

	isAnytypeNetwork(): boolean {
		return Object.values(J.Constant.networkId).includes(
			S.Auth.account?.info?.networkId,
		);
	}
		...
		C.WalletCreate(dataPath, (message) => {
			...
			this.createSession(phrase, "", (message) => {
				...
				C.AccountCreate(
					...
					"",
					dataPath,
					U.Common.rand(1, J.Constant.count.icon),
					mode,
					path,
					(message) => {
						if (message.error.code) {
	...
	groupDateSections(records: any[], key: string, sectionTemplate?: any) {
		...
		records.forEach((record) => {
			...
			} else if (diff < lastWeek) {
				groups.lastWeek.push(record);
			} else if (diff < lastMonth) {
