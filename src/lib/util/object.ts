...
class UtilObject {
	actionByLayout(v: I.ObjectLayout): string {
		...
		switch (v) {
			...
			case I.ObjectLayout.Store:
				r = "store";
				break;
			case I.ObjectLayout.History:
				r = "history";
				break;
			case I.ObjectLayout.Archive:
				r = "archive";
				break;
			case I.ObjectLayout.Block:
				r = "block";
				break;
			case I.ObjectLayout.Empty:
				r = "empty";
				break;
		}
		return r;
	...
	universalRoute(object: any): string {
		...
		return object
			? `object?objectId=${object.id}&spaceId=${object.spaceId}`
			: "";
	}
	...
	openPopup(object: any, param?: any) {
		...
		if (object._routeParam_) {
			param.data.matchPopup.params = Object.assign(
				param.data.matchPopup.params,
				object._routeParam_,
			);
		}
		...
		window.setTimeout(
			() => S.Popup.open("page", param),
			S.Popup.getTimeout(),
		);
	}

	openConfig(object: any, param?: any) {
		S.Common.fullscreenObject
			? this.openAuto(object, param)
			: this.openPopup(object, param);
	}
	...
	create(
		...
		if (details.type) {
			...
			if (type) {
				...
				if (!templateId) {
					templateId =
						type.defaultTemplateId || J.Constant.templateId.blank;
				}
			}
		}
	...
	setDefaultTemplateId(
		...
		C.ObjectListSetDetails(
			[rootId],
			[{ key: "defaultTemplateId", value: id }],
			callBack,
		);
	}
	...
	getFileTypeByLayout(layout: I.ObjectLayout): I.FileType {
		switch (layout) {
			default:
				return I.FileType.File;
			case I.ObjectLayout.Image:
				return I.FileType.Image;
			case I.ObjectLayout.Audio:
				return I.FileType.Audio;
			case I.ObjectLayout.Video:
				return I.FileType.Video;
			case I.ObjectLayout.Pdf:
				return I.FileType.Pdf;
		}
	}
	...
	isAllowedTemplate(typeId): boolean {
		...
		return type
			? !this.getLayoutsWithoutTemplates().includes(
