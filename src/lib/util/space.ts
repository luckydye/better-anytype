...
class UtilSpace {
	...
	getDashboard() {
		...
		if (id == I.HomePredefinedId.Graph) {
			ret = this.getGraph();
		} else if (id == I.HomePredefinedId.Last) {
	...
	getList() {
		return S.Record.getRecords(
			...
			U.Data.spaceRelationKeys(),
		).filter((it) => it.isAccountActive && it.isLocalOk);
	}
	...
	getSpaceviewBySpaceId(id: string) {
		return S.Record.getRecords(J.Constant.subId.space).find(
			(it) => it.targetSpaceId == id,
		);
	}
	...
	getParticipantsList(statuses?: I.ParticipantStatus[]) {
		...
		return statuses
			? ret.filter((it) => statuses.includes(it.status))
			: ret;
	}
	...
	getParticipant(id?: string) {
		...
		const object = S.Detail.get(
			J.Constant.subId.participant,
			id || this.getParticipantId(space, account.id),
		);
	...
	getMyParticipant(spaceId?: string) {
		...
		const object = S.Detail.get(
			J.Constant.subId.myParticipant,
			this.getParticipantId(spaceId || space, account.id),
		);
		return object._empty_ ? null : object;
	}
	...
	getReaderLimit() {
		...
		const participants = this.getParticipantsList([
	...
	getWriterLimit() {
		...
		const participants = this.getParticipantsList([
			I.ParticipantStatus.Active,
		]).filter((it) => it.isWriter || it.isOwner);
		return space.writersLimit - participants.length;
	}

	getInviteLink(cid: string, key: string) {
		return U.Data.isAnytypeNetwork()
			? U.Common.sprintf(J.Url.invite, cid, key)
			: `${J.Constant.protocol}://invite/?cid=${cid}&key=${key}`;
	}
