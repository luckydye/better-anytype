...
class Survey {
	...
	checkPmf(type: I.SurveyType) {
		...
		const lastCompleted =
			Number(obj.time || Storage.get("lastSurveyTime")) || 0;
		const lastCanceled =
			Number(obj.time || Storage.get("lastSurveyCanceled")) || 0;
		const week = 86400 * 7;
		const month = 86400 * 30;

		const registerTime = timeRegister <= time - week;
		const completeTime =
			obj.complete && registerTime && lastCompleted <= time - month;
		const cancelTime =
			obj.cancel && registerTime && lastCanceled <= time - month;
	...
	checkRegister(type: I.SurveyType) {
		...
		const surveyTime =
			timeRegister && U.Date.now() - 86400 * 7 - timeRegister > 0;
	...
	checkShared(type: I.SurveyType) {
		...
		const { account } = S.Auth;
		const check = U.Space.getList().filter(
			(it) =>
				it.isShared &&
				it.creator ==
					U.Space.getParticipantId(it.targetSpaceId, account.id),
		);
		if (!check.length) {
			return;
		}
	...
	checkMultiplayer(type: I.SurveyType) {
		...
		const timeRegister = this.getTimeRegister();
		const surveyTime =
			timeRegister && timeRegister <= U.Date.now() - 86400 * 7;
