import * as amplitudeEx from "amplitude-js";
import { I, C, S, U, J, Storage, Relation } from "Lib";

const amplitude = amplitudeEx.default;
...
class Analytics {
	...
	isAllowed(): boolean {
		...
		return (
			!(
				config.sudo ||
				["alpha"].includes(config.channel) ||
				!U.Common.getElectron().isPackaged
			) || this.debug()
		);
	}
	...
	setVersion() {
		...
		if (config.sudo || !isPackaged || ["alpha"].includes(config.channel)) {
			ret.push("dev");
		} else if (["beta"].includes(config.channel)) {
	...
	event(code: string, data?: any) {
		...
		switch (code) {
			...
			case "ChangeBlockStyle": {
				...
				switch (data.type) {
					...
					case I.BlockType.Embed: {
						data.processor =
							I.EmbedProcessor[
								Number(data.params.processor) || 0
							];
						delete data.style;
						break;
					}
			...
			case "DeleteWidget": {
				...
				if (target) {
					data.type = J.Constant.widgetId[target.id]
						? target.name
						: this.typeMapper(target.type);
				}
			...
			case "ClickOnboardingTooltip": {
				data.id = data.id
					? U.Common.toUpperCamelCase(`-${data.id}`)
					: "";
				data.type = data.type
					? U.Common.toUpperCamelCase(`-${data.type}`)
					: "";
				break;
			}
	...
	changeRelationValue(relation: any, value: any, type: string) {
		...
		if (relation.relationKey == "name") {
			...
			key = Relation.checkRelationValue(relation, value)
				? "ChangeRelationValue"
				: "DeleteRelationValue";
		}
		this.event(key, {
	...
	networkType(v: any): string {
		...
		switch (v) {
			case I.NetworkMode.Default:
				return "Anytype";
			case I.NetworkMode.Local:
				return "LocalOnly";
			case I.NetworkMode.Custom:
				return "SelfHost";
		}
		return "";
	}
