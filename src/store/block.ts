...
class BlockStore {
	...
	updateStructure(rootId: string, blockId: string, childrenIds: string[]) {
		...
		if (!element) {
			...
			map.set(
				blockId,
				new M.BlockStructure({ parentId: "", childrenIds }),
			);
		} else {
	...
	getTableData(rootId: string, blockId: string) {
		...
		const columnContainer = children.find((it) =>
			it.isLayoutTableColumns(),
		);
		...
		const rows = rowContainer
			? this.getChildren(rootId, rowContainer.id, (it) => it.isTableRow())
			: [];
	...
	updateMarkup(rootId: string) {
		...
		for (const block of blocks) {
			...
			for (let n = 0; n < marks.length; ++n) {
				...
				} else if (U.Object.isInFileLayouts(object.layout)) {
	...
	checkTypeSelect(rootId: string) {
		...
		if (change) {
			childrenIds = exists
				? childrenIds.filter((it) => it != J.Constant.blockId.type)
				: [J.Constant.blockId.type].concat(childrenIds);
		}

		if (change) {
			this.updateStructure(
				rootId,
				J.Constant.blockId.header,
				childrenIds,
			);
		}
	}
	...
	checkBlockTypeExists(rootId: string): boolean {
		const header = this.getMapElement(rootId, J.Constant.blockId.header);
		return header
			? header.childrenIds.includes(J.Constant.blockId.type)
			: false;
	}
