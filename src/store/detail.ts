...
class DetailStore {
	...
	public update(rootId: string, item: Item, clear: boolean): void {
		...
		if (U.Object.isRelationLayout(item.details.layout)) {
			S.Record.relationKeyMapSet(
				item.details.spaceId,
				item.details.relationKey,
				item.details.id,
			);
		}
		if (U.Object.isTypeLayout(item.details.layout)) {
			S.Record.typeKeyMapSet(
				item.details.spaceId,
				item.details.uniqueKey,
				item.details.id,
			);
		}
	...
	public get(
		...
		const keys = new Set(
			withKeys
				? [...withKeys, ...(!forceKeys ? J.Relation.default : [])]
				: [],
		);
		const object = { id };
	...
	private mapCommon(object: any) {
		object.name =
			Relation.getStringValue(object.name) ||
	...
	private mapType(object: any) {
		object.recommendedLayout =
			Number(object.recommendedLayout) || I.ObjectLayout.Page;
		object.recommendedRelations = Relation.getArrayValue(
		...
		object.defaultTemplateId = Relation.getStringValue(
	...
	private mapRelation(object: any) {
		object.relationFormat =
			Number(object.relationFormat) || I.RelationType.LongText;
		object.format = object.relationFormat;
		object.maxCount =
			Number(object.maxCount || object.relationMaxCount) || 0;
		object.objectTypes = Relation.getArrayValue(
			object.objectTypes || object.relationFormatObjectTypes,
		);
		object.isReadonlyRelation = Boolean(
			object.isReadonlyRelation || object.isReadonly,
		);
		object.isReadonlyValue = Boolean(
			object.isReadonlyValue || object.relationReadonlyValue,
		);
		object.isInstalled = object.spaceId != J.Constant.storeSpaceId;
		object.sourceObject = Relation.getStringValue(object.sourceObject);
	...
	private mapOption(object: any) {
		...
		object.color = Relation.getStringValue(
			object.color || object.relationOptionColor,
		);
	...
	private mapSpaceView(object: any) {
		object.spaceAccessType =
			Number(object.spaceAccessType) || I.SpaceType.Private;
		object.spaceAccountStatus =
			Number(object.spaceAccountStatus) || I.SpaceStatus.Unknown;
		object.spaceLocalStatus =
			Number(object.spaceLocalStatus) || I.SpaceStatus.Unknown;
		...
		object.spaceDashboardId = Relation.getStringValue(
			object.spaceDashboardId,
		);
		...
		object.isAccountActive = [
			I.SpaceStatus.Unknown,
			I.SpaceStatus.Active,
		].includes(object.spaceAccountStatus);
		object.isAccountJoining =
			object.spaceAccountStatus == I.SpaceStatus.Joining;
		object.isAccountRemoving =
			object.spaceAccountStatus == I.SpaceStatus.Removing;
		object.isAccountDeleted =
			object.spaceAccountStatus == I.SpaceStatus.Deleted;

		// Local status
		object.isLocalOk = [I.SpaceStatus.Unknown, I.SpaceStatus.Ok].includes(
			object.spaceLocalStatus,
		);
	...
	private mapParticipant(object) {
		object.permissions =
			Number(object.permissions || object.participantPermissions) ||
			I.ParticipantPermissions.Reader;
		object.status =
			Number(object.status || object.participantStatus) ||
