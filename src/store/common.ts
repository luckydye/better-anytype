...
class CommonStore {
	...
	constructor() {
		...
		intercept(this.configObj as any, (change) =>
			U.Common.intercept(this.configObj, change),
		);
	}
	...
	get type(): string {
		const key = String(
			this.defaultType ||
				Storage.get("defaultType") ||
				J.Constant.default.typeKey,
		);
	...
	get pinTime(): number {
		return (
			(Number(this.pinTimeId) ||
				Storage.get("pinTime") ||
				J.Constant.default.pinTime) * 1000
		);
	}

	get emailConfirmationTime(): number {
		return (
			Number(this.emailConfirmationTimeId) ||
			Storage.get("emailConfirmationTime") ||
			0
		);
	}
	...
	setThemeClass() {
		...
		if (c) {
			head.append(
				`<link id="link-prism" rel="stylesheet" href="./css/theme/${c}/prism.css" />`,
			);
		}
	...
	membershipTiersListSet(list: I.MembershipTier[]) {
		this.membershipTiersList = (list || []).map(
			(it) => new M.MembershipTier(it),
		);
	}
