...
const PreviewComponent = observer(
	class PreviewComponent extends React.Component<object, State> {
		ref = null;
		state = {
			object: null,
		};
		...
		onEdit(e) {
			...
			const rect = U.Common.objectCopy(
				$("#preview").get(0).getBoundingClientRect(),
			);

			S.Menu.open("blockLink", {
				rect: rect
					? { ...rect, height: 0, y: rect.y + win.scrollTop() }
					: null,
		...
		onUnlink() {
			...
			onChange(
				Mark.toggleLink(
					{ type: this.getMarkType(), param: "", range },
					preview.marks,
				),
			);
			Preview.previewHide(true);
		...
		position() {
			...
			let pe = ps + (nw / ow) * 100;
			let cpTop =
				"polygon(0% 0%, " + ps + "% 100%, " + pe + "% 100%, 100% 0%)";
			let cpBot =
				"polygon(0% 100%, " + ps + "% 0%, " + pe + "% 0%, 100% 100%)";
			...
			window.setTimeout(() => {
				obj.css({ opacity: 1, transform: "translateY(0%)" });
			}, 15);
		}
	},
);
