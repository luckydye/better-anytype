...
const PreviewObject = observer(
	class PreviewObject extends React.Component<Props, State> {
		state = {
			loading: false,
		};
		...
		render() {
			...
			const childBlocks = S.Block.getChildren(
				contextId,
				rootId,
				(it) => !it.isLayoutHeader(),
			).slice(0, 10);
			...
			const Block = (item: any) => {
				...
				switch (item.type) {
					...
					case I.BlockType.File: {
						if (
							[I.FileState.Empty, I.FileState.Error].indexOf(
								content.state,
							) >= 0
						) {
							break;
						...
						inner = (
							<div className="bookmark">
								...
								<div
									...
									style={{
										backgroundImage: `url("${S.Common.imageUrl(content.imageHash, 170)}")`,
									}}
								/>
							</div>
						);
				...
				return (
					<div className={cn.join(" ")} style={item.css}>
						{inner ? <div className="inner">{inner}</div> : ""}
								...
								<div className="scroller">
									{object.templateIsBundled ? (
										<Icon
											className="logo"
											tooltip={translate(
												"previewObjectTemplateIsBundled",
											)}
										/>
									) : (
										""
