...
const Group = observer(
	class Group extends React.Component<Props> {
		...
		render() {
			...
			return (
				<div ref={(node) => (this.node = node)} className="group">
					<div
						id={`item-${id}`}
						className="clickable"
			...
			U.Data.searchSubscribe(
				...
				() => {
					S.Record.recordsSet(
						subId,
						"",
						this.applyObjectOrder(
							id,
							S.Record.getRecordIds(subId, ""),
						),
					);
				},
			);
		...
		getItems() {
			const { id } = this.props;
			const subId = this.getSubId();
			const records = U.Common.objectCopy(
				S.Record.getRecordIds(subId, ""),
			);
		...
		onAll(e: any) {
			...
			U.Object.openEvent(e, {
				...object,
				_routeParam_: { viewId: parent.content.viewId },
			});
		}
	},
);
