...
const WidgetViewList = observer(
	class WidgetViewList extends React.Component<I.WidgetViewComponent> {
		node = null;
		refList = null;
		cache: any = null;
		top = 0;
		...
		onSortEnd(result: any) {
			...
			if (
				oldIndex == newIndex ||
				targetBlockId != J.Constant.widgetId.favorite
			) {
				return;
			}
			...
			const children = S.Block.getChildren(root, root, (it) =>
				it.isLink(),
			);
			const ro = records[oldIndex];
			const rn = records[newIndex];
			const oidx = children.findIndex(
				(it) => it.content.targetBlockId == ro,
			);
			const nidx = children.findIndex(
				(it) => it.content.targetBlockId == rn,
			);
			...
			S.Block.updateStructure(
				root,
				root,
				arrayMove(childrenIds, oidx, nidx),
			);
			Action.move(root, root, target.id, [current.id], position);
		}

		getItems() {
			const { block, addGroupLabels, isPreview, getRecordIds, subId } =
				this.props;
			const { targetBlockId } = block.content;
			const isRecent = [
			...
			let items = getRecordIds().map((id) =>
				S.Detail.get(subId, id, J.Relation.sidebar),
			);
		...
		isCompact() {
			...
			return (
				!isSystemTarget() ||
				[I.WidgetLayout.Compact, I.WidgetLayout.View].includes(
