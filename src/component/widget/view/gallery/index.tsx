...
import { type I, S, U, J, translate } from "Lib";
import { Icon } from "Component";
import Item from "./item";

const WidgetViewGallery = observer(
	class WidgetViewGallery extends React.Component<I.WidgetViewComponent> {
		_isMounted = false;
		node = null;
		page = 0;
		maxPages = 0;
		...
		componentDidMount() {
			...
			$(window)
				.off(`sidebarResize.${block.id}`)
				.on(`sidebarResize.${block.id}`, () => this.resize());
		}
		...
		setPage(animate: boolean) {
			...
			itemObj.css({
				transform: `translate3d(${-this.page * (width + 8)}px, 0px, 0px)`,
			});
		}
