...
const WidgetView = observer(
	class WidgetView extends React.Component<I.WidgetComponent, State> {
		node = null;
		state = {
			isLoading: false,
		};
		...
		render(): React.ReactNode {
			...
			const views = S.Record.getViews(
				rootId,
				J.Constant.blockId.dataview,
			).map((it) => ({
				...it,
				name: it.name || translate("defaultNamePage"),
			}));
			const viewType = this.getViewType();
			const cn = ["innerWrap"];
			const showEmpty = ![I.ViewType.Calendar, I.ViewType.Board].includes(
				viewType,
			);
				...
				content = (
					<div className="emptyWrap">
						<Label
							...
							text={
								canCreate
									? translate("widgetEmptyLabelCreate")
									: translate("widgetEmptyLabel")
							}
						/>
						{canCreate ? (
							<Button
		...
		updateData() {
			...
			const srcBlock = S.Block.getLeaf(
				targetId,
				J.Constant.blockId.dataview,
			);
			...
			if (srcBlock) {
				...
				if (!srcObject._empty_) {
					S.Detail.update(
						rootId,
						{ id: srcObject.id, details: srcObject },
						false,
					);
				}
			}
		...
		updateViews() {
			...
			const { targetBlockId } = block.content;
			const views = U.Common.objectCopy(
				S.Record.getViews(targetBlockId, J.Constant.blockId.dataview),
			).map((it) => new M.View(it));
			const rootId = this.getRootId();
		...
		load(viewId: string) {
			...
			Dataview.getData({
				...
				keys: J.Relation.sidebar
					.concat([view.groupRelationKey, view.coverRelationKey])
		...
		getView() {
			return Dataview.getView(
				this.getRootId(),
				J.Constant.blockId.dataview,
				this.props.parent.content.viewId,
			);
		}
		...
		getObject() {
			return S.Detail.get(
				S.Block.widgets,
				this.props.block.getTargetObjectId(),
			);
		}
		...
		getRecordIds() {
			...
			const viewId =
				parent.content.viewId || (views.length ? views[0].id : "");
			const ret = Dataview.applyObjectOrder(
				rootId,
				J.Constant.blockId.dataview,
				viewId,
				"",
				U.Common.objectCopy(records),
			);

			return targetBlockId == J.Constant.widgetId.favorite
				? sortFavorite(ret)
				: ret;
		}
		...
		isAllowedObject() {
			...
			let isAllowed = S.Block.checkFlags(
				rootId,
				J.Constant.blockId.dataview,
