...
const WidgetViewCalendar = observer(
	class WidgetViewCalendar extends React.Component<
		I.WidgetViewComponent,
		State
	> {
		node = null;
		refMonth = null;
		...
		getDateParam(t: number) {
			const [d, m, y] = U.Date.date("j,n,Y", t)
				.split(",")
				.map((it) => Number(it));
			return { d, m, y };
		}
			...
			S.Menu.closeAll(["dataviewCalendarDay"], () => {
				S.Menu.open("dataviewCalendarDay", {
					...
					data: {
						...
						onCreate: () => {
							const details = {};
							details[view.groupRelationKey] = U.Date.timestamp(
								y,
								m,
								d,
								12,
