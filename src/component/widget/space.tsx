...
const WidgetSpace = observer(
	class WidgetSpace extends React.Component<I.WidgetComponent> {
		node = null;
		...
		render(): React.ReactNode {
			...
			const participants = U.Space.getParticipantsList([
			...
			if (space && !space._empty_) {
				if (space.isShared) {
					status = U.Common.sprintf(
						"%d %s",
						memberCnt,
						U.Common.plural(memberCnt, translate("pluralMember")),
					);
				} else {
					status = translate(
		...
		setCnt() {
			...
			const requestCnt = participants.filter(
				(it) => it.isJoining || it.isRemoving,
			).length;
