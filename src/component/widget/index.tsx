...
const WidgetIndex = observer(
	class WidgetIndex extends React.Component<Props> {
		node = null;
		ref = null;
		timeout = 0;
		subId = "";
		...
		render() {
			...
			const withSelect =
				!this.isSystemTarget() &&
				(!isPreview || !U.Common.isPlatformMac());
				...
				buttons = (
					<div className="buttons">
						...
						<div className="iconWrap collapse">
							<Icon
								className="collapse"
								tooltip={translate("widgetToggle")}
			...
			if (layout != I.WidgetLayout.Space) {
				const onClick = this.isSystemTarget()
					? this.onSetPreview
					: this.onClick;
			...
			return (
				<div
					...
					onDragOver={(e) =>
						onDragOver ? onDragOver(e, block.id) : null
					}
		...
		rebind() {
			...
			win.on(
				`updateWidgetData.${block.id}`,
				() => this.ref && this.ref.updateData && this.ref.updateData(),
			);
			win.on(
				`updateWidgetViews.${block.id}`,
				() =>
					this.ref && this.ref.updateViews && this.ref.updateViews(),
			);
		}
		...
		getTargetBlock(): I.Block {
			...
			return childrenIds.length
				? S.Block.getLeaf(widgets, childrenIds[0])
				: null;
		}
			...
			switch (targetBlockId) {
				...
				case J.Constant.widgetId.collection: {
					object = {
						id: targetBlockId,
						name: translate(
							U.Common.toCamelCase(`widget-${targetBlockId}`),
						),
					};
					break;
				}
		...
		onClick(e: React.MouseEvent): void {
			if (!e.button) {
				U.Object.openEvent(e, {
					...this.getObject(),
					_routeParam_: { viewId: this.props.block.content.viewId },
				});
			}
		}
		...
		onCreate(param?: any): void {
			...
			if (isSetOrCollection) {
				...
				const view = Dataview.getView(
					rootId,
					J.Constant.blockId.dataview,
					viewId,
				);
				const typeId = Dataview.getTypeId(
					rootId,
					J.Constant.blockId.dataview,
					object.id,
					viewId,
		...
		getMinHeight() {
			return [I.WidgetLayout.List, I.WidgetLayout.Compact].includes(
				this.props.block.content.layout,
			)
				? 8
				: 0;
		}
			...
			switch (targetBlockId) {
				...
				case J.Constant.widgetId.recentEdit: {
					filters.push({
						operator: I.FilterOperator.And,
						relationKey: "lastModifiedDate",
						condition: I.FilterCondition.Greater,
						value: space.createdDate + 3,
					});
					break;
				}
		...
		getFavoriteIds(): string[] {
			const { root } = S.Block;
			const ids = S.Block.getChildren(root, root, (it) =>
				it.isLink(),
			).map((it) => it.content.targetBlockId);
			const items = ids
		...
		sortFavorite(records: string[]): string[] {
			...
			let sorted = U.Common.objectCopy(records || []).sort(
				(c1: string, c2: string) => {
		...
		onSetPreview() {
			...
			if (!isPreview) {
				blockId = block.id;
				event = "SelectHomeTab";
				data.tab = this.isSystemTarget()
					? object.name
					: analytics.typeMapper(object.type);
			}
		...
		isSystemTarget(): boolean {
			...
			return Object.values(J.Constant.widgetId).includes(
		...
		canCreate(): boolean {
			...
			const { layout } = block.content;
			const target = this.getTargetBlock();
			const layoutWithPlus = [
				I.WidgetLayout.List,
				I.WidgetLayout.Tree,
				I.WidgetLayout.Compact,
				I.WidgetLayout.View,
			].includes(layout);
			const isRecent = target
				? [
						J.Constant.widgetId.recentOpen,
						J.Constant.widgetId.recentEdit,
					].includes(target.getTargetObjectId())
				: null;
		...
		getRootId(): string {
			const target = this.getTargetBlock();
			return target
				? [target.content.targetBlockId, "widget", target.id].join("-")
				: "";
		}
		...
		getLimit({ limit, layout }): number {
			...
			const options = U.Menu.getWidgetLimitOptions(layout).map((it) =>
				Number(it.id),
			);
		...
		addGroupLabels(records: any[], widgetId: string) {
			...
			return U.Data.groupDateSections(records, relationKey, {
				type: "",
				links: [],
			});
		}
