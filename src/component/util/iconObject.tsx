...
const icons = import.meta.glob("/src/assets/img/icon/**/*.svg", {
	eager: true,
	query: "?url",
});
const themeIcons = import.meta.glob("/src/assets/img/theme/**/*.svg", {
	eager: true,
	query: "?url",
});

function getIcon(key: string) {
	return Object.entries(icons).find(([k]) => k.match(key))?.[1]?.default;
}
...
for (const i in I.RelationType) {
	...
	if (
		...
		[I.RelationType.Icon, I.RelationType.Relations].includes(it)
	) {
		continue;
	}

	const iconKey = U.Common.toCamelCase(I.RelationType[i]);

	Relation.small[i] = getIcon(`img/icon/relation/small/${iconKey}`);
	Relation.big[i] = getIcon(`img/icon/relation/bug/${iconKey}`);
}
...
const CheckboxTask = {
	"": {
		0: getIcon("img/icon/object/checkbox0.svg"),
		1: getIcon("img/icon/object/checkbox1.svg"),
		2: getIcon("img/icon/object/checkbox2.svg"),
	},
	dark: {
		0: getIcon("img/icon/object/checkbox0.svg"),
		1: getIcon("img/theme/dark/icon/object/checkbox1.svg"),
		2: getIcon("img/icon/object/checkbox2.svg"),
	},
};

const IconObject = observer(
	class IconObject extends React.Component<Props> {
		...
		render() {
			...
			const cn = [
				"iconObject",
				"c" + size,
				U.Data.layoutClass(object.id, layout),
			];
			...
			switch (layout) {
				...
				case I.ObjectLayout.Participant: {
					if (iconImage) {
						...
						icon = (
							<img
								src={S.Common.imageUrl(iconImage, iconSize * 2)}
								className={icn.join(" ")}
							/>
						);
					} else {
				...
				case I.ObjectLayout.Task: {
					...
					icon = (
						<img
							id="checkbox"
							src={
								done ? CheckboxTask[tc][2] : CheckboxTask[tc][0]
							}
							className={icn.join(" ")}
						/>
					);
				...
				case I.ObjectLayout.Relation: {
					...
					if (Relation[key][relationFormat]) {
						...
						icon = (
							<img
								src={Relation[key][relationFormat]}
								className={icn.join(" ")}
							/>
						);
					}
				...
				case I.ObjectLayout.Image: {
					if (id) {
						...
						icon = (
							<img
								src={S.Common.imageUrl(id, iconSize * 2)}
								className={icn.join(" ")}
							/>
						);
					} else {
		...
		onEmoji(e: any) {
			...
			const object = this.getObject();
			const noGallery =
				this.props.noGallery ||
				[I.ObjectLayout.SpaceView, I.ObjectLayout.Human].includes(
					object.layout,
				);
			const noUpload =
				this.props.noUpload ||
				[I.ObjectLayout.Type].includes(object.layout);
		...
		iconSize() {
			...
			if (
				[I.ObjectLayout.Human, I.ObjectLayout.Participant].includes(
					layout,
				) &&
				size >= 40
			) {
			...
			if (
				[I.ObjectLayout.Set, I.ObjectLayout.SpaceView].includes(
					layout,
				) &&
				iconImage
			) {
			...
			if (
				[I.ObjectLayout.Set, I.ObjectLayout.Type].indexOf(layout) >=
					0 &&
				!iconImage &&
				!iconEmoji &&
				size >= 40
			...
			if (
				[I.ObjectLayout.Task, I.ObjectLayout.Relation].indexOf(layout) <
					0 &&
				forceLetter &&
				!iconImage &&
				!iconEmoji &&
		...
		fontSize(layout: I.ObjectLayout, size: number) {
			...
			if (
				size == 64 &&
				[I.ObjectLayout.Type, I.ObjectLayout.Set].indexOf(layout) >= 0
			) {
				s = 44;
			}
