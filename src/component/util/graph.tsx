	...
	class Graph extends React.Component<Props> {
		...
		rebind() {
			...
			win.on("updateGraphRoot.graph", (e: any, data: any) =>
				this.setRootId(data.id),
			);
			win.on("removeGraphNode.graph", (e: any, data: any) =>
				this.send("onRemoveNode", {
					ids: U.Common.objectCopy(data.ids),
				}),
			);
			...
			d3.select(this.canvas)
				...
				.on("click", (e: any) => {
					...
					if (local) {
						...
						this.send(e.shiftKey ? "onSelect" : "onClick", {
							x,
							y,
						});
					}
		...
		updateSettings() {
			this.send(
				"updateSettings",
				S.Common.getGraph(this.props.storageKey),
			);
		...
		onPreviewShow(data: any) {
			...
			if (!el.length || render) {
				...
				analytics.event("SelectGraphNode", {
					objectType: this.subject.type,
					layout: this.subject.layout,
				});
			} else {
				position();
			}
