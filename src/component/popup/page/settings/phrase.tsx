...
const PopupSettingsPagePhrase = observer(
	class PopupSettingsPagePhrase extends React.Component<
		I.PopupSettings,
		State
	> {
		node: any = null;
		...
		render() {
			...
			return (
				<div ref={(node) => (this.node = node)}>
					...
					<Title
						className="sub"
						text={translate("popupSettingsMobileQRSubTitle")}
					/>
					<Label
						className="description"
						text={translate("popupSettingsMobileQRText")}
					/>
						...
						<div className={!showCode ? "isBlurred" : ""}>
							<QRCode
								value={
									showCode
										? entropy
										: translate("popupSettingsCodeStub")
								}
