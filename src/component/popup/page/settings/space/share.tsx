...
const PopupSettingsSpaceShare = observer(
	class PopupSettingsSpaceShare extends React.Component<
		I.PopupSettings,
		State
	> {
		node: any = null;
		...
		render() {
			...
			if (space.isShared) {
				if (!U.Space.getReaderLimit() && membership.isExplorer) {
					limitLabel = translate(
						"popupSettingsSpaceShareInvitesReaderLimitReachedLabel",
					);
					limitButton = translate(
						"popupSettingsSpaceShareInvitesReaderLimitReachedButton",
					);
					memberUpgradeType = "members";
					showLimit = true;
				} else if (!U.Space.getWriterLimit()) {
					limitLabel = translate(
						"popupSettingsSpaceShareInvitesWriterLimitReachedLabel",
					);
					limitButton = translate(
						"popupSettingsSpaceShareInvitesWriterLimitReachedButton",
					);
			...
			const Member = (item: any) => {
				...
				if (item.isJoining) {
					tag = (
						<Tag
							text={translate(
								"popupSettingsSpaceShareJoinRequest",
							)}
						/>
					);
					button = (
						<Button
							...
							color="blank"
							text={translate(
								"popupSettingsSpaceShareViewRequest",
							)}
					...
					tag = (
						<Tag
							text={translate(
								"popupSettingsSpaceShareLeaveRequest",
							)}
						/>
					);
					button = (
				...
				} else if (item.isDeclined || item.isRemoved) {
					button = (
						<Label
							color="red"
							text={translate(`participantStatus${item.status}`)}
						/>
					);
					...
					button = (
						<Label
							color="grey"
							text={translate(
								`participantPermissions${I.ParticipantPermissions.Owner}`,
							)}
						/>
					);
				} else {
					button = (
						<div
							...
							className="select"
							onClick={() => this.onPermissionsSelect(item)}
						>
							<div className="item">
								<div className="name">
									{translate(
										`participantPermissions${item.permissions}`,
									)}
								</div>
							</div>
			...
			return (
				<div ref={(node) => (this.node = node)}>
					<Head
						{...this.props}
						returnTo="spaceIndex"
						name={translate("popupSettingsSpaceIndexTitle")}
					/>
					...
					<div id="titleWrapper" className="titleWrapper">
						...
						<div className="icons">
							...
							{space.isShared ? (
								<Icon
									id="button-more-space"
									className="more"
									onClick={this.onMoreSpace}
								/>
							) : (
								""
							)}
					...
					<div id="sectionInvite" className="section sectionInvite">
						<Title
							text={translate(
								"popupSettingsSpaceShareInviteLinkTitle",
							)}
						/>
						<Label
							text={translate(
								"popupSettingsSpaceShareInviteLinkLabel",
							)}
						/>
						...
						{hasLink ? (
							<div className="inviteLinkWrapper">
								<div className="inputWrapper">
									<Input
										...
										readonly={true}
										value={U.Space.getInviteLink(cid, key)}
								...
								<Button
									ref={(ref) => (this.refCopy = ref)}
									onClick={this.onCopy}
									className="c40"
									color="blank"
									text={translate("commonCopyLink")}
								/>
							</div>
						) : (
							<div className="buttons">
								<Button
									...
									text={translate(
										"popupSettingsSpaceShareGenerateInvite",
									)}
								/>
							</div>
						)}
					...
					<div id="sectionMembers" className="section sectionMembers">
						<Title
							text={translate(
								"popupSettingsSpaceShareMembersTitle",
							)}
						/>
						...
						{showLimit ? (
							<div className="row payment">
								...
								<Button
									...
									text={limitButton}
									onClick={() =>
										this.onUpgrade(memberUpgradeType)
									}
								/>
							</div>
						) : (
						...
						{this.cache ? (
							<div id="list" className="rows">
								<WindowScroller
									scrollElement={$(
										"#popupSettings-innerWrap .mainSides #sideRight",
									).get(0)}
								>
									{({
										height,
		...
		onUpgrade(type: string) {
			...
			analytics.event("ClickUpgradePlanTooltip", {
				type,
				route: analytics.route.settingsSpaceShare,
			});
		}

		getParticipantList() {
			const records = U.Space.getParticipantsList([
				I.ParticipantStatus.Joining,
				I.ParticipantStatus.Removing,
				I.ParticipantStatus.Active,
			]);
			...
			C.SpaceMakeShareable(S.Common.space, (message: any) => {
				...
				C.SpaceInviteGenerate(S.Common.space, (message: any) => {
					...
					if (!this.setError(message.error)) {
						...
						Preview.toastShow({
							text: translate("toastInviteGenerate"),
						});
		...
		getParticipantOptions() {
			...
			items.push({
				id: "remove",
				name: translate("popupSettingsSpaceShareRemoveMember"),
				color: "red",
			});
			...
			switch (v) {
				case "remove": {
					...
					text = U.Common.sprintf(
						translate("popupConfirmMemberRemoveText"),
						item.name,
					);
				...
				default: {
					...
					text = U.Common.sprintf(
						translate("popupConfirmMemberChangeText"),
						item.name,
						translate(`participantPermissions${v}`),
					);

					onConfirm = () => {
						C.SpaceParticipantPermissionsChange(space, [
							{ identity: item.identity, permissions: Number(v) },
						]);

						analytics.event("ChangeSpaceMemberPermissions", {
							type: v,
						});
					};
					break;
			...
			const options = [
				{
					id: "stop-sharing",
					color: "red",
					name: translate("popupSettingsSpaceShareStopSharing"),
				},
			];
		...
		resize() {
			...
			const mh =
				...
				sectionInvite.outerHeight(true) -
