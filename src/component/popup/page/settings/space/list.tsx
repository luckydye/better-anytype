...
const PopupSettingsPageSpacesList = observer(
	class PopupSettingsPageSpacesList extends React.Component<I.PopupSettings> {
		render() {
			...
			const Row = (space: any) => {
				...
				return (
					<div className="row">
						<div
							className="col colSpace"
							onClick={() => this.onClick(space)}
						>
						...
						<div className="col">
							{translate(
								`spaceStatus${space.spaceAccountStatus}`,
							)}
						</div>
						<div className="col colMore">
							{hasMenu ? (
								<Icon
									id={`icon-more-${space.id}`}
									className="more"
									onClick={() => this.onMore(space)}
								/>
							) : (
								""
							)}
					...
					<div className="items">
						<div className="row isHead">
							<div className="col colSpace">
								{translate("popupSettingsSpacesListSpace")}
							</div>
							<div className="col">
								{translate("popupSettingsSpacesListAccess")}
							</div>
							<div className="col">
								{translate("popupSettingsSpacesListNetwork")}
							</div>
							<div className="col colMore" />
						</div>
		...
		getItems() {
			...
			return items
				.filter((it) => !it.isAccountDeleted && it.isLocalOk)
				.map((it) => {
					it.participant =
						U.Space.getMyParticipant(it.targetSpaceId) || {};
					return it;
				})
				.sort((c1, c2) => {
					...
					if (c1.isAccountRemoving && !c2.isAccountRemoving)
						return -1;
