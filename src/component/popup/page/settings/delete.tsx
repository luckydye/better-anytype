...
const PopupSettingsPageDelete = observer(
	class PopupSettingsPageDelete extends React.Component<
		I.PopupSettings,
		State
	> {
		...
		render() {
			...
			return (
				<div ref={(node) => (this.node = node)}>
					<Head
						{...this.props}
						returnTo="dataManagement"
						name={translate("commonBack")}
					/>
					<Title
						text={translate("popupSettingsAccountDeleteTitle")}
					/>

					<div className="text">
						<b>{translate("popupSettingsDeleteTitle1")}</b>
						<p className="first">
							{translate("popupSettingsDeleteText1")}
						</p>
					...
					<div className="check" onClick={this.onCheck}>
						<Checkbox
							...
							value={false}
						/>{" "}
						{translate("popupSettingsDeleteCheckboxLabel")}
					</div>

					<Button
						ref={(ref) => (this.refButton = ref)}
						text={translate("commonDelete")}
						color="red"
						className="c36"
