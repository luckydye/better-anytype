...
const PopupMembershipPagePaid = observer(
	class PopupMembershipPagePaid extends React.Component<I.Popup, State> {
		state = {
			status: "",
			statusText: "",
		};
		...
		render() {
			...
			if (membership.tier == I.TierType.Builder) {
				switch (paymentMethod) {
					...
					case I.PaymentMethod.Google: {
						platformText = translate(
							"popupMembershipPaidOnOtherPlatform",
						);
						canEnterName = false;
						break;
					}
			...
			if (period) {
				if (period == 1) {
					...
					periodText = U.Common.sprintf(
						translate("popupSettingsMembershipPerYears"),
						period,
						U.Common.plural(period, translate("pluralYear")),
					);
					labelText = U.Common.sprintf(
						translate("popupMembershipPaidTextPerYears"),
						period,
						U.Common.plural(period, translate("pluralYear")),
					);
				}
			...
			return (
				<form className="anyNameForm" onSubmit={this.onSubmit}>
					{tierItem.namesCount ? (
						<React.Fragment>
							<Title
								text={translate(`popupMembershipPaidTitle`)}
							/>
							...
							<div className="inputWrapper">
								<Input
									...
									placeholder={translate(
										`popupMembershipPaidPlaceholder`,
									)}
								/>
								<div className="ns">
									{J.Constant.namespace[nameType]}
								</div>
							</div>

							<div className={["statusBar", status].join(" ")}>
								{statusText}
							</div>
						</React.Fragment>
					) : (
					...
					<div className="priceWrapper">
						{tierItem.price ? (
							<span className="price">{`$${tierItem.price}`}</span>
						) : (
							""
						)}
						{periodText}
					...
					{platformText ? (
						<div className="platformLabel">
							<Label
								className="paidOnOtherPlatform"
								text={platformText}
							/>
							{withContactButton ? (
								<Button
									onClick={() => Action.membershipUpgrade()}
									text={translate(
										"popupMembershipWriteToAnyteam",
									)}
									className="c36"
									color="blank"
								/>
						...
						<React.Fragment>
							<Button
								...
								ref={(ref) => (this.refButtonCard = ref)}
								className="c36"
								text={translate("popupMembershipPayByCard")}
							/>
							<Button
								onClick={() =>
									this.onPay(I.PaymentMethod.Crypto)
								}
								ref={(ref) => (this.refButtonCrypto = ref)}
								className="c36"
								text={translate("popupMembershipPayByCrypto")}
							/>
		...
		onPay(method: I.PaymentMethod) {
			...
			const tierItem = U.Data.getMembershipTier(tier);
			const name =
				globalName || !tierItem.namesCount ? "" : this.getName();
			const refButton =
				method == I.PaymentMethod.Stripe
					? this.refButtonCard
					: this.refButtonCrypto;
			const cb = () => {
				C.MembershipRegisterPaymentRequest(
					tier,
					method,
					name,
					(message) => {
						...
						analytics.event("ClickMembership", {
							params: { tier, method },
						});
					},
				);
			...
			this.checkName(name, () => {
				...
				C.NameServiceResolveName(name, (message: any) => {
					...
					if (message.error.code) {
						...
						error = translate(
							"popupMembershipStatusNameNotAvailable",
						);
					}
					...
					if (error) {
						...
						this.setOk(
							translate("popupMembershipStatusNameAvailable"),
						);
