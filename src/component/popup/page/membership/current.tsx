...
const PopupMembershipPageCurrent = observer(
	class PopupMembershipPageCurrent extends React.Component<Props, State> {
		state = {
			verificationStep: 1,
			countdown: 60,
			hasCountdown: false,
		...
		render() {
			const { verificationStep, countdown, status, statusText } =
				this.state;
			...
			if (membership.isExplorer) {
				...
				if (!userEmail) {
						...
						case 1: {
							content = (
								<form onSubmit={this.onVerifyEmail}>
									<Title
										text={translate(
											`popupMembershipCurrentEmailTitle`,
										)}
									/>
									<Label
										text={translate(
											`popupMembershipCurrentEmailText`,
										)}
									/>
									...
									<div
										className={["statusBar", status].join(
											" ",
										)}
									>
										{statusText}
					...
					buttonText = "";
					verificationForm = (
						<div className="emailVerification">{content}</div>
					);
				}
			} else {
				if (paymentMethod != I.PaymentMethod.None) {
					paidText = U.Common.sprintf(
						translate("popupMembershipPaidBy"),
					...
					} else if (paymentMethod == I.PaymentMethod.Stripe) {
						buttonText = translate("popupMembershipManagePayment");
					} else {
						manageText = translate(
							"popupMembershipManageOnMobilePlatform",
						);
					}
				}
			}
			...
			return (
				<div className="currentMembership">
					...
					<div className="valid">
						<div>
							<Label
								text={translate("popupMembershipValidUntil")}
							/>
					...
					{manageText ? (
						<Label className="manageText" text={manageText} />
					) : (
						""
					)}
					{verificationForm}
		...
		onVerifyEmail(e: any) {
			...
			C.MembershipGetVerificationEmail(
				this.refEmail.getValue(),
				true,
				(message) => {
					...
					analytics.event("ClickMembership", {
						type: "Submit",
						params: { tier: I.TierType.Explorer },
					});
				},
			);
		}
