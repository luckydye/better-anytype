...
import { type I, C, S, U, translate, analytics } from "Lib";
import {
	AutoSizer,
	CellMeasurer,
	CellMeasurerCache,
...
class PopupUsecasePageList extends React.Component<I.PopupUsecase, State> {
	...
	render() {
		...
		let textEmpty = "";
		if (filter) {
			textEmpty = U.Common.sprintf(
				translate("popupUsecaseListEmptyFilter"),
				filter,
			);
		} else if (category) {
			textEmpty = U.Common.sprintf(
				translate("popupUsecaseListEmptyCategory"),
				category.name,
			);
		}
		...
		const Item = (item: any) => {
			const screenshot = item.screenshots.length
				? item.screenshots[0]
				: "";
		...
		return (
			<div ref={(ref) => (this.node = ref)} className="wrap">
				...
				<div className="banner" onClick={this.onBanner}>
					<div className="inner">
						{translate("popupUsecaseBannerText")}
					</div>
				</div>
				...
				<div className="items">
					{!items.length ? (
						...
						<WindowScroller
							scrollElement={$("#popupUsecase-innerWrap").get(0)}
						>
							{({
								height,
								isScrolling,
	...
	onCategory(item: any) {
		this.setState({
			category: item.id == this.state.category?.id ? null : item,
		});
	}
	...
	getItems() {
		...
		if (filter) {
			...
			items = items.filter(
				(it) => reg.test(it.title) || reg.test(it.description),
			);
		}
