...
const PopupMembershipFinalization = observer(
	class PopupMembershipFinalization extends React.Component<I.Popup, State> {
		state = {
			status: "",
			statusText: "",
			isLoading: false,
				...
				C.MembershipIsNameValid(tier, name, (message: any) => {
					...
					this.setState({
						statusText: translate(
							"popupMembershipStatusWaitASecond",
						),
					});
		...
		onConfirm() {
			...
			C.MembershipFinalize(name, (message) => {
				...
				U.Data.getMembershipStatus((membership) => {
					...
					S.Popup.replace("membershipFinalization", "membership", {
