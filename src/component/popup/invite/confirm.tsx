...
const PopupInviteConfirm = observer(
	class PopupInviteConfirm extends React.Component<I.Popup, State> {
		state = {
			error: "",
		};
		...
		render() {
			...
			const name = U.Common.shorten(
				String(data.name || translate("defaultNamePage")),
				32,
			);
			...
			if (!this.getReaderLimit() && membership.isExplorer) {
				buttons.push({
					...
					onClick: () => this.onMembership("members"),
				});
			} else if (!this.getWriterLimit()) {
				buttons = buttons.concat([
					{
						text: translate("popupInviteConfirmButtonReader"),
						onClick: () =>
							this.onConfirm(I.ParticipantPermissions.Reader),
					},
					{
						text: translate("popupInviteConfirmButtonWriterLimit"),
						onClick: () => this.onMembership("editors"),
					},
				]);
			} else {
				buttons = buttons.concat([
					...
					{
						text: translate("popupInviteConfirmButtonWriter"),
						onClick: () =>
							this.onConfirm(I.ParticipantPermissions.Writer),
					},
				]);
			}
				...
				<React.Fragment>
					...
					<Title
						text={U.Common.sprintf(
							translate("popupInviteConfirmTitle"),
							name,
							U.Common.shorten(space.name, 32),
						)}
					/>
					...
					<div className="buttons">
						...
						<Button
							...
							text={translate("popupInviteConfirmButtonReject")}
							className="c36"
							color="red"
						/>
					</div>
		...
		onMembership(type: string) {
			...
			analytics.event("ClickUpgradePlanTooltip", {
				type,
				route: analytics.route.inviteConfirm,
			});
		}

		onConfirm(permissions: I.ParticipantPermissions) {
			C.SpaceRequestApprove(
				this.getSpaceId(),
				this.getIdentity(),
				permissions,
				(message: any) => {
					...
					analytics.event("ApproveInviteRequest", {
						type: permissions,
					});
					this.props.close();
				},
		...
		getWriterLimit() {
			...
			const participants = this.participants.filter(
				(it) => it.isActive && (it.isWriter || it.isOwner),
			);
