...
const PopupExport = observer(
	class PopupExport extends React.Component<I.Popup> {
		data: any = {};
		...
		render() {
			...
			const Option = (item: any) => {
				...
				return (
					<div className="row">
						...
						<div className="value">{control}</div>
					</div>
				);
			};
			...
			let items: any[] = [
				{
					id: "format",
					name: translate("popupExportFormat"),
					control: "select",
					options: formats,
				},
			];
			...
			switch (format) {
				...
				case I.ExportType.Protobuf:
					if (format == I.ExportType.Protobuf) {
						items.push({
							id: "json",
							name: translate("popupExportFileFormat"),
							control: "select",
					...
					items = items.concat([
						{
							id: "zip",
							name: translate("popupExportZipArchive"),
							control: "switch",
						},
						{
							id: "nested",
							name: translate("popupExportIncludeLinkedObjects"),
							control: "switch",
						},
						{
							id: "files",
							name: translate("popupExportIncludeFiles"),
							control: "switch",
						},
						{
							id: "archived",
							name: translate(
								"popupExportIncludeArchivedObjects",
							),
							control: "switch",
						},
					]);
				...
				case I.ExportType.Pdf:
					items = items.concat([
						{
							id: "pageSize",
							name: translate("popupExportPageSize"),
							control: "select",
							options: pageSize,
						},
						{
							id: "landscape",
							name: translate("popupExportLandscape"),
							control: "switch",
						},
						{
							id: "printBg",
							name: translate("popupExportPrintBackground"),
							control: "switch",
						},
					]);
					break;
		...
		getFormats() {
			...
			return [
				...
				allowHtml && config.experimental
					? { id: I.ExportType.Html, name: "HTML" }
					: null,
			].filter((it) => it);
		}
		...
		onConfirm(e: any) {
			...
			switch (format) {
				default:
					Action.export(space, objectIds, format, {
						...this.data,
						route,
					});
					close();
