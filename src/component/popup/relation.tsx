...
const PopupRelation = observer(
	class PopupRelation extends React.Component<I.Popup, State> {
		refCheckbox = null;
		...
		render() {
			...
			return (
				<div>
					...
					{!relations.length ? (
						<EmptySearch text={translate("popupRelationEmpty")} />
					) : (
						<div className="blocks">
							{relations.map((item) => (
								<Item key={item.relationKey} {...item} />
							))}
						</div>
					)}
		...
		loadDeps(callBack?: () => void) {
			...
			for (const k in this.details) {
				...
				if (relation && Relation.isArrayType(relation.format)) {
					depIds = depIds.concat(
						Relation.getArrayValue(this.details[k]),
					);
				}
			}
		...
		initValues() {
			...
			objects.forEach((object) => {
				relations.forEach((relation) => {
					...
					const value = Relation.formatValue(
						relation,
						object[relationKey],
						false,
					);
					...
					if (
						reference &&
						U.Common.compareJSON(value, reference[relationKey])
					) {
						cnt[relationKey]++;
					}
		...
		getRelationKeys(): string[] {
			return U.Common.arrayUnique(
				[].concat(
					this.props.param.data.relationKeys || J.Relation.default,
				),
			);
		}
		...
		getRelations(): any[] {
			...
			let ret = this.getRelationKeys().map((relationKey) =>
				S.Record.getRelationByKey(relationKey),
			);

			ret = ret.filter((it) => {
				return (
					(config.debug.hiddenObject ? true : !it.isHidden) &&
					!it.isReadonlyValue
				);
			});
		...
		onCellChange(
			...
			this.details[relationKey] = Relation.formatValue(
				relation,
				value,
				true,
			);
			...
			S.Menu.open("relationSuggest", {
				...
				data: {
					...
					addCommand: (
						...
						this.details[relation.relationKey] =
							Relation.formatValue(relation, null, true);
						this.props.param.data.relationKeys =
							this.getRelationKeys().concat([
		...
		save() {
			...
			S.Popup.open("confirm", {
				data: {
					title: "Are you sure?",
					text: U.Common.sprintf(
						"This will update relation values of %d objects",
						objectIds.length,
					),
					onConfirm: () => {
						...
						if (
							this.addRelationKeys.length &&
							this.refCheckbox?.getValue()
						) {
							const cb = () => {
