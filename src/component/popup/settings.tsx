...
const PopupSettings = observer(
	class PopupSettings extends React.Component<I.Popup, State> {
		node = null;
		ref = null;
		state = {
			loading: false,
		...
		render() {
			...
			const Item = (action: any) => {
				...
				if (action.id == "account") {
					...
					icon = (
						<Icon
							className={`settings-${action.icon || action.id}`}
						/>
					);
					name = action.name;
				}
				...
				if (action.id == "membership") {
					if (!membership.isNone) {
						const tierItem = U.Data.getMembershipTier(
							membership.tier,
						);
						caption = (
							<div className="caption">{tierItem.name}</div>
						);
					} else {
						caption = (
							<div className="caption join">
								{translate(`commonJoin`)}
							</div>
						);
					}
				}
			...
			return (
				<div ref={(node) => (this.node = node)} className="mainSides">
					{sections.length ? (
		...
		unbind() {
			$(window).off(
				"resize.settings keydown.settings mousedown.settings",
			);
		}
		...
		getSections(): any[] {
			...
			if (isSpace) {
				...
				const settingsVault = [
					{
						id: "spaceList",
						name: translate("popupSettingsSpacesListTitle"),
						icon: "spaces",
					},
					{
						id: "dataManagement",
				...
				if (isAnytypeNetwork && isOnline) {
					settingsVault.push({
						id: "membership",
						icon: "membership",
						name: translate("popupSettingsMembershipTitle1"),
					});
				}
				...
				return [
					...
					{
						name: translate("popupSettingsApplicationTitle"),
						children: [
							{
								id: "personal",
							...
							},
							{
								...
								icon: "pin",
								subPages: ["pinSelect", "pinConfirm"],
							},
						],
					},
					{
						name: translate("popupSettingsAccountAndKeyTitle"),
						children: settingsVault,
					},
				];
			}
		}
		...
		onExport(type: I.ExportType, param: any) {
			...
			Action.export(
				...
				[],
				type,
				{ ...param, route: analytics.route.settings },
				() => this.props.close(),
			);
		}
		...
		onKeyDown(e: any) {
			keyboard.shortcut(
				U.Common.isPlatformMac() ? "cmd+[" : "alt+arrowleft",
				e,
				() => this.onBack(),
			);
		}
		...
		isSubPage(page) {
			...
			for (const item of items) {
				if (
					(item.subPages || []).includes(page) &&
					!(item.noHeader || []).includes(page)
				) {
					return true;
				}
			}
		...
		onSpaceTypeTooltip(e) {
			Preview.tooltipShow({
				title: translate(
					"popupSettingsSpaceIndexSpaceTypePersonalTooltipTitle",
				),
				text: translate(
					"popupSettingsSpaceIndexSpaceTypePersonalTooltipText",
				),
				className: "big",
