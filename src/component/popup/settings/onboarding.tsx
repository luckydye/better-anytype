...
const PopupSettingsOnboarding = observer(
	class PopupSettingsOnboarding extends React.Component<I.Popup> {
		config: any = {};
		refMode = null;
					...
					<div id="sideRight" className="side right tabOnboarding">
						...
						<div className="actionItems">
							<div className="item">
								<Label
									text={translate(
										"popupSettingsPersonalInterfaceLanguage",
									)}
								/>
							...
							<div className="item">
								<Label
									text={translate(
										"popupSettingsOnboardingModeTitle",
									)}
								/>
								<Select
									id="networkMode"
							...
							{mode == I.NetworkMode.Custom ? (
								<div
									className="item"
									onMouseEnter={(e) =>
										this.onTooltipShow(e, path)
									}
									onMouseLeave={this.onTooltipHide}
								>
									<div onClick={() => this.onPathClick(path)}>
										<Label
											text={translate(
												"popupSettingsOnboardingNetworkTitle",
											)}
										/>
										{path ? (
											<Label
												className="small"
												text={U.Common.shorten(
													path,
													32,
												)}
											/>
							...
							>
								<div onClick={() => this.onPathClick(userPath)}>
									<Label
										text={translate(
											"popupSettingsOnboardingStoragePath",
										)}
									/>
									<Label
										className="small"
										text={U.Common.shorten(userPath, 32)}
									/>
								</div>
								<div className="buttons">
									<Button
										className="c28"
										text={translate("commonChange")}
										onClick={this.onChangeStorage}
									/>
									{!isDefault ? (
										<Button
											className="c28"
											text={translate("commonReset")}
											onClick={this.onResetStorage}
										/>
									) : (
										""
									)}
		...
		onUpload() {
			Action.openFileDialog(["yml"], (paths: string[]) =>
		...
		onSave() {
			...
			if (this.config.mode !== networkConfig.mode) {
				analytics.event("SelectNetwork", {
					route: analytics.route.onboarding,
					type: this.config.mode,
				});
			}

			if (this.config.path !== networkConfig.path) {
				analytics.event("UploadNetworkConfiguration", {
					route: analytics.route.onboarding,
				});
			}
		...
		onConfirmStorage(onConfirm: () => void) {
			S.Popup.open("confirm", {
				data: {
					...
					text: translate(
						"popupSettingsOnboardingLocalOnlyWarningText",
					),
					textConfirm: translate(
						"popupSettingsOnboardingLocalOnlyWarningConfirm",
					),
					onConfirm,
				},
			});
