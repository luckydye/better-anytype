...
const PopupSearch = observer(
	class PopupSearch extends React.Component<I.Popup, State> {
		_isMounted = false;
		node: any = null;
		state = {
			isLoading: false,
			...
			const Context = (meta: any): any => {
				...
				if (relationKey) {
					if (["name", "type"].includes(relationKey)) {
						...
						key = relation ? (
							<div className="key">{relation.name}:</div>
						) : (
							""
						);
					}
				...
				if (highlight) {
					const text = Mark.toHtml(
						highlight,
						ranges.map((it) => ({
							type: I.MarkType.Highlight,
							range: it,
						})),
					);
					...
					value = (
						<div
							...
							dangerouslySetInnerHTML={{
								__html: U.Common.sanitize(text),
							}}
						/>
					);
				} else if (relationDetails.name) {
					const { relationOptionColor } = relationDetails;
					const color = relationOptionColor
						? `textColor-${relationOptionColor}`
						: "";
					const cn = ["value"];
					...
					if (item.links.length || item.backlinks.length) {
						advanced = (
							<Icon
								className="advanced"
								tooltip={translate(
									"popupSearchTooltipSearchByBacklinks",
								)}
						...
						<div className="sides">
							<div className="side left">
								<div
									className="name"
									dangerouslySetInnerHTML={{
										__html: U.Common.sanitize(name),
									}}
								/>
								<Context {...meta} />
								<div className="caption">{item.caption}</div>
							</div>
							<div className="side right">{advanced}</div>
						</div>
					);
				} else {
					content = (
					...
					{!items.length && !isLoading ? (
						<EmptySearch
							text={
								filter
									? U.Common.sprintf(
											translate("popupSearchEmptyFilter"),
											filter,
										)
		...
		load(clear: boolean, callBack?: () => void) {
			...
			const filters: any[] = [
				{
					...
					value: U.Object.getSystemLayouts(),
				},
				{
					operator: I.FilterOperator.And,
					relationKey: "type",
			...
			if (backlink) {
				filters.push({
					...
					value: [].concat(backlink.links, backlink.backlinks),
				});
			}
			...
			C.ObjectSearchWithMeta(
				filters,
				sorts,
				J.Relation.default.concat(["links", "backlinks"]),
				filter,
				this.offset,
				limit,
				(message) => {
		...
		getItems() {
			...
			const hasRelations =
				keyboard.isMainEditor() || keyboard.isMainSet();
			const filter = this.getFilter();
			...
			if (filter) {
				name = U.Common.sprintf(
					translate("commonCreateObjectWithName"),
					filter,
				);
			} else {
			...
			let items = this.items.filter((it) =>
				this.filterMapper(it, config),
			);

			if (backlink) {
				items.unshift({
					name: U.Common.sprintf(
						translate("popupSearchBacklinksFrom"),
						backlink.name,
					),
					isSection: true,
					withClear: true,
				...
				items.unshift({
					name: translate("popupSearchRecentObjects"),
					isSection: true,
				});
			}
			...
			if (filter) {
						...
						[
							{
								id: "importHtml",
								icon: "import-html",
								name: translate("popupSettingsImportHtmlTitle"),
								format: I.ImportType.Html,
							},
							{
								id: "importText",
								icon: "import-text",
								name: translate("popupSettingsImportTextTitle"),
								format: I.ImportType.Text,
							},
							{
								id: "importProtobuf",
								icon: "import-protobuf",
								name: translate(
									"popupSettingsImportProtobufTitle",
								),
								format: I.ImportType.Protobuf,
							},
							{
								id: "importMarkdown",
								icon: "import-markdown",
								name: translate(
									"popupSettingsImportMarkdownTitle",
								),
								format: I.ImportType.Markdown,
							},
						] as any[]
					).map((it) => ({ ...it, isImport: true, isSmall: true }));
				}
				...
				let settingsSpace: any[] = [
					...
					{
						id: "exportIndex",
						icon: "settings-export",
						name: translate("popupSettingsExportTitle"),
					},
					{
						id: "exportProtobuf",
						icon: "import-protobuf",
						name: translate("popupSettingsExportProtobufTitle"),
					},
					{
						id: "exportMarkdown",
						icon: "import-markdown",
						name: translate("popupSettingsExportMarkdownTitle"),
					},
				];
				...
				if (canWrite) {
					settingsSpace = settingsSpace.concat([
						{
							id: "importIndex",
							icon: "settings-import",
							name: translate("popupSettingsImportTitle"),
						},
						{
							id: "importNotion",
							icon: "import-notion",
							name: translate("popupSettingsImportNotionTitle"),
						},
						{
							id: "importCsv",
							icon: "import-csv",
							name: translate("popupSettingsImportCsvTitle"),
						},
					]);
				}

				settingsSpace = settingsSpace.map((it) => ({
					...it,
					isSpace: true,
					className: "isSpace",
				}));
				...
				const settingsAccount: any[] = [
					{
						id: "account",
						name: translate("popupSettingsProfileTitle"),
					},
					{
						id: "personal",
						icon: "settings-personal",
						name: translate("popupSettingsPersonalTitle"),
						aliases: [
							translate("commonLanguage", lang),
							translate("commonLanguage"),
							translate("commonSpelling", lang),
							translate("commonSpelling"),
						],
					},
					{
						id: "appearance",
						icon: "settings-appearance",
						name: translate("popupSettingsAppearanceTitle"),
						aliases: [
							translate("commonSidebar", lang),
							translate("commonSidebar"),
						],
					},
					{
						id: "pinIndex",
						icon: "settings-pin",
						name: translate("popupSettingsPinTitle"),
					},
					{
						id: "dataManagement",
						icon: "settings-storage",
						name: translate("popupSettingsDataManagementTitle"),
					},
					{
						id: "phrase",
						icon: "settings-phrase",
						name: translate("popupSettingsPhraseTitle"),
					},
				];
				...
				const settingsItems = settingsAccount
					.concat(settingsSpace)
					.map((it) => ({ ...it, isSettings: true, isSmall: true }));
				const filtered = itemsImport
			...
			if (canWrite) {
				...
				if (hasRelations) {
					items.push({
						id: "relation",
						name: translate("commonAddRelation"),
						icon: "relation",
			...
			close(() => {
				...
				// Settings item
					...
					S.Popup.open("settings", {
						data: { page: item.id, isSpace: item.isSpace },
				...
				// Import action
					...
					Action.import(
						item.format,
						J.Constant.fileExtension.import[item.format],
					);
					...
					switch (item.id) {
						...
						case "relation": {
							...
							window.setTimeout(
								() =>
									$(
										"#menuBlockRelationView #item-add",
									).trigger("click"),
								S.Menu.getTimeout() * 2,
							);
							break;
