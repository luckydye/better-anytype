...
const MenuOnboarding = observer(
	class MenuSelect extends React.Component<I.Menu, State> {
		node: any = null;
		confetti: any = null;
		video: any = null;
		state = {
		...
		render() {
			...
			return (
				<div ref={(node) => (this.node = node)} className="wrap">
					...
					{item.name ? (
						<Label className="name" text={item.name} />
					) : (
						""
					)}
					{item.description ? (
						<Label className="descr" text={item.description} />
					) : (
						""
					)}
					{item.video ? (
						<video
							...
							src={item.video}
							onClick={(e: any) =>
								this.onVideoClick(e, item.video)
							}
							controls={false}
							autoPlay={true}
							loop={true}
					...
					{showConfetti ? (
						<ReactCanvasConfetti
							refConfetti={(ins) => (this.confetti = ins)}
		...
		event() {
			...
			analytics.event("OnboardingTooltip", {
				step: current + 1,
				id: key,
			});
		}
		...
		onKeyDown(e: any) {
			keyboard.shortcut("arrowleft, arrowright", e, (pressed: string) =>
				this.onArrow(e, pressed == "arrowleft" ? -1 : 1),
			);
		}
						...
						data: {
							...
							onClick: (item: any) => {
								...
								analytics.event("ChangeObjectType", {
									objectType: item.id,
									count: 1,
									route: analytics.route.menuOnboarding,
								});
