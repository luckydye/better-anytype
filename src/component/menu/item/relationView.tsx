...
const MenuItemRelationView = observer(
	class MenuItemRelationView extends React.Component<Props> {
		...
		render() {
			...
			const tooltip = translate(
				isFeatured
					? "menuItemRelationViewFeaturedRemove"
					: "menuItemRelationViewFeaturedAdd",
			);
				...
				<div id={`item-${id}`} className={cn.join(" ")}>
					...
					<div id={cellId} className={ccn.join(" ")}>
						<Cell
							...
							readonly={readonly}
							onClick={(e) =>
								onCellClick(e, relationKey, object.id)
							}
