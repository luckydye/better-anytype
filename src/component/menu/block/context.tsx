...
const MenuBlockContext = observer(
	class MenuBlockContext extends React.Component<I.Menu> {
		menuContext = null;
		...
		render() {
			...
			const colorMark =
				Mark.getInRange(marks, I.MarkType.Color, range) || {};
			const bgMark =
				Mark.getInRange(marks, I.MarkType.BgColor, range) || {};
			...
			const color = (
				<div
					className={[
						"inner",
						"textColor",
						"textColor-" + (colorMark.param || "default"),
					].join(" ")}
				/>
			);
			const background = (
			...
			let markActions = [
				...
				},
				{
					...
					icon: "link",
					name: translate("commonLink"),
					caption: `${cmd} + K`,
				},
				{
					type: I.MarkType.Code,
					icon: "kbd",
					name: translate("commonCode"),
					caption: `${cmd} + L`,
				},
			];
			...
			if (block.isTextHeader()) {
				markActions = markActions.filter(
					(it) => ![I.MarkType.Bold].includes(it.type),
				);
			}
						...
						<React.Fragment>
									...
									{markActions.map(
										(action: any, i: number) => {
											...
											if (
												...
												isSet =
													inRange && inRange.param;
											} else {
												isSet = Mark.getInRange(
													marks,
													action.type,
													range,
												);
							...
							<div className="section">
								<Icon
									...
									tooltipY={I.MenuDirection.Top}
									onMouseDown={(e) =>
										this.onMark(e, I.MarkType.Color)
									}
								/>
								...
								<Icon
									...
									tooltipY={I.MenuDirection.Top}
									onMouseDown={(e) =>
										this.onMark(e, I.MarkType.BgColor)
									}
								/>
							</div>
						</React.Fragment>
					...
					menuParam.data = Object.assign(menuParam.data, {
						options: [
							...
							},
							{
								...
								icon: [
									"align",
									U.Data.alignHIcon(block.hAlign),
								].join(" "),
								arrow: true,
							},
							{
						...
						onOver: (e: any, item: any) => {
							if (
								!this.menuContext ||
								S.Menu.isAnimating(this.menuContext.props.id)
							) {
								return;
							}
					...
					menuParam.data = Object.assign(menuParam.data, {
						...
						onChange: (newType: I.MarkType, param: string) => {
							marks = Mark.toggleLink(
								{ type: newType, param, range: { from, to } },
								marks,
							);
							S.Menu.updateData(this.props.id, { marks });
							onChange(marks);
