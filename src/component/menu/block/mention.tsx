...
const MenuBlockMention = observer(
	class MenuBlockMention extends React.Component<I.Menu, State> {
		state = {
			loading: false,
		};
		...
		getSections() {
			...
			if (this.items.length) {
				sections.push({
					...
					children: this.items.concat({ isDiv: true }),
				});
			}
		...
		load(clear: boolean, callBack?: (value: any) => void) {
			...
			const { skipIds } = data;
			const filter = this.getFilter();
			const skipLayouts = U.Object.getSystemLayouts().filter(
				(it) => it != I.ObjectLayout.Date,
			);
			const filters: any[] = [
				{
		...
		resize() {
			...
			const height = Math.max(
				44,
				Math.min(
					HEIGHT_ITEM * LIMIT_HEIGHT,
					items.length * HEIGHT_ITEM + offset,
				),
			);
