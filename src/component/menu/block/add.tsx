...
const MenuBlockAdd = observer(
	class MenuBlockAdd extends React.Component<I.Menu> {
		_isMounted = false;
		emptyLength = 0;
		timeout = 0;
		cache: any = {};
		...
		render() {
			...
			const rowRenderer = (param: any) => {
				...
				if (item.isRelationAdd) {
					...
					content = (
						...
						>
							...
							<div
								...
								className={[
									"cell",
									Relation.className(item.format),
								].join(" ")}
							>
								<Cell
									rootId={rootId}
					...
					if (item.isTextColor) {
						icn.push(
							"textColor textColor-" + (item.value || "default"),
						);
					}
					if (item.isBgColor) {
						icn.push(
							"bgColor bgColor-" + (item.value || "default"),
						);
					}
		...
		componentDidMount() {
			...
			$(`#${getId()}`)
				.off("mouseleave")
				.on("mouseleave", () => window.clearTimeout(this.timeout));
		}
		...
		getRelations() {
			...
			const type = S.Record.getTypeById(
				isTemplate ? object.targetObjectType : object.type,
			);
			...
			const typeRelations = (type ? type.recommendedRelations || [] : [])
				...
				.filter(
					(it) =>
						it &&
						it.relationKey &&
						!relationKeys.includes(it.relationKey),
				);

			const ret = relations
				.concat(typeRelations)
				.filter((it) =>
					!config.debug.hiddenObject && it.isHidden
						? false
						: it.isInstalled,
				)
			...
			ret.unshift({
				id: "add",
				name: translate("menuBlockAddNewRelation"),
				isRelationAdd: true,
			});
		...
		getSections() {
			...
			let sections: any[] = [
				{
					id: "text",
					name: translate("menuBlockAddSectionsText"),
					children: U.Menu.getBlockText(),
				},
				{
					id: "list",
					name: translate("menuBlockAddSectionsList"),
					children: U.Menu.getBlockList(),
				},
				{
					id: "media",
					name: translate("menuBlockAddSectionsMedia"),
					children: U.Menu.getBlockMedia(),
				},
				{
					id: "embed",
					name: translate("menuBlockAddSectionsEmbed"),
					children: U.Menu.getBlockEmbed(),
				},
				{
					id: "other",
					name: translate("menuBlockAddSectionsOther"),
					children: U.Menu.getBlockOther(),
				},
				{
					id: "object",
					name: translate("menuBlockAddSectionsObjects"),
					children: U.Menu.getBlockObject(),
				},
			].map((s) => ({
				...s,
				children: s.children.map((c) => ({ ...c, isBig: true })),
			}));

			sections = sections.concat([
				{
					id: "relation",
					name: translate("menuBlockAddSectionsRelations"),
			...
			if (filter && filter.text) {
				...
				if (block.canHaveAlign()) {
					...
					sections.push({
						...
						color: "",
						children: U.Menu.getHAlign(restricted),
					});
				}
				if (block.canHaveColor()) {
					sections.push({
						id: "color",
						icon: "color",
						name: translate("menuBlockAddSectionsTextColor"),
						color: "",
						children: U.Menu.getTextColors(),
					});
				}
				if (block.canHaveBackground()) {
					sections.push({
						id: "bgColor",
						icon: "bgColor",
						name: translate("menuBlockAddSectionsBackgroundColor"),
						color: "",
						children: U.Menu.getBgColors(),
					});
				}
		...
		onOver(e: any, item: any) {
			...
			const { filter } = S.Common;
			const block = S.Block.getLeaf(rootId, blockId);
			const text = U.Common.stringCut(
				data.text,
				filter.from - 1,
				filter.from + filter.text.length,
			);
			...
			const cb = () => {
				if (item.isTextColor) {
					C.BlockTextListSetColor(
						...
						(message: any) => {
							...
							analytics.event("ChangeBlockColor", {
								color: item.value,
								count: 1,
							});
						},
				...
				if (item.isBgColor) {
					C.BlockListSetBackgroundColor(
						...
						(message: any) => {
							onCommand(message.blockId || blockId);
							analytics.event("ChangeBlockBackground", {
								color: item.value,
								count: 1,
							});
						},
				...
				if (item.isAlign) {
					C.BlockListSetAlign(
						...
						(message: any) => {
							onCommand(message.blockId || blockId);
							analytics.event("ChangeBlockAlign", {
								align: item.itemId,
								count: 1,
							});
						},
				...
				if (item.isBlock) {
					...
					if (item.type == I.BlockType.Text) {
						...
						if (param.content.style == I.TextStyle.Code) {
							...
							param.fields = {
								lang:
									Storage.get("codeLang") ||
									J.Constant.default.codeLang,
							};
						}

						if (
							[I.TextStyle.Code, I.TextStyle.Callout].includes(
								param.content.style,
							)
						) {
							param.bgColor = "grey";
					...
					if (item.type == I.BlockType.Dataview) {
						param.content.isCollection =
							item.itemId == "collection";
						param.content.views = [
							{
								type: I.ViewType.Grid,
								name: translate("commonAll"),
							},
						];
					}
					...
					if (item.type == I.BlockType.Table) {
						C.BlockTableCreate(
							...
							blockId,
							position,
							Number(item.rowCnt) || 3,
							Number(item.columnCnt) || 3,
							false,
							(message: any) => {
								...
								const { rows, columns } = S.Block.getTableData(
									rootId,
									message.blockId,
								);
						...
						const type =
							S.Record.getTypeById(item.objectTypeId) || {};
						...
						blockCreate(
							...
							(newBlockId: string) => {
								...
								window.setTimeout(() => {
									...
									if (
										param.type == I.BlockType.Relation &&
										!param.content.key
									) {
		...
		recalcIndex() {
			...
			const itemsWithoutSection = itemsWithSection.filter(
				(it) => !it.isSection,
			);
