...
const MenuBlockLink = observer(
	class MenuBlockLink extends React.Component<I.Menu, State> {
		state = {
			loading: false,
		};
				...
				<div className="wrap">
					<Filter
						...
						placeholder={translate(
							"menuBlockLinkFilterPlaceholder",
						)}
						value={filter}
		...
		onFilterChange(e: any) {
			...
			this.timeout = window.setTimeout(() => {
				S.Menu.updateData(this.props.id, {
		...
		getSections() {
			...
			const isLocal =
				filter.match(/^file:/) || U.Common.matchLocalPath(filter);
			const isUrl =
				U.Common.matchUrl(filter) || U.Common.matchDomain(filter);
			...
			if (isLocal || isUrl) {
				items.unshift({
					id: "link",
					name: translate("menuBlockLinkSectionsLinkToWebsite"),
					icon: "link",
					isLocal,
				});
			}
			...
			if (items.length) {
				sections.push({
					id: I.MarkType.Object,
					children: items.map((it) => ({ ...it, isBig: true })),
				});
			}
		...
		onClick(e: any, item: any) {
			...
			if (item.itemId == "link") {
				...
				U.Object.create(
					...
					"",
					[I.ObjectFlag.SelectType, I.ObjectFlag.SelectTemplate],
					"Link",
					(message: any) => {
		...
		getListHeight() {
			return this.getItems(true).reduce(
				(res: number, item: any) => res + this.getRowHeight(item),
				0,
			);
		}
