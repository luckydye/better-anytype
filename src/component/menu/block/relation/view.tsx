...
const MenuBlockRelationView = observer(
	class MenuBlockRelationView extends React.Component<I.Menu> {
		node: any = null;
		...
		render() {
			...
			let allowedBlock = S.Block.checkFlags(rootId, rootId, [
				I.RestrictionObject.Block,
			]);
			let allowedRelation = S.Block.checkFlags(rootId, rootId, [
				I.RestrictionObject.Relation,
			]);
			let allowedValue = S.Block.checkFlags(rootId, rootId, [
							...
							return (
								<Item
									...
									readonly={
										!(
											allowedValue &&
											!item.isReadonlyValue &&
											!readonly
										)
									}
									canEdit={
										allowedRelation &&
										!item.isReadonlyRelation
									}
									canDrag={allowedBlock}
									canFav={allowedValue}
									diffType={
										diffKeys.includes(item.relationKey)
											? I.DiffType.Change
											: I.DiffType.None
									}
			...
			return (
				<div ref={(node) => (this.node = node)} className="sections">
		...
		getSections() {
			...
			items = items
				...
				.filter((it: any) => {
					...
					if (
						(readonly || it.isReadonlyValue) &&
						Relation.isEmpty(object[it.relationKey]) &&
						it.scope === I.RelationScope.Type
					) {
						return false;
					}
			...
			const sections = [
				{
					id: "featured",
					name: translate("menuBlockRelationViewFeaturedRelations"),
					children: items.filter((it) =>
						featured.includes(it.relationKey),
					),
				},
				{
					id: "object",
					name: translate("menuBlockRelationViewInThisObject"),
					children: items.filter(
						(it) =>
							!featured.includes(it.relationKey) &&
							it.scope == I.RelationScope.Object,
					),
				},
			];
			...
			if (type) {
				sections.push({
					...
					name: U.Common.sprintf(
						translate("menuBlockRelationViewFromType"),
						type.name,
					),
					children: items.filter(
						(it) =>
		...
		onFav(e: any, relationKey: string) {
			...
			const featured = U.Common.objectCopy(
				object.featuredRelations || [],
			);
		...
		onCellChange(
			...
			if (
				undefined !== object[relationKey] &&
				!U.Common.compareJSON(object[relationKey], value)
			) {
			...
			events.forEach((it) => {
				switch (it.type) {
					...
					case "ObjectRelationsAmend": {
						keys = keys.concat(
							it.data.relations.map((it) => it.relationKey),
						);
						break;
					}
				}
