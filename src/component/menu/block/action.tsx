...
class MenuBlockAction extends React.Component<I.Menu, State> {
	...
	getSections() {
		...
		for (const id of blockIds) {
			...
			hasDataview =
				hasBookmark && (block.isDataview() ? checkFlag : false);
		...
		const changeFile = {
			id: "changeFile",
			icon: "link",
			name: translate("menuBlockActionsExistingFile"),
			arrow: true,
		};
		...
		if (filter) {
			const turnText = {
				id: "turnText",
				icon: "",
				name: translate("menuBlockActionsSectionsTextStyle"),
				children: U.Menu.getBlockText(),
			};
			const turnList = {
				id: "turnList",
				icon: "",
				name: translate("menuBlockActionsSectionsListStyle"),
			...
			const turnPage = {
				...
				children: U.Menu.getTurnPage(),
			};
			const turnDiv = {
				id: "turnDiv",
				icon: "",
				name: translate("menuBlockActionsSectionsDividerStyle"),
				children: U.Menu.getTurnDiv(),
			};
			const turnFile = {
				id: "turnFile",
				icon: "",
				name: translate("menuBlockActionsSectionsFileStyle"),
				children: U.Menu.getTurnFile(),
			};
			const action = {
				id: "action",
				icon: "",
			...
			const align = {
				...
				children: U.Menu.getHAlign(restrictedAlign),
			};
			const bgColor = {
				id: "bgColor",
				icon: "",
				name: translate("commonBackground"),
				children: U.Menu.getBgColors(),
			};
			const color = {
				id: "color",
				icon: "color",
				name: translate("commonColor"),
				arrow: true,
			...
			const turnText = {
				id: "turnStyle",
				icon: U.Data.styleIcon(I.BlockType.Text, style),
				name: translate("menuBlockActionsSectionsTextStyle"),
				arrow: true,
				caption: I.TextStyle[style]
					? translate(
							U.Common.toCamelCase(
								`blockName-${I.TextStyle[style]}`,
							),
						)
					: "",
			};
			...
			const c2: any[] = [
				hasLink
					? {
							id: "linkSettings",
							icon: `linkStyle${content.cardStyle}`,
							name: translate("commonPreview"),
							arrow: true,
						}
					: null,
				hasTurnFile
				...
				hasTurnDiv
					? {
							id: "turnStyle",
							icon: U.Data.styleIcon(I.BlockType.Div, style),
							name: translate(
								"menuBlockActionsSectionsDividerStyle",
							),
							arrow: true,
							isBlockDiv: true,
						}
		...
		switch (item.itemId) {
			...
			case "dataviewSource": {
				...
				const name = translate(
					isCollection ? "commonCollection" : "commonSet",
				);

				const addParam: any = {
					name: U.Common.sprintf(
						translate("menuBlockActionsCreateNew"),
						name,
					),
				};
				if (isCollection) {
					...
					addParam.onClick = (details: any) => {
						C.ObjectCreateSet([], details, "", S.Common.space, () =>
							onCreate(),
						);
					};
		...
		switch (item.itemId) {
			...
			case "clear": {
				C.BlockTextListClearStyle(rootId, blockIds, () => {
					analytics.event("ClearBlockStyle", {
						count: blockIds.length,
					});
				});
				break;
			...
			default: {
				...
				// Blocks
					...
					} else if (item.type == I.BlockType.File) {
