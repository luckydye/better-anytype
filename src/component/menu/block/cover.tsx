...
const MenuBlockCover = observer(
	class MenuBlockCover extends React.Component<I.Menu, State> {
		_isMounted = false;
		node: any = null;
		state = {
			filter: "",
		...
		render() {
			...
			const Item = (item: any) => (
				<div className="item" onClick={(e) => this.onSelect(e, item)}>
					...
					{item.artist ? (
						<div className="name">{item.artist}</div>
					) : (
						""
					)}
				</div>
								...
								<EmptySearch
									text={
										filter
											? U.Common.sprintf(
													translate(
														"menuBlockCoverEmptyFilter",
													),
													filter,
												)
											: translate("menuBlockCoverEmpty")
									}
								/>
							)}
						</React.Fragment>
			...
			return (
				<div ref={(node) => (this.node = node)} className="wrap">
					...
					<div
						className={["body", Tab[this.tab].toLowerCase()].join(
							" ",
						)}
					>
						{filterElement}
			...
			switch (this.tab) {
				case Tab.Gallery: {
					sections = sections.concat([
						{
							name: translate("menuBlockCoverGradients"),
							children: U.Menu.getCoverGradients(),
						},
						{
							name: translate("menuBlockCoverSolidColors"),
							children: U.Menu.getCoverColors(),
						},
					]);
					break;
				}
