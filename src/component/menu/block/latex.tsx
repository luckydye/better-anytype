...
const MenuBlockLatex = observer(
	class MenuBlockLatex extends React.Component<I.Menu> {
		_isMounted = false;
		emptyLength = 0;
		refList: any = null;
		cache: any = {};
		...
		render() {
			...
			const rowRenderer = (param: any) => {
				...
				if (item.isSection) {
					...
					const math = katex.renderToString(
						item.comment || item.symbol,
						{
							displayMode: true,
							throwOnError: false,
							output: "html",
			...
			return (
				<div className="items">
					{items.length ? (
						...
						<div className="item empty">No options available</div>
					)}
				</div>
			);
		}
