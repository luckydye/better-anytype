...
const MenuBlockLinkSettings = observer(
	class MenuBlockLinkSettings extends React.Component<I.Menu> {
		n = -1;
		timeout = 0;
		...
		getStyles() {
			return [
				{
					id: I.LinkCardStyle.Text,
					name: translate("menuBlockLinkSettingsStyleText"),
					icon: "style-text",
				},
				{
					id: I.LinkCardStyle.Card,
					name: translate("menuBlockLinkSettingsStyleCard"),
					icon: "style-card",
				},
			].map((it: any) => {
		...
		getIcons() {
			return [
				...
				{
					id: I.LinkIconSize.Small,
					name: translate("menuBlockLinkSettingsSizeSmall"),
				},
				{
					id: I.LinkIconSize.Medium,
					name: translate("menuBlockLinkSettingsSizeMedium"),
				},
			];
		}
		...
		getDescriptions() {
			return [
				...
				{
					id: I.LinkDescription.Added,
					name: translate(
						"menuBlockLinkSettingsDescriptionRelationDescription",
					),
				},
				{
					id: I.LinkDescription.Content,
					name: translate(
						"menuBlockLinkSettingsDescriptionContentPreview",
					),
				},
			];
		}
		...
		getSections() {
			...
			const style =
				styles.find((it) => it.id == content.cardStyle) || styles[0];
			...
			if (canIcon) {
				icons = this.getIcons();
				icon =
					icons.find((it) => it.id == content.iconSize) || icons[0];
			}

			if (canDescription) {
				descriptions = this.getDescriptions();
				description =
					descriptions.find((it) => it.id == content.description) ||
					descriptions[0];
			}

			const itemStyle = {
				id: "cardStyle",
				name: translate("menuBlockLinkSettingsPreviewLayout"),
				caption: style.name,
				arrow: true,
			};
			const itemIconSize = canIconSize
				? {
						id: "iconSize",
						name: translate("commonIcon"),
						caption: icon.name,
						arrow: true,
					}
				: null;
			const itemIconSwitch = canIconSwitch
				? {
						id: "iconSwitch",
						name: translate("commonIcon"),
						withSwitch: true,
						switchValue: icon.id != I.LinkIconSize.None,
					}
				: null;
			const itemCover = canCover
				? {
						id: "cover",
						name: translate("menuBlockLinkSettingsCover"),
						withSwitch: true,
						switchValue: this.hasRelationKey("cover"),
					}
				: null;
			const itemName = {
				id: "name",
				name: translate("menuBlockLinkSettingsName"),
				icon:
					"relation " + Relation.className(I.RelationType.ShortText),
			};
			const itemDescription = canDescription
				? {
						id: "description",
						name: translate("menuBlockLinkSettingsDescription"),
						icon:
							"relation " +
							Relation.className(I.RelationType.LongText),
			...
			const itemType = {
				...
				icon: "relation " + Relation.className(I.RelationType.Object),
				withSwitch: true,
				switchValue: this.hasRelationKey("type"),
			};
			...
			let sections: any[] = [
				...
				{
					name: translate("menuBlockLinkSettingsAttributes"),
					children: [itemName, itemDescription, itemType],
				},
			];
					...
					if (child.withSwitch) {
						child.onSwitch = (e: any, v: boolean) => {
							...
							if (child.itemId == "iconSwitch") {
								content.iconSize = v
									? I.LinkIconSize.Small
									: I.LinkIconSize.None;
								key = "iconSize";
							} else {
								content.relations = v
									? content.relations.concat([child.itemId])
