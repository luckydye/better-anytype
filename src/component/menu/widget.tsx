...
const MenuWidget = observer(
	class MenuWidget extends React.Component<I.Menu> {
		_isMounted = false;
		node: any = null;
		n = -1;
		target = null;
		...
		checkButton() {
			...
			button.addClass(
				this.target !== null && this.layout !== null
					? "black"
					: "blank disabled",
			);
		}
		...
		getSections() {
			...
			const { isEditing } = data;
			const hasLimit =
				![I.WidgetLayout.Link, I.WidgetLayout.Tree].includes(
					this.layout,
				) || U.Menu.isWidgetCollection(this.target?.id);
			...
			const sections: any[] = [
				{
					id: "source",
					name: translate("menuWidgetWidgetSource"),
					children: [
						{
							id: "source",
							name: sourceName,
			...
			if (isEditing) {
				sections.push({
					children: [
						...
						{
							id: "edit",
							name: translate("menuWidgetEditWidgets"),
							icon: "source",
						},
					],
				});
		...
		checkState() {
			...
			const layoutOptions = U.Menu.getWidgetLayoutOptions(id, layout).map(
				(it) => it.id,
			);
			...
			if (U.Menu.isWidgetCollection(id)) {
				...
				if (
					[I.WidgetLayout.List, I.WidgetLayout.Compact].includes(
						this.layout,
					) &&
			...
			this.layout = layoutOptions.includes(this.layout)
				? this.layout
				: layoutOptions.length
					? layoutOptions[0]
					: null;

			const limitOptions = U.Menu.getWidgetLimitOptions(this.layout).map(
				(it) => Number(it.id),
			);

			this.limit = limitOptions.includes(this.limit)
				? this.limit
				: limitOptions.length
					? limitOptions[0]
					: null;
		...
		onOver(e: React.MouseEvent, item) {
			...
			switch (item.itemId) {
					...
					menuParam.data = Object.assign(menuParam.data, {
						...
						dataChange: (context: any, items: any[]) => {
							const reg = new RegExp(
								U.Common.regexEscape(context.filter),
								"gi",
							);
							const fixed: any[] =
								U.Menu.getFixedWidgets().filter((it) =>
									it.name.match(reg),
								);

							return !items.length
								? fixed
								: fixed.concat([{ isDiv: true }]).concat(items);
						},
						onSelect: (target: any, isNew: boolean) => {
							...
							if (isEditing && this.target) {
								...
								C.BlockWidgetSetTargetId(
									widgets,
									blockId,
									this.target.id,
									() => {
										C.BlockWidgetSetLayout(
											widgets,
											blockId,
											this.layout,
											() => close(),
				...
				case "layout": {
					...
					menuParam.width = 320;
					menuParam.data = Object.assign(menuParam.data, {
						options: U.Menu.getWidgetLayoutOptions(
							this.target?.id,
							this.target?.layout,
						),
						value: this.layout,
		...
		getTargetId(): string {
			...
			if (isEditing) {
				...
				const widgetIds = S.Block.getChildrenIds(
					S.Block.widgets,
					S.Block.widgets,
				);
