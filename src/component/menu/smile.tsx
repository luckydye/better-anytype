	...
	class MenuSmile extends React.Component<I.Menu, State> {
				...
				case Tab.Smile: {
					...
					const rowRenderer = (param: any) => {
						...
						return (
							...
							>
								<div style={param.style}>{content}</div>
							</CellMeasurer>
						);
					};
					...
					const Item = (item: any) => (
						...
						>
							<div
								className="img"
								style={{
									backgroundImage: `url("${S.Common.imageUrl(item.id, 72)}")`,
								}}
							/>
							<div className="name">{item.name}</div>
						</div>
		...
		checkRecent(sections: any[]) {
			...
			if (recent && recent.length) {
				...
				sections.unshift({
					id: ID_RECENT,
					name: translate("menuSmileRecent"),
					children: recent,
				});
			}
		...
		getLibraryItems() {
			...
			if (
				row.children.length &&
				row.children.length < LIMIT_LIBRARY_ROW
			) {
				ret.push(row);
			}
		...
		getSmileItems() {
			...
			const length = sections.reduce((res: number, section: any) => {
				return section.id == ID_RECENT
					? res
					: res + section.children.length;
			}, 0);
			...
			if (length && length <= LIMIT_SEARCH) {
				sections = [
					{
						id: "search",
						name: translate("menuSmileSearch"),
						isSection: true,
						children: sections.reduce(
							(res: any[], section: any) => {
								return section.id == ID_RECENT
									? res
									: res.concat(section.children);
							},
							[],
						),
					},
			...
			for (let i = 0; i < items.length; ++i) {
				...
				if (
					...
					(next &&
						...
						row.children.length < LIMIT_SMILE_ROW)
				) {
					ret.push(row);
					row = { children: [] };
					n = 0;
		...
		getRowHeight(item: any) {
			...
			switch (this.state.tab) {
				case Tab.Smile:
					return HEIGHT_SMILE_ITEM;
				case Tab.Library:
					return HEIGHT_LIBRARY_ITEM;
			}
			...
			this.timeoutFilter = window.setTimeout(
				() => {
					this.setState(
						{
							page: 0,
							filter: U.Common.regexEscape(
								this.refFilter.getValue(),
							),
						},
						() => this.load(),
					);
		...
		onKeyDown(e: any) {
			...
			const { tab } = this.state;
			const checkFilter = () =>
				this.refFilter && this.refFilter.isFocused;
		...
		getTabs() {
			...
			if (!noGallery) {
				tabs.push({
					id: Tab.Smile,
					text: translate("menuSmileGallery"),
				});
			}
