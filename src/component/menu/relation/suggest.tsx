...
const MenuRelationSuggest = observer(
	class MenuRelationSuggest extends React.Component<I.Menu, State> {
		state = {
			loading: false,
		};
					...
					{!noFilter ? (
						<Filter
							...
							placeholderFocus={translate(
								"menuRelationSuggestFilterOrCreateRelation",
							)}
							value={filter}
			...
			const filters: any[] = [
				{
					...
					value: [S.Common.space, J.Constant.storeSpaceId],
				},
				{
					operator: I.FilterOperator.And,
					relationKey: "layout",
		...
		getSections() {
			...
			const library = items.filter(
				(it) => it.isInstalled && !systemKeys.includes(it.relationKey),
			);
			...
			let sections: any[] = [
				{
					id: "library",
					name: translate("menuRelationSuggestMyRelations"),
					children: library,
				},
				{
					id: "system",
					name: translate("menuRelationSuggestSystem"),
					children: system,
				},
			];
				...
				if (filter) {
					const store = items.filter(
						(it) =>
							...
							!systemKeys.includes(it.relationKey),
					);
							...
							children: [
								{
									...
									name: U.Common.sprintf(
										translate(
											"menuRelationSuggestCreateRelation",
										),
										filter,
									),
								},
		...
		onMouseEnter(e: any, item: any) {
			...
			if (
				!keyboard.isMouseDisabled &&
				!S.Menu.isAnimating(this.props.id)
			) {
		...
		onClick(e: any, item: any) {
			...
			const { rootId, blockId, menuIdEdit, addCommand, ref, noInstall } =
				data;
			...
			if (item.id == "add") {
				...
				if (item.isInstalled || noInstall) {
					...
					if (!noInstall) {
						analytics.event("AddExistingRelation", {
							format: item.format,
							type: ref,
		...
		getLibrarySources() {
			return this.items
				.filter((it) => it.spaceId == S.Common.space)
				.map((it) => it.sourceObject)
				.filter((it) => it);
		}
		...
		resize() {
			...
			const height = Math.min(
				376,
				items.reduce(
					(res: number, current: any) => {
						return res + this.getRowHeight(current);
					},
					16 + (!noFilter ? 42 : 0),
				),
			);
