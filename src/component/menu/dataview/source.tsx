...
const MenuSource = observer(
	class MenuSource extends React.Component<I.Menu> {
		n = -1;
					...
					<div className="scrollWrap">
						...
						{!items.length ? (
							<div className="item empty">
								<div className="inner">
									{translate(
										"menuDataviewSourceSelectOneOrMoreSources",
									)}
								</div>
							</div>
						) : (
					...
					{!types.length ? (
						<div className="bottom">
							...
							>
								<Icon className="plus" />
								<div className="name">
									{translate("menuDataviewSourceAddRelation")}
								</div>
							</div>
						</div>
					) : (
			...
			S.Menu.open("searchObject", {
				...
				data: {
					...
					onSelect: (item: any) => {
						...
						if (!value.length) {
							analytics.event("SetSelectQuery", {
								type: "relation",
							});
						}
					},
