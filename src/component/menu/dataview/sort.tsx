...
const MenuSort = observer(
	class MenuSort extends React.Component<I.Menu> {
		node: any = null;
		n = -1;
		top = 0;
		cache: any = {};
		...
		render() {
			...
			const typeOptions = [
				{
					id: String(I.SortType.Asc),
					name: translate("commonAscending"),
				},
				{
					id: String(I.SortType.Desc),
					name: translate("commonDescending"),
				},
			];
					...
					>
						...
						<div className="txt">
							<Select
								...
								onChange={(v) =>
									this.onChange(item.id, "relationKey", v)
								}
								readonly={isReadonly}
							/>
					...
					<div className="items">
						{!items.length ? (
							<div className="item empty">
								<div className="inner">
									{translate(
										"menuDataviewSortNoSortsApplied",
									)}
								</div>
							</div>
						) : (
		...
		onMore(e: any, item: any) {
			...
			const options = [
				{
					name: translate("menuDataviewSortShowEmpty"),
					isSection: true,
				},
				{
					id: I.EmptyType.Start,
					name: translate("menuDataviewSortShowEmptyTop"),
				},
				{
					id: I.EmptyType.End,
					name: translate("menuDataviewSortShowEmptyBottom"),
				},
			];
		...
		onChange(id: number, k: string, v: string) {
			...
			C.BlockDataviewSortReplace(rootId, blockId, view.id, item.id, {
				...item,
			});
		...
		resize() {
			...
			const height = Math.max(
				HEIGHT + offset,
				Math.min(360, items.length * HEIGHT + offset),
			);
