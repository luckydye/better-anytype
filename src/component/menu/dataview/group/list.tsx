...
const MenuGroupList = observer(
	class MenuGroupList extends React.Component<I.Menu> {
		node: any = null;
		n = -1;
		top = 0;
		cache: any = {};
		...
		render() {
			...
			const Item = SortableElement((item: any) => {
				...
				const subId = S.Record.getSubId(
					rootId,
					[blockId, item.id].join(":"),
				);
				const cn = ["item"];
				const head = {};
		...
		onSortEnd(result: any) {
			...
			S.Record.groupsSet(
				rootId,
				blockId,
				arrayMove(this.getItems(), oldIndex, newIndex),
			);
			this.save();
		...
		resize() {
			...
			const height = Math.max(
				HEIGHT * 2,
				Math.min(360, items.length * HEIGHT + 16),
			);
