...
const MenuRelationEdit = observer(
	class MenuRelationEdit extends React.Component<I.Menu> {
		node: any = null;
		...
		render() {
			...
			if (
				isObject &&
				!isReadonly &&
				(!relation || !relation.isReadonlyValue)
			) {
				...
				opts = (
					<div className="section noLine">
						<div className="name">
							{translate(
								"menuDataviewRelationEditLimitObjectTypes",
							)}
						</div>
						<MenuItemVertical
							id="object-type"
				...
				opts = (
					<div className="section">
						<MenuItemVertical
							...
							icon="clock"
							name={translate(
								"menuDataviewRelationEditIncludeTime",
							)}
			...
			return (
				...
				>
					<div className="section">
						<div className="name">
							{translate("menuDataviewRelationEditRelationName")}
						</div>
						{!isReadonly ? (
					...
					<div
						className={[
							"section",
							!opts && !isReadonly ? "noLine" : "",
						].join(" ")}
					>
						<div className="name">
							{translate("menuDataviewRelationEditRelationType")}
						</div>
						<MenuItemVertical
							id="relation-type"
							icon={
								this.format === null
									? undefined
									: "relation " +
										Relation.className(this.format)
							}
							name={
								this.format === null
									? translate(
											"menuDataviewRelationEditSelectRelationType",
										)
									: translate("relationName" + this.format)
							}
		...
		getSections() {
			...
			if (relation) {
				canDuplicate = canDelete =
					relation &&
					S.Block.checkFlags(rootId, blockId, [
				...
				sections.push({
					children: [
						canFilter
							? {
									id: "filter",
									icon: "relation-filter",
									name: translate(
										"menuDataviewRelationEditAddFilter",
									),
								}
							: null,
						canSort
							? {
									...
									icon: "relation-sort0",
									name: translate(
										"menuDataviewRelationEditSortAscending",
									),
									type: I.SortType.Asc,
								}
							: null,
						canSort
							? {
									...
									icon: "relation-sort1",
									name: translate(
										"menuDataviewRelationEditSortDescending",
									),
						...
						{
							id: "insert-left",
							icon: "relation-insert-left",
							name: translate(
								"menuDataviewRelationEditInsertLeft",
							),
							dir: -1,
						},
						{
							id: "insert-right",
							icon: "relation-insert-right",
							name: translate(
								"menuDataviewRelationEditInsertRight",
							),
							dir: 1,
						},
						canHide
							? {
									...
									icon: "relation-hide",
									name: translate(
										"menuDataviewRelationEditHideRelation",
									),
								}
							: null,
					],
		...
		onClick(e: any, item: any) {
			...
			const idx = view.relations.findIndex(
				(it) => it && it.relationKey == relation.relationKey,
			);
			...
			switch (item.id) {
				...
				case "remove": {
					this.props.close();
					C.BlockDataviewRelationDelete(rootId, blockId, [
				...
				case "filter": {
					const conditions = Relation.filterConditionsByType(
						relation.format,
					);
					...
					C.BlockDataviewFilterAdd(
						...
						() => {
							S.Menu.open("dataviewFilterList", {
								...
								offsetY: 10,
								onOpen: () =>
									$(`#block-${blockId} .hoverArea`).addClass(
										"active",
									),
								onClose: () =>
									$(
										`#block-${blockId} .hoverArea`,
									).removeClass("active"),
								data: {
									...data,
				...
				case "sort0":
				case "sort1": {
					C.BlockDataviewSortRemove(
						rootId,
						blockId,
						view.id,
						view.sorts.map((it) => it.id),
						() => {
					...
					S.Menu.open("relationSuggest", {
						...
						data: {
							...
							addCommand: (
								...
								Dataview.relationAdd(
									rootId,
									blockId,
									relation.relationKey,
									Math.max(0, idx + item.dir),
									view,
									(message: any) => {
										S.Menu.closeAll([
											this.props.id,
			...
			this.menuOpen("dataviewObjectValues", {
				...
				data: {
					...
					placeholderFocus: translate(
						"menuDataviewRelationEditFilterObjectTypes",
					),
		...
		checkButton() {
			...
			const button = node.find("#button");
			const canSave =
				name.length && this.format !== null && !this.isReadonly();

			button
				.removeClass("black blank")
				.addClass(canSave ? "black" : "blank");
		}
		...
		isAllowed() {
			...
			let ret = relation
				? S.Block.isAllowed(relation.restrictions, [
						I.RestrictionObject.Details,
					])
				: true;
			if (ret) {
				ret = S.Block.checkFlags(rootId, blockId, [
					I.RestrictionDataview.Relation,
				]);
			}
		...
		isReadonly() {
			...
			return (
				readonly ||
				!isAllowed ||
				(relation && relation.isReadonlyRelation)
			);
		}
		...
		save() {
			...
			const item: any = {
				...
				relationFormatObjectTypes:
					this.format == I.RelationType.Object
						? this.objectTypes || []
						: [],
			};
			...
			C.ObjectCreateRelation(item, S.Common.space, (message: any) => {
				...
				Preview.toastShow({
					text: U.Common.sprintf(
						translate("menuDataviewRelationEditToastOnCreate"),
						details.name,
					),
				});
				analytics.event("CreateRelation", {
		...
		getViewRelation(): I.ViewRelation {
			...
			return relation
				? getView()?.getRelation(relation.relationKey)
				: null;
		}
	},
);
