...
const MenuRelationList = observer(
	class MenuRelationList extends React.Component<I.Menu> {
		node: any = null;
		n = -1;
		top = 0;
		cache: any = {};
			...
			const Item = SortableElement((item: any) => {
				...
				return (
					...
					>
						...
						>
							<Icon
								className={`relation ${Relation.className(item.relation.format)}`}
							/>
		...
		onSortEnd(result: any) {
			...
			const view = getView();
			const list = arrayMove(this.getItems(), oldIndex, newIndex).map(
				(it) => it && it.relationKey,
			);
		...
		getItems() {
			...
			return Dataview.viewGetRelations(rootId, blockId, view).map(
				(it: any) => ({
					...it,
					id: it.relationKey,
