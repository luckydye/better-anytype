...
const MenuObjectValues = observer(
	class MenuObjectValues extends React.Component<I.Menu> {
				...
				if (item.isDiv) {
					...
					content = (
						<EmptySearch
							style={param.style}
							text={translate(
								"menuDataviewObjectValuesEmptySearch",
							)}
						/>
					);
				} else if (item.id == "add") {
		...
		getItems() {
			...
			value = value.filter(
				(it) => it && !it._empty_ && !it.isArchived && !it.isDeleted,
			);
			...
			value = value.concat([
				{ isDiv: true },
				{
					...
					name:
						nameAdd ||
						translate("menuDataviewObjectValuesAddObject"),
				},
			]);
			return value;
		}
		...
		resize() {
			...
			const offset = 16;
			const height = items.reduce((res: number, current: any) => {
