...
const MenuDataviewObjectList = observer(
	class MenuDataviewObjectList extends React.Component<I.Menu, State> {
		state = {
			isLoading: false,
		};
		...
		render() {
			...
			const rowRenderer = (param: any) => {
				...
				if (item.isDiv) {
					...
					content = (
						<div className="sectionName" style={param.style}>
							{item.name}
						</div>
					);
				} else if (item.id == "add") {
			...
			return (
				<div
					className={["wrap", !noFilter ? "withFilter" : ""].join(
						" ",
					)}
				>
					{!noFilter ? (
		...
		load(clear: boolean, callBack?: (message: any) => void) {
			...
			if (types && types.length) {
				filters.push({
					...
					value: types.map((it) => it.uniqueKey),
				});
			}
		...
		getItems() {
			...
			const ret = U.Common.objectCopy(this.items).filter(
		...
		onClick(e: any, item: any) {
			...
			const cb = (id: string) => {
				...
				let value = U.Common.arrayUnique(
					Relation.getArrayValue(data.value).concat([id]),
				);
				if (maxCount) {
			...
			if (item.id == "add") {
				const { details, flags } = Relation.getParamForNewObject(
					filter,
					relation,
				);
		...
		resize() {
			...
			const itemsHeight = items.reduce((res: number, current: any) => {
				return res + this.getRowHeight(current);
			}, offset);
			const height = Math.max(
				HEIGHT_ITEM + offset,
				Math.min(300, itemsHeight),
			);
