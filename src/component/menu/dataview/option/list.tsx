...
const MenuOptionList = observer(
	class MenuOptionList extends React.Component<I.Menu> {
		_isMounted = false;
		refFilter: any = null;
		refList: any = null;
		cache: any = {};
		...
		render() {
			...
			if (canAdd) {
				placeholder = translate(
					"menuDataviewOptionListFilterOrCreateOptions",
				);
		...
		rebind() {
			...
			$(`#${this.props.getId()}`).on("click", () =>
				S.Menu.close("dataviewOptionEdit"),
			);
		...
		getItems(): any[] {
			...
			let items = Relation.getOptions(
				S.Record.getRecordIds(J.Constant.subId.option, ""),
			).filter((it) => it.relationKey == relation.relationKey);
			let check = [];
			...
			if (data.filter) {
				const filter = new RegExp(
					U.Common.regexEscape(data.filter),
					"gi",
				);

				check = items.filter(
					(it) => it.name.toLowerCase() == data.filter.toLowerCase(),
				);
				...
				if (canAdd && !check.length) {
					ret.unshift({
						...
						name: U.Common.sprintf(
							isSelect
								? translate("menuDataviewOptionListSetStatus")
								: translate(
										"menuDataviewOptionListCreateOption",
									),
							data.filter,
						),
					});
		...
		resize() {
			...
			const height = Math.max(
				HEIGHT + offset,
				Math.min(maxHeight || 360, items.length * HEIGHT + offset),
			);
