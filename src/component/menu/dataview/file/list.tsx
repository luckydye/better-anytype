...
const MenuDataviewFileList = observer(
	class MenuDataviewFileList extends React.Component<I.Menu, State> {
		state = {
			isLoading: false,
		};
		...
		getItems() {
			...
			return U.Common.objectCopy(this.items).filter(
				(it) =>
					it &&
					!it._empty_ &&
					!it.isArchived &&
		...
		resize() {
			...
			const itemsHeight = items.reduce(
				(res: number, current: any) => res + this.getRowHeight(current),
				offset,
			);
			const height = Math.max(
				HEIGHT_ITEM + offset,
				Math.min(360, itemsHeight),
			);
