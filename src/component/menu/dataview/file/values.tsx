...
const MenuDataviewFileValues = observer(
	class MenuDataviewFileValues extends React.Component<I.Menu> {
		_isMounted = false;
		node: any = null;
		...
		render() {
			...
			const Image = (item: any) => (
				<img
					src={S.Common.imageUrl(item.id, 208)}
					className="img"
					onLoad={() => position()}
				/>
			);
		...
		onSortEnd(result: any) {
			...
			const value = arrayMove(
				Relation.getArrayValue(data.value),
				oldIndex,
				newIndex,
			);
			...
			S.Menu.open("dataviewFileList", {
				...
				data: {
					...
					placeholderFocus: translate(
						"menuDataviewFileValuesFindAFile",
					),
					filters: [
						{
