...
const MenuViewList = observer(
	class MenuViewList extends React.Component<I.Menu> {
		_isMounted = false;
		node: any = null;
		n = -1;
		top = 0;
		...
		render() {
			...
			const allowed = S.Block.checkFlags(rootId, blockId, [
			...
			const Item = SortableElement((item: any) => (
				...
				>
					...
					<div className="buttons">
						<Icon
							className="more"
							onClick={(e) => this.onViewContext(e, item)}
						/>
					</div>
				</div>
			));
					...
					<div className="items">
						{!items.length ? (
							<div className="item empty">
								<div className="inner">
									{translate(
										"menuDataviewViewListNoViewsFound",
									)}
								</div>
							</div>
						) : (
		...
		getItems() {
			...
			const items: any[] = U.Common.objectCopy(
				S.Record.getViews(rootId, blockId),
			).map((it) => ({
				...it,
			...
			items.unshift({
				id: "label",
				name: translate("menuDataviewViewListViews"),
				isSection: true,
			});
			return items;
		}
		...
		onAdd() {
			...
			const newView = {
				...
				groupRelationKey: Relation.getGroupOption(
					rootId,
					blockId,
					view.type,
					"",
