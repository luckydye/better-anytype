...
const MenuViewSettings = observer(
	class MenuViewSettings extends React.Component<I.Menu> {
		n = -1;
		refName = null;
		isFocused = false;
		...
		getSections() {
			...
			const relations = view
				...
				.map((it) => {
					const relation =
						S.Record.getRelationByKey(it.relationKey) || {};
					return relation
						? U.Common.shorten(relation.name || "", 16)
						: "";
				})
				.filter((it) => it);
			...
			const layoutSettings = [
				{
					id: "layout",
					name: translate("menuDataviewObjectTypeEditLayout"),
					subComponent: "dataviewViewLayout",
					caption: Dataview.defaultViewName(type),
				},
				isBoard
					? {
							id: "group",
							name: translate("libDataviewGroups"),
							subComponent: "dataviewGroupList",
						}
					: null,
				{
					id: "relations",
					name: translate("libDataviewRelations"),
					subComponent: "dataviewRelationList",
					caption: relationCnt.join(", "),
				},
			];
			const tools = [
				{
					id: "filter",
					name: translate("menuDataviewViewFilter"),
					subComponent: "dataviewFilterList",
					caption: filterCnt
						? U.Common.sprintf(
								translate("menuDataviewViewApplied"),
								filterCnt,
							)
						: "",
				},
				{
					id: "sort",
					name: translate("menuDataviewViewSort"),
					subComponent: "dataviewSort",
					caption: sortCnt
						? U.Common.sprintf(
								translate("menuDataviewViewApplied"),
								sortCnt,
					...
					children: [
						{
							...
							icon: "copy",
							name: translate(
								"menuDataviewViewEditDuplicateView",
							),
						},
						views.length > 1
							? {
									...
									icon: "remove",
									name: translate(
										"menuDataviewViewEditRemoveView",
									),
								}
							: null,
					],
				...
				switch (item.id) {
					case "copy": {
						C.BlockDataviewViewCreate(
							rootId,
							blockId,
							{ ...view, name: this.getViewName(view.name) },
							sources,
							(message: any) => {
								if (onSave) {
									onSave();
