...
const MenuViewLayout = observer(
	class MenuViewLayout extends React.Component<I.Menu> {
		n = -1;
		ref = null;
		isFocused = false;
		...
		save(withName?: boolean) {
			...
			const clearGroups =
				isBoard &&
				this.param.groupRelationKey &&
				view.groupRelationKey != this.param.groupRelationKey;
			...
			if (isBoard || isCalendar) {
				const groupOptions = Relation.getGroupOptions(
					rootId,
					blockId,
					this.param.type,
				);
				if (
					!groupOptions
						.map((it) => it.id)
						.includes(this.param.groupRelationKey)
				) {
					if (isCalendar) {
						this.param.groupRelationKey = "lastModifiedDate";
					} else {
						this.param.groupRelationKey = Relation.getGroupOption(
							rootId,
							blockId,
							this.param.type,
							this.param.groupRelationKey,
			...
			Dataview.viewUpdate(rootId, blockId, view.id, this.param, () => {
				if (clearGroups) {
					...
					C.BlockDataviewGroupOrderUpdate(
						rootId,
						blockId,
						{ viewId: view.id, groups: [] },
						onSave,
					);
				} else if (onSave) {
					onSave();
		...
		getSections() {
			...
			if (isGallery) {
				const coverOption = Relation.getCoverOptions(
					rootId,
					blockId,
				).find((it) => it.id == coverRelationKey);
				const sizeOption = Relation.getSizeOptions().find(
					(it) => it.id == cardSize,
				);
				...
				settings = settings.concat([
					{
						id: "cardSize",
						name: translate("menuDataviewViewEditCardSize"),
						caption: sizeOption
							? sizeOption.name
					...
					{
						id: "coverRelationKey",
						name: translate("menuDataviewViewEditCover"),
						caption: coverOption
							? coverOption.name
							: translate("commonSelect"),
						arrow: true,
					...
					{
						id: "coverFit",
						name: translate("menuDataviewViewEditFitMedia"),
						withSwitch: true,
						switchValue: coverFit,
						onSwitch: (e: any, v: boolean) => {
							this.onSwitch(e, "coverFit", v);
						},
					},
				]);
			}
			...
			if (isBoard || isCalendar) {
				const groupOption = Relation.getGroupOption(
					rootId,
					blockId,
					type,
					groupRelationKey,
				...
				const name = isBoard
					? translate("menuDataviewViewEditGroupBy")
					: translate("menuDataviewViewEditDate");
			...
			if (isBoard) {
				settings.push({
					...
					onSwitch: (e: any, v: boolean) => {
						this.onSwitch(e, "groupBackgroundColors", v);
					},
				});
			}
			...
			if (!isGraph) {
				settings.push({
					id: "hideIcon",
					name: translate("menuDataviewViewEditShowIcon"),
					withSwitch: true,
					switchValue: !hideIcon,
					onSwitch: (e: any, v: boolean) => {
						this.onSwitch(e, "hideIcon", !v);
					},
				});
			} else {
				settings.push({
			...
			if (isInline || isBoard) {
				...
				settings.push({
					id: "pageLimit",
					name: translate("menuDataviewViewEditPageLimit"),
					caption: pageLimit || options[0].id,
					arrow: true,
				});
			}
		...
		onOver(e: any, item: any) {
			...
			switch (item.id) {
				...
				case "groupRelationKey": {
					menuId = "select";
					menuParam.data = Object.assign(menuParam.data, {
						value: Relation.getGroupOption(
							rootId,
							blockId,
							view.type,
							groupRelationKey,
						)?.id,
						options: Relation.getGroupOptions(
							rootId,
							blockId,
							view.type,
						),
		...
		onClick(e: any, item: any) {
			...
			if (item.sectionId == "type") {
				...
				if (
					this.param.name == Dataview.defaultViewName(this.param.type)
				) {
		...
		getViewName(name?: string) {
			return (
				name ||
				this.param.name ||
				Dataview.defaultViewName(this.param.type)
			).trim();
		}
