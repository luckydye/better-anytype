...
const MenuFilterList = observer(
	class MenuFilterList extends React.Component<I.Menu> {
		node: any = null;
		n = -1;
		top = 0;
		cache: any = {};
				...
				<div className="items">
					{!items.length ? (
						<div className="item empty">
							<div className="inner">
								{translate("menuDataviewFilterListEmpty")}
							</div>
						</div>
					) : (
						<InfiniteLoader
		...
		onClick(e: any, item: any) {
			...
			S.Menu.open("dataviewFilterValues", {
				...
				data: {
					...data,
					save: () => {
						C.BlockDataviewFilterReplace(
							rootId,
							blockId,
							view.id,
							item.id,
							view.getFilter(item.id),
							() => {
								loadData(view.id, 0);
							},
						);
		...
		onSortEnd(result: any) {
			const { param } = this.props;
			const { data } = param;
			const { rootId, blockId, getView, loadData, isInline, getTarget } =
				data;
			const view = getView();
			const object = getTarget();
			const { oldIndex, newIndex } = result;

			view.filters = arrayMove(
				view.filters as I.Filter[],
				oldIndex,
				newIndex,
			);
			C.BlockDataviewFilterSort(
				rootId,
				blockId,
				view.id,
				view.filters.map((it) => it.id),
				() => loadData(view.id, 0),
			);
		...
		getItems() {
			...
			return U.Common.objectCopy(view.filters || [])
				.map((it: any) => {
					return {
						...it,
