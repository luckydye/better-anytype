...
const MenuDataviewFilterValues = observer(
	class MenuDataviewFilterValues extends React.Component<I.Menu> {
		_isMounted = false;
		node: any = null;
		timeoutChange = 0;
		refInput = null;
		...
		render() {
			...
			const relation: any =
				S.Record.getRelationByKey(item.relationKey) || {};
			const relationOptions = this.getRelationOptions();
			const conditionOptions = Relation.filterConditionsByType(
				relation.format,
			);
			const checkboxOptions: I.Option[] = [
				...
				{
					id: "0",
					name: translate("menuDataviewFilterValuesUnchecked"),
				},
			];
			const relationOption: any =
				relationOptions.find((it) => it.id == item.relationKey) || {};
			const conditionOption: any =
				conditionOptions.find((it) => it.id == item.condition) || {};
			...
			switch (relation.format) {
					...
					Item = (element: any) => {
						...
						return (
							<div
								...
								onMouseEnter={() =>
									setHover({ id: `object-${element.id}` })
								}
							>
								<div
									className="clickable"
									onClick={(e) => this.onObject(e, item)}
								>
				...
				case I.RelationType.Checkbox: {
					value = (
						<Select
							...
							onChange={(v) =>
								this.onChange("value", Boolean(Number(v)), true)
							}
				...
				case I.RelationType.Date: {
					if (
						...
						[I.FilterQuickOption.ExactDate].includes(
							item.quickOption,
						)
					) {
						value = (
			...
			if (Relation.isDictionary(item.relationKey)) {
				value = (
					<Select
						...
						className="checkboxValue"
						arrowClassName="light"
						options={Relation.getDictionaryOptions(
							item.relationKey,
						)}
						value={item.value}
						onChange={(v) =>
							this.onChange("value", Number(v), true)
						}
						menuParam={selectParam}
						readonly={isReadonly}
					/>
				...
				if (this.refInput.setValue) {
					if (isDate) {
						if (item.quickOption == I.FilterQuickOption.ExactDate) {
							this.refInput.setValue(
								item.value === null
									? ""
									: U.Date.date("d.m.Y H:i:s", item.value),
							);
						} else {
		...
		getItems() {
			...
			const filterQuickOptions = Relation.filterQuickOptions(
				relation.format,
				item.condition,
			);
			const filterOption: any =
				filterQuickOptions.find((it) => it.id == item.quickOption) ||
				{};
			...
			if (
				relation.format == I.RelationType.Date &&
				filterQuickOptions.length
			) {
				ret.push({
					id: "quickOption",
					icon: "",
		...
		onOver(e: any, item: any) {
			...
			switch (item.id) {
				...
				case "quickOption": {
					options = Relation.filterQuickOptions(
						item.format,
						item.condition,
					);
					break;
			...
			this.timeoutChange = window.setTimeout(
				() => {
					...
					if (k == "relationKey") {
						...
						item.condition = conditions.length
							? conditions[0].id
							: I.FilterCondition.None;
						item.quickOption = I.FilterQuickOption.ExactDate;
						item.value = Relation.formatValue(
							relation,
							null,
							false,
						);
					...
					if (k == "condition") {
						...
						const quickOptions = Relation.filterQuickOptions(
							relation.format,
							item.condition,
						);
						const filterOption = quickOptions.find(
							(it) => it.id == item.quickOption,
						);

						if (!filterOption) {
							item.quickOption = quickOptions.length
								? quickOptions[0].id
		...
		onObject(e: any, item: any) {
			...
			S.Menu.closeAll(
				["dataviewObjectValues", "dataviewObjectList", "select"],
				() => {
		...
		isReadonly() {
			...
			const allowedView = S.Block.checkFlags(rootId, blockId, [
				I.RestrictionDataview.View,
			]);
