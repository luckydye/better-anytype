...
const MenuDataviewDate = observer(
	class MenuDataviewDate extends React.Component<I.Menu> {
		_isMounted = false;
		n = -1;
		...
		getSections() {
			...
			dateFormat =
				dateOptions.find((it) => it.id == df) || dateOptions[0];
			timeFormat =
				timeOptions.find((it) => it.id == tf) || timeOptions[0];

			let sections = [
				{
					id: "date",
					name: translate("menuDataviewDateDateFormat"),
					children: [
						{
							id: "dateFormat",
				...
				{
					id: "time",
					name: translate("menuDataviewDateTimeFormat"),
					children: [
						{
							id: "timeFormat",
			...
			switch (key) {
					...
					options = [
						{
							...
							name: U.Date.date("M d, Y", U.Date.now()),
						},
						{
							id: I.DateFormat.MonthAbbrAfterDay,
							name: U.Date.date("d M, Y", U.Date.now()),
						},
						{
						...
						{
							id: I.DateFormat.ISO,
							name: U.Date.date("Y-m-d", U.Date.now()),
						},
					];
					break;
				...
				case "timeFormat":
					options = [
						{
							id: I.TimeFormat.H12,
							name: translate("menuDataviewDate12Hour"),
						},
						{
							id: I.TimeFormat.H24,
							name: translate("menuDataviewDate24Hour"),
						},
					];
					break;
			}
		...
		onOver(e: any, item: any) {
			...
			const value =
				options.find((it) => it.id == relation[item.itemId]) ||
				options[0];
