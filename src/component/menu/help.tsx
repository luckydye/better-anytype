...
class MenuHelp extends React.Component<I.Menu> {
	...
	getItems() {
		const btn = (
			<Button className="c16" text={U.Common.getElectron().version.app} />
		);
		...
		return [
			...
			...it,
			name: translate(U.Common.toCamelCase(`menuHelp-${it.id}`)),
		}));
	}
	...
	onClick(e: any, item: any) {
		...
		close();
		analytics.event(
			U.Common.toUpperCamelCase([getId(), item.id].join("-")),
			{
				route: analytics.route.menuHelp,
			},
		);
		...
		switch (item.id) {
			...
			case "hints": {
				...
				if (isEditor && home && rootId == home.id) {
					...
					key = S.Block.checkBlockTypeExists(rootId)
						? "objectCreationStart"
						: "editor";
				} else if (isGraph) {
					key = "mainGraph";
