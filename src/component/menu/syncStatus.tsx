...
const MenuSyncStatus = observer(
	class MenuSyncStatus extends React.Component<I.Menu, {}> {
		_isMounted = false;
		node = null;
		cache: any = {};
		items: any[] = [];
		...
		onContextMenu(e, item) {
			...
			const canDelete = S.Block.isAllowed(item.restrictions, [
		...
		load() {
			...
			const sorts = [{ relationKey: "syncDate", type: I.SortType.Desc }];
		...
		getItems() {
			return U.Data.groupDateSections(
				S.Record.getRecords(SUB_ID),
				"syncDate",
			);
		}
		...
		getIconP2P(syncStatus) {
			...
			if (devicesCounter) {
				message = U.Common.sprintf(
					translate("menuSyncStatusP2PDevicesConnected"),
					devicesCounter,
					U.Common.plural(devicesCounter, translate("pluralDevice")),
				);
			} else {
		...
		getIconNetwork(syncStatus) {
			...
			if (
				[I.SyncStatusSpace.Syncing, I.SyncStatusSpace.Synced].includes(
					status,
				)
			) {
				isConnected = true;
				className = "connected";
			} else if (I.SyncStatusSpace.Error == status) {
				isError = true;
				className = "error";
			}
				...
				case I.SyncStatusNetwork.Anytype: {
					...
					if (isConnected) {
						if (syncingCounter) {
							message = U.Common.sprintf(
								translate(
									"menuSyncStatusInfoNetworkMessageSyncing",
								),
								syncingCounter,
								U.Common.plural(
									syncingCounter,
									translate("pluralLCObject"),
								),
							);
						} else {
							message = translate(
								"menuSyncStatusInfoNetworkMessageSynced",
							);
						}
					} else if (isError) {
						if (error) {
							message = translate(
								`menuSyncStatusInfoNetworkMessageError${error}`,
							);
						}
						...
						if (error == I.SyncStatusError.IncompatibleVersion) {
							buttons.push({
								id: "updateApp",
								name: translate(
									"menuSyncStatusInfoNetworkMessageErrorUpdateApp",
								),
							});
						} else if (
							error == I.SyncStatusError.StorageLimitExceed
						) {
							buttons.push({
								id: "upgradeMembership",
								name: translate(
									"menuSyncStatusInfoNetworkMessageErrorAddMoreStorage",
								),
							});
						}
					} else {
						message = translate(
							"menuSyncStatusInfoNetworkMessageOffline",
						);
					}
				...
				case I.SyncStatusNetwork.SelfHost: {
					...
					switch (status) {
						case I.SyncStatusSpace.Syncing: {
							message = translate(
								"menuSyncStatusInfoSelfMessageSyncing",
							);
							break;
						}
						case I.SyncStatusSpace.Synced: {
							message = translate(
								"menuSyncStatusInfoSelfMessageSynced",
							);
							break;
						}
						case I.SyncStatusSpace.Error: {
							message = translate(
								"menuSyncStatusInfoSelfMessageError",
							);
							break;
						}
					}
