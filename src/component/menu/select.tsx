...
import { type I, U, Relation, keyboard, translate } from "Lib";
...
const MenuSelect = observer(
	class MenuSelect extends React.Component<I.Menu> {
		_isMounted = false;
		n = -1;
		cache: any = null;
		filter = "";
		...
		render() {
			...
			const Item = (item) => {
				...
				if (item.isSection) {
					...
					content = (
						<div className={cn.join(" ")} style={item.style}>
							{item.name}
						</div>
					);
				} else if (item.isDiv) {
		...
		isWithFilter() {
			...
			const options = this.getItemsWithoutFilter().filter(
				(it) => !it.isDiv,
			);
		...
		resize() {
			...
			noVirtualisation
				? obj.addClass("noVirtualisation")
				: obj.removeClass("noVirtualisation");
