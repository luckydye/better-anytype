...
const MenuSearchObject = observer(
	class MenuSearchObject extends React.Component<I.Menu, State> {
		state = {
			isLoading: false,
		};
		...
		render() {
			...
			const placeholderFocus =
				data.placeholderFocus || translate("commonFilterObjects");
			...
			const rowRenderer = (param: any) => {
				...
				const checkbox =
					value && value.length && value.includes(item.id);
				const cn = [];
				...
				if (addParam) {
					if (addParam.nameWithFilter && filter) {
						name = U.Common.sprintf(
							addParam.nameWithFilter,
							filter,
						);
					} else if (addParam.name) {
						name = addParam.name;
					}
				}
		...
		load(clear: boolean, callBack?: (message: any) => void) {
			...
			if (
				[I.NavigationType.Move, I.NavigationType.LinkTo].includes(type)
			) {
				filters.push({
					operator: I.FilterOperator.And,
			...
			if (
				[
					I.NavigationType.Move,
					I.NavigationType.LinkTo,
					I.NavigationType.Link,
				].includes(type)
			) {
				filters.push({
					operator: I.FilterOperator.And,
			...
			U.Data.search(
				{
					...
					ignoreWorkspace:
						typeof ignoreWorkspace === "undefined"
							? false
							: ignoreWorkspace,
				},
				(message: any) => {
				...
				switch (type) {
					...
					case I.NavigationType.Link:
						C.BlockCreate(
							...
							blockId,
							position,
							U.Data.getLinkBlockParam(target.id, item.layout),
							(message: any) => {
								if (message.error.code) {
									return;
								}
					...
					case I.NavigationType.LinkTo:
						const isCollection = U.Object.isCollectionLayout(
							target.layout,
						);
						const cb = (message: any) => {
							...
							const action = isCollection
								? I.ToastAction.Collection
								: I.ToastAction.Link;
							const linkType = isCollection
								? "Collection"
								: "Object";
