...
class MenuQuickCapture extends React.Component<I.Menu, State> {
	...
	render() {
			...
			<div id={`section-${section.id}`} className="section">
				...
				{section.id == "pinned" ? (
					<Items
						...
						helperContainer={() =>
							$(`#${getId()} #section-${section.id} .items`).get(
								0,
							)
						}
					/>
		...
		const Item = (item: any) => {
			...
			if (
				[SystemIds.Search, SystemIds.Add, SystemIds.Clipboard].includes(
					item.itemId,
				)
			) {
			...
			if (
				![SystemIds.Search, SystemIds.Clipboard].includes(item.itemId)
			) {
		...
		const filters: any[] = [
			{
				...
				value: [J.Constant.storeSpaceId, S.Common.space],
			},
			{
				operator: I.FilterOperator.And,
				relationKey: "layout",
			...
			},
			{
				...
				condition: I.FilterCondition.In,
				value: U.Object.getPageLayouts().concat(
					U.Object.getSetLayouts(),
				),
			},
		];
		const sorts = [
	...
	getSections() {
		...
		if (isExpanded) {
			const items = U.Common.objectCopy(this.items || []).map((it) => ({
				...it,
				object: it,
			}));
			const library = items
			...
			const pinned = items
				...
				.sort((c1: any, c2: any) =>
					U.Data.sortByPinnedTypes(c1, c2, pinnedIds),
				);
			const librarySources = library.map((it) => it.sourceObject);
			const groups = library.filter(
				(it) =>
					U.Object.isInSetLayouts(it.recommendedLayout) &&
					!pinnedIds.includes(it.id),
			);
			...
			if (this.filter) {
				sections.push({
					...
					children: items.filter(
						(it) =>
							it.spaceId == J.Constant.storeSpaceId &&
			...
			const pinned = pinnedIds
				.map((id) => S.Record.getTypeById(id))
				.filter((it) => it)
				.slice(0, LIMIT_PINNED);

			items = U.Data.getObjectTypesForNewObject().filter(
				(it) => !pinnedIds.includes(it.id),
			);
	...
	onClick(e: any, item: any) {
		...
		if (item.itemId == SystemIds.Add) {
			...
			if (item.isInstalled) {
				...
				Action.install({ ...item, id: item.itemId }, true, (message) =>
					cb(message.details),
				);
			}
		}
	...
	onContextMenu(e: any, item: any) {
		...
		if (
			[SystemIds.Add, SystemIds.Search, SystemIds.Clipboard].includes(
				item.itemId,
			)
		) {
			return;
		...
		const canDefault =
			type.isInstalled &&
			!U.Object.isInSetLayouts(item.recommendedLayout) &&
			type.id != S.Common.type;
		const canDelete =
			type.isInstalled &&
			S.Block.isAllowed(item.restrictions, [I.RestrictionObject.Delete]);
			...
			data: {
				...
				onSelect: (event: any, element: any) => {
					switch (element.id) {
						...
						case "pin": {
							isPinned
								? Storage.removePinnedType(item.itemId)
								: Storage.addPinnedType(item.itemId);
							analytics.event(
								isPinned ? "UnpinObjectType" : "PinObjectType",
								{
	...
	async getClipboardData() {
		let ret = [];
		try {
			ret = await navigator.clipboard.read();
		} catch (e) {
			/**/
		}
		return ret;
	...
	beforePosition() {
		...
		obj.css({
			width: Math.min(
				ww - J.Size.menuBorder * 2 - sw,
				Math.ceil(obj.outerWidth()),
			),
		});
	}
}
