...
class MenuObject extends React.Component<I.Menu> {
	...
	render() {
		...
		const restrictions = S.Block.getRestrictions(rootId, rootId).map(
			(it) => I.RestrictionObject[it],
		);
	...
	getSections() {
		...
		const print = {
			id: "print",
			name: translate("menuObjectPrint"),
			caption: `${cmd} + P`,
		};
		const pageExport = {
			id: "pageExport",
		...
		const canDelete = S.Block.checkFlags(rootId, rootId, [
		...
		let search = {
			id: "search",
			name: translate("menuObjectSearchOnPage"),
			caption: `${cmd} + F`,
		};
		let history = {
			id: "history",
			name: translate("commonVersionHistory"),
			caption: U.Common.isPlatformMac() ? `${cmd} + Y` : `Ctrl + H`,
		};
		let pageCopy = {
			id: "pageCopy",
			icon: "copy",
		...
		let pageReload = {
			id: "pageReload",
			icon: "reload",
			name: translate("menuObjectReloadFromSource"),
		};
		let createWidget = {
			id: "createWidget",
			icon: "createWidget",
		...
		if (isTemplate) {
			...
			setDefaultTemplate = {
				id: "setDefault",
				icon: "pin",
				name: translate("menuObjectSetDefaultTemplate"),
			};
			...
			template = {
				id: "templateCreate",
				icon: "template",
				name: translate("menuObjectUseAsTemplate"),
			};
		}
		...
		if (object.isArchived) {
			...
			remove = {
				id: "pageRemove",
				icon: "remove",
				name: translate("commonDeleteImmediately"),
			};
			archive = {
				id: "pageUnarchive",
				icon: "restore",
		...
		if (block.isLocked()) {
			pageLock = {
				id: "pageUnlock",
				icon: "pageUnlock",
				name: translate("menuObjectUnlockPage"),
				caption: `Ctrl + Shift + L`,
			};
		} else {
			pageLock = {
				id: "pageLock",
				icon: "pageLock",
				name: translate("menuObjectLockPage"),
				caption: `Ctrl + Shift + L`,
			};
		}
		...
		if (object.isInstalled) {
			...
			pageInstall = {
				id: "pageInstall",
				icon: "install",
				name: translate("menuObjectInstall"),
			};
		}
		...
		const allowedHistory =
			!U.Object.isInFileOrSystemLayouts(object.layout) &&
			!block.isObjectParticipant() &&
		...
		const allowedLock =
			canWrite &&
			S.Block.checkFlags(rootId, rootId, [I.RestrictionObject.Details]);
		const allowedLinkTo = canWrite;
		const allowedAddCollection = canWrite;
		const allowedPageLink = true;
		const allowedCopy =
			canWrite &&
			S.Block.checkFlags(rootId, rootId, [I.RestrictionObject.Duplicate]);
		const allowedReload =
			canWrite && object.source && block.isObjectBookmark();
		const allowedInstall =
			canWrite &&
			!object.isInstalled &&
			U.Object.isTypeOrRelationLayout(object.layout);
		const allowedUninstall =
			canWrite &&
			object.isInstalled &&
			U.Object.isTypeOrRelationLayout(object.layout) &&
			canDelete;
		const allowedTemplate =
			canWrite &&
			!U.Object.getLayoutsWithoutTemplates().includes(object.layout) &&
			S.Block.checkFlags(rootId, rootId, [I.RestrictionObject.Template]);
					...
					addParam: {
						...
						onClick: (details: any) => {
							C.ObjectCreate(
								...
								S.Common.space,
								(message) => {
									Action.addToCollection(message.objectId, [
										rootId,
									]);
								},
							);
	...
	onClick(e: any, item: any) {
		...
		switch (item.id) {
			...
			case "pageCopy": {
				C.ObjectListDuplicate([rootId], (message: any) => {
					if (!message.error.code && message.ids.length) {
						U.Object.openConfig(
							{ id: message.ids[0], layout: object.layout },
							{
								onClose: () =>
									$(window).trigger(
										`updatePreviewObject.${message.ids[0]}`,
									),
							},
						);
			...
			case "pageExport": {
				S.Popup.open("export", {
					data: { objectIds: [rootId], allowHtml: true, route },
				});
				break;
			}
			...
			case "pageCreate": {
				U.Object.create(
					"",
					"",
					{ type: object.targetObjectType },
					I.BlockPosition.Bottom,
					rootId,
					[],
					route,
					(message) => U.Object.openAuto(message.details),
				);
				break;
			}
			...
			case "pageLink": {
				U.Common.clipboardCopy({
					text: `${J.Constant.protocol}://${U.Object.universalRoute(object)}`,
				});
			...
			case "setDefault": {
				...
				Preview.toastShow({
					text: translate("toastSetDefaultTemplate"),
				});
			...
			case "createWidget": {
				const first = S.Block.getFirstBlock(S.Block.widgets, 1, (it) =>
					it.isWidget(),
				);
