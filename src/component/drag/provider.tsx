	...
	class DragProvider extends React.Component<Props> {
		...
		initData() {
			...
			this.objects.each((i: number, el: any) => {
				...
				if (
					data.dropType == I.DropType.Block &&
					data.type != I.BlockType.Layout
				) {
					const block = $("#block-" + data.id);
					if (block.length) {
						const top = Number.parseInt(block.css("paddingTop"));
						const bot = Number.parseInt(block.css("paddingBottom"));
		...
		onDropCommon(e: any) {
			...
			const last = S.Block.getFirstBlock(
				rootId,
				-1,
				(it) => it && it.canCreateBlock(),
			);
			...
			if (this.hoverData && this.position != I.BlockPosition.None) {
				data = this.hoverData;
			} else if (last && isFileDrop) {
				data = this.objectData.get(
					[I.DropType.Block, last.id].join("-"),
				);
			...
			if (isFileDrop) {
				...
				C.FileDrop(rootId, targetId, position, paths, () => {
					if (
						target &&
						target.isTextToggle() &&
						position == I.BlockPosition.InnerFirst
					) {
		...
		onDragStart(
			...
			container.off("scroll.drag").on(
				"scroll.drag",
				throttle(() => this.onScroll(), 20),
			);
			sidebar.off("scroll.drag").on(
				"scroll.drag",
				throttle(() => this.onScroll(), 20),
			);
			...
			switch (targetType) {
				case I.DropType.Block: {
					...
					if (this.hoverData.isTargetCol) {
						const childrenIds = S.Block.getChildrenIds(
							targetContextId,
							targetId,
						);

						targetId = childrenIds.length
					...
					if (target) {
						...
						if (
							(target.isLink() || target.isBookmark()) &&
							position == I.BlockPosition.InnerFirst
						) {
							...
							if (contextId == targetContextId) {
								console.log(
									"[DragProvider].onDrop Contexts are equal",
								);
								return;
							}
						} else {
							const parent = S.Block.getParentLeaf(
								targetContextId,
								targetId,
							);
				...
				case I.DropType.Widget: {
					...
					switch (dropType) {
						case I.DropType.Block: {
							const blocks = S.Block.getBlocks(
								contextId,
								(it) =>
									ids.includes(it.id) && it.canBecomeWidget(),
							);
							if (!blocks.length) {
								break;
							}
							...
							if (block.isText()) {
								const marks = block.content.marks.filter((it) =>
									[
										I.MarkType.Object,
										I.MarkType.Mention,
									].includes(it.type),
		...
		checkNodes(e: any, ex: number, ey: number) {
			...
			const isItemDrag = U.Common.getDataTransferItems(dataTransfer.items)
				.length
				? true
				: false;
			...
			if (this.hoverData) {
				...
				} else if (ex > col1 && ex <= col2) {
					if (ey <= y + height * 0.3) {
						this.setPosition(I.BlockPosition.Top);
					} else if (ey >= y + height * 0.7) {
				...
				if (this.position == I.BlockPosition.Bottom) {
					const targetBot = this.objectData.get(
						this.hoverData.cacheKey + "-bot",
					);
					if (targetBot) {
				...
				if (
					dropType == I.DropType.Record &&
					this.hoverData.dropType == I.DropType.Record &&
					!canDropMiddle
				) {
					isReversed
						? this.recalcPositionX(ex, x, width)
						: this.recalcPositionY(ey, y, height);
				}
			...
			this.frame = raf(() => {
				...
				if (
					...
					obj.addClass(
						"isOver " + this.getDirectionClass(this.position),
					);
				}
			});
		}
		...
		checkParentIds(ids: string[], id: string): boolean {
			...
			for (const dropId of ids) {
				if (
					dropId == id ||
					(parentIds.length && parentIds.includes(dropId))
				) {
					return false;
				}
			}
		...
		getDirectionClass(dir: I.BlockPosition) {
			...
			switch (dir) {
				...
				case I.BlockPosition.Bottom:
					c = "bottom";
					break;
				case I.BlockPosition.Left:
					c = "left";
					break;
				case I.BlockPosition.Right:
					c = "right";
					break;
		...
		clearStyle() {
			$(".dropTarget.isOver").removeClass(
				"isOver top bottom left right middle",
			);
		}
