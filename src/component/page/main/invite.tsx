...
import { type I, C, S, U, translate } from "Lib";
...
class PageMainInvite extends React.Component<I.PageComponent, State> {
	...
	render() {
		...
		return (
			<div ref={(ref) => (this.node = ref)} className="wrapper">
				<Frame ref={(ref) => (this.refFrame = ref)}>
					<Title
						text={
							error
								? translate("commonError")
								: translate("pageMainInviteTitle")
						}
					/>
					<Error text={error} />
		...
		if (!data.cid || !data.key) {
			...
			C.SpaceInviteView(data.cid, data.key, (message: any) => {
				...
				S.Popup.closeAll(null, () => {
					const space = U.Space.getSpaceviewBySpaceId(
						message.spaceId,
					);
					...
					if (message.error.code) {
						...
						if (space.isAccountJoining) {
							...
							!space.isAccountRemoving &&
							!space.isAccountDeleted
						) {
							S.Popup.open("confirm", {
								data: {
									title: translate(
										"popupConfirmDuplicateSpace",
									),
