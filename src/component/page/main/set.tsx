...
const PageMainSet = observer(
	class PageMainSet extends React.Component<I.PageComponent, State> {
		_isMounted = false;
		node: any = null;
		id = "";
		refHeader: any = null;
		...
		render() {
			...
			if (isLoading) {
				...
				const children = S.Block.getChildren(rootId, rootId, (it) =>
					it.isDataview(),
				);
				const cover = new M.Block({
					id: rootId + "-cover",
		...
		unbind() {
			...
			$(window).off(
				events.map((it) => `${it}.set${namespace}`).join(" "),
			);
		}
		...
		onKeyDown(e: any): void {
			...
			keyboard.shortcut(`${cmd}+f`, e, () => {
				...
				node.find("#dataviewControls .filter .icon.search").trigger(
					"click",
				);
			});
			...
			if (!keyboard.isFocused) {
				keyboard.shortcut(`${cmd}+a`, e, () => {
					...
					const records = S.Record.getRecordIds(
						S.Record.getSubId(rootId, J.Constant.blockId.dataview),
						"",
					);
