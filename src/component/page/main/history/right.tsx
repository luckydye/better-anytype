...
import { type I, C, S, U, translate, analytics, dispatcher } from "Lib";
...
const HistoryRight = observer(
	class HistoryRight extends React.Component<Props, State> {
		node = null;
		refScroll = null;
		state = {
		...
		render() {
			...
			const Section = (item: any) => {
				...
				const authors = U.Common.arrayUnique(
					item.list.map((it) => it.authorId),
				).slice(0, LIMIT_AUTHORS);
				...
				return (
					<div id={`section-${item.hash}`} className="section">
						<div
							className="head"
							onClick={(e) =>
								this.toggleSection(e, item.id, item.hash)
							}
						>
			...
			const Child = (item: any) => {
				...
				if (withChildren) {
					icon = (
						<Icon
							className="arrow"
							onClick={(e) => this.onArrow(e, item.id)}
						/>
					);
				} else {
				...
				>
					...
					{showButtons ? (
						<div className="buttons">
							...
							/>
							<Button
								text={translate("pageMainHistoryRestore")}
		...
		onScroll() {
			...
			if (
				this.lastId != lastId &&
				this.top >= height - scroll.height() - 12
			) {
		...
		loadList(lastId: string) {
			...
			C.HistoryGetVersions(
				...
				(message: any) => {
					...
					const list = U.Common.arrayUniqueObjects(
						versions.concat(message.versions || []),
						"id",
					);
		...
		loadVersion(id: string) {
			...
			C.HistoryShowVersion(rootId, id, (message: any) => {
				if (
					!U.Common.checkErrorOnOpen(rootId, message.error.code, this)
				) {
					return;
				}
