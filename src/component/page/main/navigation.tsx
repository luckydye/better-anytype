...
const PageMainNavigation = observer(
	class PageMainNavigation extends React.Component<I.PageComponent, State> {
		_isMounted = false;
		node: any = null;
		state = {
			loading: false,
				...
				return (
					<div id={"item-" + item.id} className="item selected">
						...
						<div className="buttons">
							<Button
								text={translate("popupNavigationOpen")}
								className="c36"
								onClick={(e) => this.onConfirm(e, item)}
							/>
							{isPopup ? (
								<Button
									text={translate("popupNavigationCancel")}
									className="c36"
									color="blank"
					...
					<div key="sides" className="sides">
						<div id={"panel-" + Panel.Left} className="items left">
							<div className="sideName">
								{translate("popupNavigationLinkTo")}
							</div>
							{!pagesIn.length ? (
								<ItemEmpty
									name={translate("popupNavigationEmptyTo")}
								/>
							) : (
								<InfiniteLoader
						...
						>
							{info ? (
								...
								<ItemEmpty
									name={translate(
										"pageMainNavigationItemEmptyTitle",
									)}
								/>
							)}
						</div>
						...
						<div
							id={"panel-" + Panel.Right}
							className="items right"
						>
							<div className="sideName">
								{translate("popupNavigationLinkFrom")}
							</div>
							{!pagesOut.length ? (
								<ItemEmpty
									name={translate("popupNavigationEmptyFrom")}
								/>
							) : (
								<InfiniteLoader
		...
		setActive(item?: any) {
			...
			const node = $(this.node);
			node.find(`#panel-${this.panel} #item-${item.id}`).addClass(
				"active",
			);
		}
		...
		loadPage(id: string) {
			...
			C.NavigationGetObjectInfoWithLinks(id, (message: any) => {
				...
				const pagesIn = message.object.links.inbound
					.map(this.getPage)
					.filter(this.filterMapper);
				const pagesOut = message.object.links.outbound
					.map(this.getPage)
					.filter(this.filterMapper);
