...
const PageMainType = observer(
	class PageMainType extends React.Component<I.PageComponent, State> {
		_isMounted = false;
		id = "";
		refHeader: any = null;
		refHead: any = null;
		...
		render() {
			...
			const layout: any =
				U.Menu.getLayouts().find(
					(it) => it.id == object.recommendedLayout,
				) || {};
			const showTemplates =
				!U.Object.getLayoutsWithoutTemplates().includes(
					object.recommendedLayout,
				);
			const recommendedRelations = Relation.getArrayValue(
				object.recommendedRelations,
			);

			const allowedObject =
				object.isInstalled &&
				U.Object.isInPageLayouts(object.recommendedLayout);
			const allowedDetails =
				object.isInstalled &&
				S.Block.checkFlags(rootId, rootId, [
					I.RestrictionObject.Details,
				]);
			const allowedRelation =
				object.isInstalled &&
				S.Block.checkFlags(rootId, rootId, [
					I.RestrictionObject.Relation,
				]);
			const allowedTemplate =
				...
				canWrite;
			const allowedLayout =
				object.recommendedLayout != I.ObjectLayout.Bookmark;
			...
			const relations = recommendedRelations
				...
				.filter((it) => {
					...
					if (
						Relation.systemKeysWithoutUser().includes(
							it.relationKey,
						)
					) {
						return false;
			...
			const isFileType = U.Object.isInFileLayouts(
				object.recommendedLayout,
			);
			const columns: any[] = [
				{
			...
			const ItemRelation = (item: any) => (
				...
				>
					<div
						className="clickable"
						onClick={(e) => this.onRelationEdit(e, item.id)}
					>
						<Icon
							className={[
								"relation",
			...
			return (
				<div>
					...
					<div
						className={["blocks", "wrapper", check.className].join(
							" ",
						)}
					>
						<Controls
						...
						{allowedLayout ? (
							<div className="section layout">
								<div className="title">
									{translate("pageMainTypeRecommendedLayout")}
								</div>
								<div className="content">
									{allowedDetails ? (
										<Select
		...
		templateOpen(object: any) {
			U.Object.openConfig(object, {
				onClose: () =>
					$(window).trigger(`updatePreviewObject.${object.id}`),
			});
		}
		...
		onCreate() {
			...
			const allowedObject =
				U.Object.isInPageLayouts(type.recommendedLayout) ||
				U.Object.isInSetLayouts(type.recommendedLayout);
			const options = [];

			if (allowedObject) {
				options.push({
					id: "object",
					name: translate("commonNewObject"),
				});
			}

			options.push({
				id: "set",
				name: translate("pageMainTypeNewSetOfObjects"),
			});
		...
		onRelationAdd(e: any) {
			...
			const recommendedKeys = object.recommendedRelations
				.map((id) => S.Record.getRelationById(id))
				.map((it) => it && it.relationKey);
			const systemKeys = Relation.systemKeys().filter(
				(it) => !skipSystemKeys.includes(it),
			);
		...
		onRelationEdit(e: any, id: string) {
			...
			S.Menu.open("blockRelationEdit", {
				...
				data: {
					...
					deleteCommand: () => {
						C.ObjectTypeRelationRemove(rootId, [
							relation.relationKey,
						]);
					},
				},
				...
				[
					{
						...
						value: Number(layout) || I.ObjectLayout.Page,
					},
				],
			);
			analytics.event("ChangeRecommendedLayout", {
				objectType: rootId,
				layout: layout,
			});
		}
