...
const PageMainMedia = observer(
	class PageMainMedia extends React.Component<I.PageComponent, State> {
		_isMounted = false;
		node = null;
		refHeader = null;
		refHead = null;
		...
		render() {
			...
			if (isVideo || isImage || isAudio) {
				if (
					isVideo ||
					isAudio ||
					object.widthInPixels > object.heightInPixels
				) {
					cn.push("horizontal");
				} else {
					cn.push("vertical");
					...
					<div id="blocks" className={cn.join(" ")}>
						{file ? (
							...
							<div id="empty" className="empty">
								{translate("pageMainMediaNotFound")}
							</div>
						)}
					</div>
