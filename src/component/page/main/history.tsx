...
const PageMainHistory = observer(
	class PageMainHistory extends React.Component<I.PageComponent, State> {
		node = null;
		refHeader = null;
		refSideLeft = null;
		refSideRight = null;
		...
		unbind() {
			...
			$(window).off(
				events.map((it) => `${it}.history${namespace}`).join(" "),
			);
		}
		...
		renderDiff(previousId: string, diff: any[]) {
			...
			elements = elements
				.map((it) => ({ ...it, element: $(it.element) }))
				.filter((it) => it.element.length);
		...
		getElements(previousId: string, event: any) {
			...
			switch (type) {
				case "BlockAdd": {
					data.blocks.forEach((it) => {
						elements = elements.concat([
							{
								type: I.DiffType.None,
								element: `#block-${it.id}`,
							},
							{
								type: I.DiffType.Add,
								element: `#block-${it.id} > .wrapContent`,
							},
						]);
					});
					break;
				...
				case "BlockSetChildrenIds": {
					...
					const oldChildrenIds = S.Block.getChildrenIds(
						oldContextId,
						data.id,
					);
					...
					const removed = oldChildrenIds.filter(
						(item) => !newChildrenIds.includes(item),
					);
					if (removed.length) {
						removed.forEach((it) => {
				...
				case "BlockSetText": {
					...
					if (data.text !== null) {
						...
						if (added) {
							const marks = U.Common.objectCopy(
								block.content.marks || [],
							);
					...
					if (type == I.DiffType.Change) {
						elements = elements.concat(
							this.getBlockChangeElements(data.id),
						);
					} else {
				...
				case "ObjectDetailsAmend": {
					...
					if (undefined !== data.details.name) {
						elements = elements.concat([
							{
								type: I.DiffType.Change,
								element: `#block-${J.Constant.blockId.title}`,
							},
							{
								type: I.DiffType.Change,
								element: `.headSimple #editor-${J.Constant.blockId.title}`,
							},
						]);
					}
					...
					if (undefined !== data.details.description) {
						elements.push({
							type: I.DiffType.Change,
							element: `#block-${J.Constant.blockId.description}`,
						});
					}

					if (
						undefined !== data.details.iconEmoji ||
						undefined !== data.details.iconImage
					) {
						elements.push({
							type: I.DiffType.Change,
					...
					if (undefined !== data.details.featuredRelations) {
						elements.push({
							type: I.DiffType.Change,
							element: `#block-${J.Constant.blockId.featured}`,
						});
					}
					...
					if (type == "ObjectDetailsAmend") {
						for (const k in data.details) {
							const blocks = S.Block.getBlocks(
								rootId,
								(it) => it.isRelation() && it.content.key == k,
							);

							blocks.forEach((it) => {
								elements = elements.concat(
									this.getBlockChangeElements(it.id),
								);
							});
						}
					}
		...
		resize() {
			...
			sideLeft.css(cssl);
			editorWrapper.css({
				width: !this.isSetOrCollection() ? this.getWrapperWidth() : "",
			});
		}
