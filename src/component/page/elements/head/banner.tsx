	...
	render() {
		...
		switch (type) {
			...
			case I.BannerType.IsTemplate: {
				const targetObjectType = S.Record.getTypeById(
				...
				if (targetObjectType) {
					target = (
						<div
							className="typeName"
							onClick={() => U.Object.openAuto(targetObjectType)}
						>
							{translate("commonOf")}
			...
			case I.BannerType.TemplateSelect: {
				...
				if (count) {
					label = U.Common.sprintf(
						translate("selectTemplateBannerWithNumber"),
						count,
						U.Common.plural(count, translate("pluralTemplate")),
					);
				} else {
