...
const ControlButtons = observer(
	class ControlButtons extends React.Component<Props> {
		...
		render(): any {
			...
			const allowedDetails = S.Block.checkFlags(rootId, rootId, [
			...
			let allowedIcon =
				...
				!root.isObjectTask() &&
				!root.isObjectNote() &&
				!root.isObjectBookmark();
			let allowedCover =
				!checkType && allowedDetails && !root.isObjectNote();
			...
			return (
				...
				>
					{allowedIcon ? (
						<div
							id="button-icon"
							className="btn white withIcon"
							onClick={this.onIcon}
						>
							<Icon className="icon" />
							<div className="txt">
								{translate("editorControlIcon")}
							</div>
						</div>
					) : (
						""
					...
					{allowedCover ? (
						<div
							id="button-cover"
							className="btn white withIcon"
							onClick={this.onCover}
						>
							<Icon className="addCover" />
							<div className="txt">
								{translate("editorControlCover")}
							</div>
						</div>
					) : (
						""
		...
		onCover(e: any) {
			...
			const options: any[] = [
				{
					id: "change",
					icon: "coverChange",
					name: translate("pageHeadControlButtonsChangeCover"),
				},
			];
			if (U.Data.coverIsImage(coverType)) {
				options.push({
					id: "position",
					icon: "coverPosition",
					name: translate("pageHeadControlButtonsReposition"),
				});
			}
			if (
				[I.CoverType.Upload, I.CoverType.Source].includes(coverType) &&
				coverId
			) {
				options.push({
					id: "download",
					...
					onSelect: (e: any, item: any) => {
						switch (item.id) {
							...
							case "download": {
								Renderer.send(
									"download",
									S.Common.imageUrl(coverId, 1000000),
									{
										saveAs: true,
									},
								);
