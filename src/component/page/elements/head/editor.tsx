...
const PageHeadEditor = observer(
	class PageHeadEditor extends React.Component<Props> {
		node: any = null;
		refDrag: any = null;
		...
		render(): any {
			const { rootId, onKeyDown, onKeyUp, onMenuAdd, onPaste, readonly } =
				this.props;
			...
			const cover = new M.Block({
				...
				hAlign: object.layoutAlign,
				childrenIds: [],
				fields: {},
				content: {},
			});
			const icon: any = new M.Block({
				id: rootId + "-icon",
				type: I.BlockType.IconPage,
				hAlign: object.layoutAlign,
				childrenIds: [],
				fields: {},
				content: {},
			...
			return (
				<div ref={(node) => (this.node = node)}>
					...
					<Block
						...
						onPaste={onPaste}
						onMouseEnter={() => {
							$(`#editor-controls-${rootId}`).addClass("hover");
						}}
						onMouseLeave={() => {
							$(`#editor-controls-${rootId}`).removeClass(
								"hover",
							);
						}}
					/>
		...
		init() {
			...
			$("#editorWrapper").attr({
				class: ["editorWrapper", check.className].join(" "),
			});
