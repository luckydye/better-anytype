...
const HeadSimple = observer(
	class Controls extends React.Component<Props> {
		...
		render(): any {
			const { rootId, onCreate, isContextMenuDisabled, readonly } =
				this.props;
			...
			const blockFeatured: any = new M.Block({
			...
			const isTypeOrRelation = U.Object.isTypeOrRelationLayout(
				object.layout,
			);
			const isRelation = U.Object.isRelationLayout(object.layout);
			const canEditIcon =
				allowDetails && !U.Object.isRelationLayout(object.layout);
			...
			if (isTypeOrRelation) {
				if (object.isInstalled) {
					const text = isRelation
						? translate("pageHeadSimpleCreateSet")
						: translate("commonCreate");
					...
					button = (
						<Button
							id="button-install"
							text={translate("pageHeadSimpleInstall")}
							color={color}
							className={cn.join(" ")}
							onClick={onClick}
						/>
			...
			return (
				...
				>
					...
					{button ? <div className="side right">{button}</div> : ""}
				</div>
			);
		}
		...
		init() {
			...
			if (
				!focused &&
				!object._empty_ &&
				object.name == translate("defaultNamePage")
			) {
		...
		getRange(id: string): I.TextRange {
			return this.refEditable[id]
				? this.refEditable[id].getRange()
				: null;
		}

		getValue(id: string): string {
			return this.refEditable[id]
				? this.refEditable[id].getTextValue()
				: null;
		}
		...
		isInstalled() {
			...
			switch (object.layout) {
				...
				case I.ObjectLayout.Relation: {
					sources = S.Record.getRelations().map(
						(it) => it.sourceObject,
					);
					break;
				}
