...
const PageAuthSetup = observer(
	class PageAuthSetup extends React.Component<I.PageComponent, State> {
		node = null;
		refFrame = null;
		i = 0;
		state = {
			...
			return (
				<div ref={(node) => (this.node = node)} className="wrapper">
					...
					<Frame ref={(ref) => (this.refFrame = ref)}>
						...
						{title ? (
							<Title className={cn.join(" ")} text={title} />
						) : (
							""
						)}
						{label ? (
							<Label className={cn.join(" ")} text={label} />
						) : (
							""
						)}
						{loader}
