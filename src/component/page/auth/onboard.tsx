...
const PageAuthOnboard = observer(
	class PageAuthOnboard extends React.Component<I.PageComponent, State> {
		...
		render() {
			...
			switch (stage) {
				case Stage.Vault: {
					buttons = (
						<div className="animation">
							<Button
								ref={(ref) => (this.refNext = ref)}
								className={cnb.join(" ")}
								text={translate("authOnboardVaultButton")}
				...
				case Stage.Phrase: {
					const text = phraseVisible
						? translate("commonNext")
						: translate("authOnboardPhraseSubmit");
					...
					if (!phraseVisible) {
						more = (
							<div className="moreInfo animation">
								{translate("authOnboardMoreInfo")}
							</div>
						);
					}
					break;
					...
					buttons = (
						<div className="animation">
							<Button
								ref={(ref) => (this.refNext = ref)}
								className={cnb.join(" ")}
								text={translate("authOnboardSoulButton")}
								onClick={this.onForward}
							/>
						</div>
					);
			...
			return (
				...
				>
					{this.canMoveBack() ? (
						<Icon className="arrow back" onClick={this.onBack} />
					) : (
						""
					)}
					...
					<Frame ref={(ref) => (this.refFrame = ref)}>
						...
						<Title
							className="animation"
							text={translate(`authOnboard${Stage[stage]}Title`)}
						/>
						<Label
							id="label"
							className="animation"
							text={translate(`authOnboard${Stage[stage]}Label`)}
						/>
		...
		componentDidUpdate(_, prevState): void {
			...
			if (account && stage == Stage.Phrase) {
				Renderer.send("keytarGet", account.id).then((value) =>
		...
		onShowPhrase() {
			...
			if (phraseVisible) {
				...
				analytics.event("ClickOnboarding", {
					type: "ShowAndCopy",
					step: Stage[stage],
				});
			}
		...
		onCopy() {
			U.Common.copyToast(
				translate("commonPhrase"),
				this.refPhrase.getValue(),
			);
