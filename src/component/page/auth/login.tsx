...
const PageAuthLogin = observer(
	class PageAuthLogin extends React.Component<I.PageComponent, State> {
		node = null;
		refPhrase = null;
		refSubmit = null;
		isSelecting = false;
							...
							<div className="buttons">
								<div className="animation">
									<Button
										...
										text={translate("authLoginSubmit")}
		...
		onSubmit(e: any) {
			...
			if (length < J.Constant.count.phrase.word) {
				this.setError({
					code: 1,
					description: translate("pageAuthLoginInvalidPhrase"),
				});
				return;
			}
		...
		canSubmit(): boolean {
			return (
				this.refPhrase.getValue().split(" ").length ==
				J.Constant.count.phrase.word
			);
		}
