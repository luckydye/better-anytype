...
import { type I, J, keyboard, focus, translate, Action } from "Lib";
...
class InputWithFile extends React.Component<Props, State> {
				...
				<div id="text" className="txt">
					<form id="form" onSubmit={this.onSubmit}>
						{focused ? (
							...
							<span className="urlToggle" onClick={this.onFocus}>
								{textUrl + (withFile && isSmall ? or : "")}
							</span>
						)}
					</form>
					...
					{withFile ? (
						<span
							className="fileWrap"
							onMouseDown={this.onClickFile}
						>
							{!isSmall ? (
								<span>&nbsp;{translate("commonOr")}&nbsp;</span>
							) : (
								""
							)}
	...
	rebind() {
		...
		$(this.node)
			.off("resizeMove")
			.on("resizeMove", (e: any) => this.resize());
	}
	...
	resize() {
		...
		raf(() => {
			...
			const rect = (
				node.get(0) as HTMLInputElement
