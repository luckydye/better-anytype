...
const SelectionProvider = observer(
	class SelectionProvider extends React.Component<Props> {
		...
		rebind() {
			this.unbind();
			U.Common.getScrollContainer(keyboard.isPopup()).on(
				"scroll.selection",
				(e) => this.onScroll(e),
			);
		}
		...
		onMouseDown(e: any) {
			...
			isPopup
				? this.rect.addClass("fromPopup")
				: this.rect.removeClass("fromPopup");
			...
			win.on(`blur.selection mouseup.selection`, (e) =>
				this.onMouseUp(e),
			);
		}
		...
		onScroll(e: any) {
			...
			const y =
				keyboard.mouse.page.y +
				(!this.isPopup ? Math.abs(top - this.top) * d : 0);
			...
			scrollOnMove.onMouseMove(
				keyboard.mouse.client.x,
				keyboard.mouse.client.y,
			);
			this.hasMoved = true;
		}
				...
				if (!e.shiftKey && !e.altKey && !e.ctrlKey && !e.metaKey) {
					...
					if (e.ctrlKey || e.metaKey) {
						for (const i in I.SelectType) {
							const idsOnStart =
								this.idsOnStart.get(I.SelectType[i]) || [];
					...
					if (
						...
						e.shiftKey &&
						ids.length &&
						type == I.SelectType.Block
					) {
						const first = ids.length ? ids[0] : this.focused;
						const tree = S.Block.getTree(
							this.rootId,
							S.Block.getBlocks(this.rootId),
						);
		...
		drawRect(x: number, y: number) {
			...
			if (U.Common.getSelectionRange()) {
				...
				const x1 =
					this.x +
					(this.containerOffset ? this.containerOffset.left : 0);
				const y1 =
					this.y +
					(this.containerOffset
						? this.containerOffset.top - this.top
						: 0);
		...
		checkEachNode(
			...
			if (e.ctrlKey || e.metaKey) {
				ids = (this.idsOnStart.get(type) || []).includes(node.id)
					? ids.filter((it) => it != node.id)
					: ids.concat(node.id);
			} else if (e.altKey) {
		...
		checkNodes(e: any) {
			...
			const rect = U.Common.objectCopy(
				this.getRect(this.x, this.y, x, y),
			);
	...
	*/
		...
		getPageContainer() {
			return $(
				U.Common.getCellContainer(
					keyboard.isPopup() ? "popup" : "page",
				),
			);
		}
		...
		recalcCoords(x: number, y: number): { x: number; y: number } {
			if (this.containerOffset) {
				const top = U.Common.getScrollContainer(
					this.isPopup,
				).scrollTop();
