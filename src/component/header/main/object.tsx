...
const HeaderMainObject = observer(
	class HeaderMainObject extends React.Component<I.HeaderComponent, State> {
		state = {
			templatesCnt: 0,
		};
		...
		render() {
			...
			const canSync =
				...
				!root.isObjectParticipant();
			const cmd = keyboard.cmdSymbol();
			const allowedTemplateSelect = (object.internalFlags || []).includes(
			...
			} else if (U.Object.isTemplate(object.type)) {
				banner = I.BannerType.IsTemplate;
			} else if (allowedTemplateSelect && templatesCnt) {
			...
			if (banner == I.BannerType.None) {
				center = (
					<div
						...
						onClick={onSearch}
						onMouseOver={(e) =>
							onTooltipShow(e, translate("headerTooltipPath"))
						}
			...
			return (
				<React.Fragment>
					...
					<div className="side center">{center}</div>
