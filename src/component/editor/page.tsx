...
const EditorPage = observer(
	class EditorPage extends React.Component<Props, State> {
		...
		componentDidUpdate() {
			...
			U.Common.getScrollContainer(isPopup).scrollTop(
		...
		initNodes() {
			...
			this.blockFeatured = node.find(
				`#block-${J.Constant.blockId.featured}`,
			);
		}
		...
		open() {
			...
			C.ObjectOpen(
				this.id,
				"",
				U.Router.getRouteSpaceId(),
				(message: any) => {
					window.clearTimeout(this.timeoutLoading);
					this.setLoading(false);

					if (
						!U.Common.checkErrorOnOpen(
							rootId,
							message.error.code,
							this,
						)
					...
					this.containerScrollTop = Storage.getScroll(
						"editor",
						rootId,
						isPopup,
					);
		...
		focusFirstBlock() {
			const { rootId } = this.props;
			const block = S.Block.getFirstBlock(
				rootId,
				1,
				(it) => it.isText() && !it.getLength(),
			);
		...
		unbind() {
			...
			$(window).off(
				events.map((it) => `${it}.editor${namespace}`).join(" "),
			);
		...
		rebind() {
			...
			if (!isReadonly) {
				win.on(
					`mousemove.editor${namespace}`,
					throttle((e) => this.onMouseMove(e), THROTTLE),
				);
			}

			win.on(`keydown.editor${namespace}`, (e) =>
				this.onKeyDownEditor(e),
			);
			...
			Renderer.on("commandEditor", (e: any, cmd: string, arg: any) =>
		...
		onMouseMove(e: any) {
			...
			const { pageX, pageY } = e;
			const blocks = S.Block.getBlocks(rootId, (it) =>
				it.canCreateBlock(),
			);
			...
			if (this.blockFeatured.length) {
				offset =
					this.blockFeatured.offset().top +
					this.blockFeatured.outerHeight() -
					BUTTON_OFFSET;
			}
			...
			if (hovered) {
				rectContainer = (
					this.container.get(0) as Element
			...
			this.frameMove = raf(() => {
				...
				const buttonX =
					hoveredRect.x - (rectContainer.x - J.Size.blockMenu) + 2;
				const buttonY =
					pageY - rectContainer.y - BUTTON_OFFSET - this.winScrollTop;

				clear();
				this.buttonAdd.addClass("show").css({
					transform: `translate3d(${buttonX}px,${buttonY}px,0px)`,
				});
				hovered.addClass("showMenu");
				...
				if (pageX <= x + 20) {
					hovered.addClass(
						"isAdding " +
							(this.hoverPosition == I.BlockPosition.Top
								? "top"
								: "bottom"),
					);
				}
		...
		onKeyDownEditor(e: any) {
			...
			if (ids.length) {
							...
							data: {
								...
								onChange: (
									...
									C.BlockTextListSetMark(
										...
										() => {
											analytics.event("ChangeTextStyle", {
												type: newType,
												count: ids.length,
											});
										},
				...
				keyboard.shortcut(
					`${cmd}+shift+arrowup, ${cmd}+shift+arrowdown`,
					e,
					(pressed: string) => {
		...
		onKeyDownBlock(
			...
			if (block.isText()) {
				...
				keyboard.shortcut(`${cmd}+f`, e, () => {
					keyboard.onSearchMenu(
						text.substring(range.from, range.to),
						"editor",
					);
				});
			...
			keyboard.shortcut(`${cmd}+/, ctrl+shift+/`, e, () => {
				S.Menu.close("blockContext", () => {
					S.Menu.open("blockAction", {
						...
						data: {
							blockId: block.id,
							blockIds: selection.getForClick(
								block.id,
								true,
								true,
							),
			...
			if (range.from == range.to) {
				keyboard.shortcut(`${cmd}+k`, e, () =>
					keyboard.onSearchPopup(analytics.route.shortcut),
				);
			}
					...
					(pressed: string) => {
						if (block.isTextToggle()) {
							...
							S.Block.toggle(
								rootId,
								block.id,
								pressed.match("arrowdown") ? true : false,
							);
						}
					},
				);
		...
		onTabEditor(e: any, ids: string[], pressed: string) {
			...
			const next = nextId
				? S.Block.getLeaf(rootId, nextId)
				: S.Block.getNextBlock(rootId, first.id, -1);
			const obj = shift ? parent : next;
			const canTab =
				obj &&
				!first.isTextTitle() &&
				!first.isTextDescription() &&
				obj.canHaveChildren() &&
				first.isIndentable();
			...
			if (canTab) {
				Action.move(
					...
					obj.id,
					ids,
					shift ? I.BlockPosition.Bottom : I.BlockPosition.Inner,
					() => {
		...
		onCtrlShiftArrowEditor(e: any, pressed: string) {
			...
			const block = S.Block.getLeaf(
				rootId,
				dir > 0 ? ids[ids.length - 1] : ids[0],
			);
			if (!block) {
				return;
			}

			const next = S.Block.getNextBlock(
				rootId,
				block.id,
				dir,
				(it: any) => {
			...
			const nextParentElement = S.Block.getParentMapElement(
				rootId,
				next.id,
			);
			...
			const last =
				parentElement.childrenIds[parentElement.childrenIds.length - 1];

			let position =
				dir < 0 ? I.BlockPosition.Top : I.BlockPosition.Bottom;
			if (
				dir > 0 &&
				next.canHaveChildren() &&
				nextElement.childrenIds.length
			) {
				position =
					block.id == last
						? I.BlockPosition.Top
						: I.BlockPosition.InnerFirst;
			}
			if (
				dir < 0 &&
				nextParent.canHaveChildren() &&
				nextParentElement.childrenIds.length &&
		...
		onCtrlArrowBlock(e: any, pressed: string) {
			...
			const { rootId } = this.props;
			const dir = pressed.match(Key.up) ? -1 : 1;
			const next = S.Block.getFirstBlock(rootId, -dir, (it) =>
				it.isFocusable(),
			);
		...
		onShiftArrowEditor(e: any, pressed: string) {
			...
			const idsWithChildren =
				selection?.get(I.SelectType.Block, true) || [];
		...
		onShiftArrowBlock(
			...
			const lh = Number.parseInt(value.css("line-height"));
			const sy = sRect.y + st;
			const vy = vRect.y + st;
			...
			if (
				dir > 0 &&
				sy + sRect.height + lh >= vy + vRect.height &&
				range.to == length
			) {
				cb();
			}
					...
					S.Menu.open("blockLink", {
						...
						data: {
							...
							onChange: (newType: I.MarkType, param: string) => {
								marks = Mark.toggleLink(
									{ type: newType, param, range },
									marks,
								);
								cb();
							},
				...
				if (!isDelete && !range.to) {
					if (
						...
						ids.length
							...
							: this.blockMerge(block, -1, length);
					}
				}
		...
		onTabBlock(e: any, range: I.TextRange, isShift: boolean) {
			...
			const next = nextId
				? S.Block.getLeaf(rootId, nextId)
				: S.Block.getNextBlock(rootId, block.id, -1);
			...
			if (
				!isShift &&
				parentElement.childrenIds.length &&
				block.id == parentElement.childrenIds[0]
			) {
				canTab = false;
			}
			...
			Action.move(
				...
				obj.id,
				[block.id],
				isShift ? I.BlockPosition.Bottom : I.BlockPosition.Inner,
				() => {
					window.setTimeout(
		...
		onArrowVertical(
			...
			const cb = () => {
				if (!next) {
					...
					if (
						...
						next = S.Block.getNextBlock(
							...
							focused,
							dir,
							(it) => it.parentId != block.id && it.isFocusable(),
						);
					} else {
						next = S.Block.getNextBlock(
							rootId,
				...
				if (
					parent &&
					parent.isTextToggle() &&
					!Storage.checkToggle(rootId, parent.id)
				) {
					next = parent;
				}
			...
			if (idx >= 0) {
				...
				if (next && next.isTable()) {
					...
					if (tableData) {
						const rowContainerElement = S.Block.getMapElement(
							rootId,
							tableData.rowContainer.id,
						);
						...
						if (rowContainerElement) {
							const nextIdx =
								dir > 0
									? 0
									: rowContainerElement.childrenIds.length -
										1;
							const rowId =
								rowContainerElement.childrenIds[nextIdx];
			...
			if (isInsideTable) {
				...
				const rowElement = S.Block.getParentMapElement(
					rootId,
					block.id,
				);
				...
				if (idx >= 0 && nextRow) {
					const nextRowElement = S.Block.getMapElement(
						rootId,
						nextRow.id,
					);
					C.BlockTableRowListFill(rootId, [nextRow.id], () => {
						if (nextRowElement) {
							next = S.Block.getLeaf(
								rootId,
								nextRowElement.childrenIds[idx],
							);
						}
						cb();
					});
					...
					const nextIdx =
						dir > 0 ? rowElement.childrenIds.length - 1 : 0;

					next = S.Block.getNextBlock(
						rootId,
						rowElement.childrenIds[nextIdx],
						dir,
						(it) => it.isFocusable(),
					);
					cb();
		...
		onArrowHorizontal(
			...
			if (isInsideTable) {
				...
				const cb = () => {
					...
					if (
						idx + dir >= 0 &&
						idx + dir <= rowElement.childrenIds.length - 1
					) {
					...
					if (!nextCellId) {
						const row = S.Block.getParentLeaf(rootId, block.id);
						const nextRow = S.Block.getNextTableRow(
							rootId,
							row.id,
							dir,
						);
						...
						if (nextRow) {
							...
							fill(nextRow.id, () => {
								...
								this.focusNextBlock(
									S.Block.getLeaf(rootId, nextCellId),
									dir,
								);
							});
						} else {
		...
		onSelectAll() {
			...
			const all = S.Block.getBlocks(rootId, (it) =>
				it.isSelectable(),
			).map((it) => it.id);
			...
			if (!ids.includes(title)) {
				...
				ids = ids.filter(
					(id) => ![title, description, featured].includes(id),
				);
			}
		...
		onAdd(e: any) {
			...
			this.blockCreate(
				...
				{ type: I.BlockType.Text },
				(blockId: string) => {
					$(`.placeholder.c${blockId}`).text(
						translate("placeholderFilter"),
					);
		...
		onPaste(e: any, props: any, force?: boolean, data?: any) {
			...
			const selection = S.Common.getRef("selectionProvider");
			const { focused, range } = focus.state;
			const files = U.Common.getDataTransferFiles(
			...
			C.BlockPaste(
				...
				range,
				selection?.get(I.SelectType.Block, true) || [],
				data.anytype.range.to > 0,
				{ ...data, anytype: data.anytype.blocks },
				"",
				(message: any) => {
					...
					if (message.isSameBlockCaret) {
						id = focused;
					} else if (message.blockIds && message.blockIds.length) {
					...
					} else if (message.caretPosition >= 0) {
						id = focused;
		...
		onPasteUrl(url: string, isLocal: boolean, props: any) {
			...
			const win = $(window);
			const first = S.Block.getFirstBlock(
				rootId,
				1,
				(it) =>
					it.isText() && !it.isTextTitle() && !it.isTextDescription(),
			);
			...
			const isEmpty =
				...
				focused == first.id &&
				!first.getLength() &&
				(object.internalFlags || []).includes(I.ObjectFlag.DeleteEmpty);
			const length = block.getLength();
			const position = length
				? I.BlockPosition.Bottom
				: I.BlockPosition.Replace;
			...
			const options: any[] = [
				...
				canObject
					? {
							id: "object",
							name: translate("editorPageCreateBookmarkObject"),
						}
					: null,
				canBlock
					? {
							id: "block",
							name: translate("editorPageCreateBookmark"),
						}
					: null,
			...
			if (processor !== null) {
				options.unshift({
					id: "embed",
					name: translate("editorPagePasteEmbed"),
				});
			}
								...
								if (processor !== null) {
									this.blockCreate(
										...
										position,
										{
											...
											content: { processor, text: url },
										},
										(blockId: string) => {
											$(
												`#block-${blockId} .preview`,
											).trigger("click");
										},
									);
								}
		...
		getClipboardData(e: any) {
			...
			const data: any = {
				text: U.Common.normalizeLineEndings(
					String(cb.getData("text/plain") || ""),
				),
				html: String(cb.getData("text/html") || ""),
				anytype: JSON.parse(
					String(cb.getData("application/json") || "{}"),
				),
				files: [],
			};
		...
		blockCreate(
			...
			C.BlockCreate(rootId, blockId, position, param, (message: any) => {
				...
				if (param.type == I.BlockType.Dataview) {
					event.id = param.content.isCollection
						? "Collection"
						: "Set";
				}
		...
		blockMerge(focused: I.Block, dir: number, length: number) {
			const { rootId } = this.props;
			const next = S.Block.getNextBlock(rootId, focused.id, dir, (it) =>
				it.isFocusable(),
			);
			if (!next) {
				return;
		...
		blockSplit(focused: I.Block, range: I.TextRange) {
			...
			if (
				(length && isList) ||
				(!isTitle && (range.from != length || range.to != length))
			) {
				style = content.style;
			}
		...
		blockRemove(focused?: I.Block) {
			...
			let next = S.Block.getNextBlock(rootId, blockIds[0], -1, (it) =>
				it.isFocusable(),
			);
		...
		onLastClick(e: any) {
			...
			let last = S.Block.getFirstBlock(rootId, -1, (it) =>
				it.canCreateBlock(),
			);
			let create = false;
			let length = 0;
			...
			if (create) {
				this.blockCreate("", I.BlockPosition.Bottom, {
					type: I.BlockType.Text,
				});
			} else {
		...
		resizePage() {
			...
			this.frameResize = raf(() => {
				...
				const pageContainer = U.Common.getPageContainer(
					this.props.isPopup,
				);
		...
		isReadonly() {
			...
			const { rootId } = this.props;
			const allowed = S.Block.checkFlags(rootId, rootId, [
				I.RestrictionObject.Block,
			]);
			if (!allowed) {
				return true;
			}
