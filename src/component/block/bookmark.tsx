...
const BlockBookmark = observer(
	class BlockBookmark extends React.Component<I.BlockComponent> {
		_isMounted = false;
		node: any = null;
						...
						element = (
							<React.Fragment>
								{state == I.BookmarkState.Error ? (
									<Error
										text={translate("blockBookmarkError")}
									/>
								) : (
									""
								)}
		...
		resize() {
			window.setTimeout(() => {
				...
				inner.width() <= getWrapperWidth() / 2
					? inner.addClass("isVertical")
					: inner.removeClass("isVertical");
			});
		}
	},
);
