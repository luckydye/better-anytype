...
import { type I, U, keyboard, translate } from "Lib";
...
const Selection = observer(
	class Selection extends React.Component<Props> {
		...
		render() {
			const { className, isInline, isCollection, multiSelectAction } =
				this.props;
				...
				<div id="dataviewSelection" className={cn.join(" ")}>
					<div className="sides">
						<div id="sideLeft" className="side left">
							{U.Common.sprintf(
								translate("blockDataviewSelectionSelected"),
								this.ids.length,
							)}
						</div>
						...
						<div id="sideRight" className="side right">
							{buttons.map((item: any, i: number) => (
								<div
									...
									onMouseEnter={() =>
										keyboard.setSelectionClearDisabled(true)
									}
									onMouseLeave={() =>
										keyboard.setSelectionClearDisabled(
											false,
										)
									}
								>
