...
const CellObject = observer(
	class CellObject extends React.Component<I.Cell, State> {
		_isMounted = false;
		node: any = null;
		state = {
			isEditing: false,
		...
		placeholderCheck() {
			...
			value.new || value.existing.length
				? placeholder.hide()
				: placeholder.show();
		}
		...
		onKeyUp(e: any) {
			...
			this.timeoutFilter = window.setTimeout(() => {
				S.Menu.updateData("dataviewObjectList", {
		...
		onOptionAdd(text: string) {
			...
			U.Object.create(
				...
				flags,
				"Relation",
				(message) => this.onValueAdd(message.targetId),
			);
		}
