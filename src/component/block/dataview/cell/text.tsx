...
const CellText = observer(
	class CellText extends React.Component<I.Cell, State> {
		_isMounted = false;
		state = {
			isEditing: false,
		};
			...
			if (isEditing) {
						...
						if (isName && U.Object.isNoteLayout(record.layout)) {
							...
							content = (
								<div className="empty">
									{placeholder ||
										translate(
											`placeholderCell${relation.format}`,
										)}
								</div>
							);
						}
				...
				if (isDate) {
					if (value !== null) {
						...
						const day = showRelativeDates
							? U.Date.dayString(value)
							: null;
						const date = day
							? day
							: U.Date.date(
									U.Date.dateFormat(viewRelation.dateFormat),
									value,
								);
						const time = U.Date.date(
							U.Date.timeFormat(viewRelation.timeFormat),
							value,
						);

						value = viewRelation.includeTime
							? [date, time].join(day ? ", " : " ")
							: date;
					} else {
						value = "";
		...
		componentDidMount() {
			...
			this.setValue(
				Relation.formatValue(
					relation,
					record[relation.relationKey],
					true,
				),
			);
		}
		...
		componentDidUpdate() {
			...
			const viewRelation = this.getViewRelation();
			const card =
				viewType == I.ViewType.Grid ? null : $(`#record-${record.id}`);
			...
			if (isEditing) {
				...
				if (relation.relationKey == "name") {
				...
				} else if (relation.format == I.RelationType.Date) {
					const format = [
						viewRelation.dateFormat == I.DateFormat.ShortUS
							? "m.d.Y"
							: "d.m.Y",
					];
					if (viewRelation.includeTime) {
					...
					value =
						this.value !== null
							? U.Date.date(format.join(" ").trim(), this.value)
							: "";
				} else if (relation.format == I.RelationType.Number) {
		...
		*/
		...
		onBlur(e: any) {
			...
			if (
				U.Common.compareJSON(record[relation.relationKey], this.value)
			) {
				this.setEditing(false);
				return;
			}
