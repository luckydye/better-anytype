...
const CellSelect = observer(
	class CellSelect extends React.Component<I.Cell, State> {
		_isMounted = false;
		node = null;
		state = {
			isEditing: false,
		...
		render() {
			const { relation, recordId, getRecord, elementMapper, arrayLimit } =
				this.props;
		...
		onKeyUp(e: any) {
			...
			S.Menu.updateData("dataviewOptionList", {
				filter: this.getValue().new,
			});
		...
		scrollToBottom() {
			const { id } = this.props;
			const cell = $(`#${id}`);
			const content = cell.hasClass(".cellContent")
				? cell
				: cell.find(".cellContent");

			if (content.length) {
				content.scrollTop(
					content.get(0).scrollHeight +
						Number.parseInt(content.css("paddingBottom")),
				);
			}
		}
		...
		onContextMenu(e: React.MouseEvent, item: any) {
			const { id, canEdit, menuClassName, menuClassNameWrap } =
				this.props;
		...
		getItems(): any[] {
			...
			return relation && record
				? Relation.getOptions(record[relation.relationKey]).filter(
						(it) => !it.isArchived && !it.isDeleted,
					)
				: [];
		}
