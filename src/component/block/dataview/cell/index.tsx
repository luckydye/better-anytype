...
const Cell = observer(
	class Cell extends React.Component<Props> {
		...
		checkValue(): boolean {
			...
			return Relation.checkRelationValue(
				relation,
		...
		onClick(e: any) {
			...
			if (!canEdit) {
				if (Relation.isUrl(relation.format) && value) {
					Renderer.send(
						"urlOpen",
						Relation.getUrlScheme(relation.format, value) + value,
					);
				}
				return;
			}
			...
			switch (relation.format) {
				...
				case I.RelationType.Phone: {
					...
					if (e.shiftKey && value) {
						const scheme = Relation.getUrlScheme(
							relation.format,
							value,
						);
					...
					const options = [
						{
							...
							icon: "browse",
							name: translate(
								`menuDataviewUrlActionGo${relation.format}`,
							),
						},
						{
							id: "copy",
					...
					if (relation.relationKey == "source") {
						options.push({
							id: "reload",
							icon: "reload",
							name: translate("menuDataviewUrlActionGoReload"),
						});
					}
			...
			if (menuId) {
				if (S.Common.cellId != cellId) {
					...
					if (!config.debug.ui) {
						win.off("blur.cell").on("blur.cell", () =>
							S.Menu.closeAll(J.Menu.cell),
						);
					}
				} else if (closeIfOpen) {
		...
		onMouseEnter(e: any) {
			...
			const relation = this.getRelation();
			const cell = $(
				`#${Relation.cellId(idPrefix, relation.relationKey, record.id)}`,
			);
