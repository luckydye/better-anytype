	...
	class Controls extends React.Component<Props> {
		...
		render() {
			...
			const filters = view.filters.filter((it) =>
				S.Record.getRelationByKey(it.relationKey),
			);
			...
			const tooltip = Dataview.getCreateTooltip(
				rootId,
				block.id,
				target.id,
				view.id,
			...
			if (isInline) {
				cn.push("isInline");
				head = (
					<Head ref={(ref) => (this.refHead = ref)} {...this.props} />
				);
			}
			...
			const buttons = [
				{
					id: "filter",
					text: translate("blockDataviewControlsFilters"),
					menu: "dataviewFilterList",
					on: filterCnt > 0,
				},
				{
					id: "sort",
					text: translate("blockDataviewControlsSorts"),
					menu: "dataviewSort",
					on: sortCnt > 0,
				},
				{
					id: "settings",
					text: translate("blockDataviewControlsSettings"),
					menu: "dataviewViewSettings",
				},
			];
			...
			const ButtonItem = (item: any) => {
				...
				return (
					<Icon
						...
						tooltip={item.text}
						onClick={() =>
							this.onButton(`#${elementId}`, item.menu)
						}
					/>
				);
			};
			...
			const ViewItem = SortableElement((item: any) => {
				...
				return (
					<div
						id={elementId}
						className={
							"viewItem " + (item.id == view.id ? "active" : "")
						}
						onClick={() => this.onViewSet(item)}
						onContextMenu={(e) =>
							this.onViewContext(e, `#views #${elementId}`, item)
						}
					>
				...
				<div id="views" className="views">
					...
					{allowedView ? (
						<Icon
							...
							className="plus"
							tooltip={translate("blockDataviewControlsViewAdd")}
					...
					<div className="sides">
						...
						>
							...
							<Views
								...
								helperContainer={() =>
									$(`#block-${block.id} .views`).get(0)
								}
							/>
						</div>

						<div
							id="dataviewControlsSideRight"
		...
		onViewCopy(view) {
			const { rootId, block, getSources, isInline, getTarget } =
				this.props;
			const object = getTarget();
			const sources = getSources();

			C.BlockDataviewViewCreate(
				rootId,
				block.id,
				{ ...view, name: view.name },
				sources,
		...
		onButton(element: string, component: string) {
			...
			const param: any = {
				...
				onBack: (id) => {
					S.Menu.replace(id, component, {
						...param,
						noAnimation: true,
					});
					window.setTimeout(
						() => S.Menu.update(component, { noAnimation: false }),
						50,
					);
				},
				data: {
		...
		onViewAdd(e: any) {
			...
			const { rootId, block, getSources, getTarget, isInline, getView } =
				this.props;
			const sources = getSources();
			const object = getTarget();
			const view = getView();
			const newView = {
				...
				groupRelationKey:
					...
					Relation.getGroupOption(rootId, block.id, view.type, "")
						?.id,
		...
		onViewSettings() {
			this.onButton(
				`#button-${this.props.block.id}-settings`,
				"dataviewViewSettings",
			);
		}
		...
		onSortEnd(result: any) {
			...
			C.BlockDataviewViewSetPosition(
				rootId,
				block.id,
				view.id,
				newIndex,
