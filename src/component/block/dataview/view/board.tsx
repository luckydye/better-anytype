...
import $ from "jquery";
import raf from "raf";
import { type I, C, S, U, J, Dataview, keyboard, translate } from "Lib";
import Empty from "../empty";
import Column from "./board/column";

const PADDING = 46;

const ViewBoard = observer(
	class ViewBoard extends React.Component<I.ViewComponent> {
		node: any = null;
		cache: any = {};
		frame = 0;
		newIndex = -1;
		...
		render() {
			const { rootId, block, getView, className, onViewSettings } =
				this.props;
			const view = getView();
			...
			if (!relation || !relation.isInstalled) {
				return (
					<Empty
						{...this.props}
						title={translate(
							"blockDataviewBoardRelationDeletedTitle",
						)}
						description={translate(
							"blockDataviewBoardRelationDeletedDescription",
						)}
		...
		onDragStartCommon(e: any, target: any) {
			...
			clone.attr({ id: "" }).addClass("isClone").css({
				zIndex: 10000,
				position: "fixed",
				left: -10000,
				top: -10000,
		...
		onDragEndCommon(e: any) {
			...
			$(window)
				.off("dragend.board drag.board")
				.trigger("mouseup.selection");
		...
		onDragMoveColumn(e: any, groupId: any) {
			...
			this.frame = raf(() => {
				...
				if (hoverId) {
					node.find(`#column-${hoverId}`).addClass(
						"isOver " + (isLeft ? "left" : "right"),
					);
				}
			});
		}
		...
		onDragMoveCard(e: any, record: any) {
			...
			this.frame = raf(() => {
				...
				if (hoverId) {
					node.find(`#record-${hoverId}`).addClass(
						"isOver " + (isTop ? "top" : "bottom"),
					);
				}
			});
		}
		...
		onDragEndCard(e: any, record: any) {
			...
			if (
				!current.groupId ||
				!this.newGroupId ||
				(current.index == this.newIndex &&
					current.groupId == this.newGroupId)
			) {
				return;
			}
			...
			const view = getView();
			const oldSubId = S.Record.getGroupSubId(
				rootId,
				block.id,
				current.groupId,
			);
			const newSubId = S.Record.getGroupSubId(
				rootId,
				block.id,
				this.newGroupId,
			);
			const newGroup = S.Record.getGroup(
				rootId,
				block.id,
				this.newGroupId,
			);
			...
			if (change) {
				...
				C.ObjectListSetDetails(
					[record.id],
					[{ key: view.groupRelationKey, value: newGroup.value }],
					() => {
						orders = [
							{
								viewId: view.id,
								groupId: current.groupId,
								objectIds: S.Record.getRecordIds(oldSubId, ""),
							},
							{
								viewId: view.id,
				...
				records = arrayMove(
					S.Record.getRecordIds(oldSubId, ""),
					current.index,
					this.newIndex,
				);
				orders = [
				...
				objectOrderUpdate(orders, records, () =>
					S.Record.recordsSet(oldSubId, "", records),
				);
			}
		}
		...
		resize() {
			...
			const groups = this.getGroups(false);
			const width =
				groups.length * (size.card + size.margin) - size.margin;
			...
			} else if (parent && (parent.isPage() || parent.isLayoutDiv())) {
