...
const ViewGrid = observer(
	class ViewGrid extends React.Component<I.ViewComponent> {
		node: any = null;
		ox = 0;
		...
		render() {
			...
			const { offset, total } = S.Record.getMeta(
		...
		resizeColumns(relationKey: string, width: number) {
			...
			const str = relations
				.map((it) => widths[it.relationKey] + "px")
				.concat(["auto"])
				.join(" ");
			...
			relations.forEach((it) => {
				...
				const el = node.find(
					`#${Relation.cellId("head", it.relationKey, "")}`,
				);
			...
			node.find(".row .selectionTarget").css({
		...
		getColumnWidths(relationKey: string, width: number): any {
			...
			relations.forEach((it) => {
				...
				const w =
					relationKey && it.relationKey == relationKey
						? width
						: it.width;
		...
		onResizeStart(e: any, relationKey: string) {
			...
			const node = $(this.node);
			const el = node.find(
				`#${Relation.cellId("head", relationKey, "")}`,
			);
		...
		onSortEnd(result: any) {
			...
			C.BlockDataviewViewRelationSort(
				...
				block.id,
				view.id,
				view.relations.map((it) => it.relationKey),
			);
		...
		resize() {
			const { rootId, block, isPopup, isInline, getVisibleRelations } =
				this.props;
			...
			const width = getVisibleRelations().reduce(
				(res: number, current: any) => {
					return res + current.width;
				},
				J.Size.blockMenu,
			);
			const length = S.Record.getRecordIds(
				S.Record.getSubId(rootId, block.id),
				"",
			).length;
			...
			if (isInline) {
				if (parent) {
					if (parent.isPage() || parent.isLayoutDiv()) {
						...
						const vw =
							Math.max(ww, width) + (width > ww ? PADDING : 0);
						...
						wrap.css({
							width: vw + margin - offset,
							paddingRight: margin - offset,
						});
					} else {
						const parentObj = $(`#block-${parent.id}`);
						const vw = parentObj.length
							? parentObj.width() - J.Size.blockMenu
							: 0;
