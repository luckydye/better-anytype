...
const Column = observer(
	class Column extends React.Component<Props> {
		...
		render() {
			...
			const order = (block.content.groupOrder || []).find(
				(it) => it.viewId == view.id,
			);
			const orderGroup =
				(order?.groups || []).find((it) => it.groupId == id) || {};
			...
			if (view.groupBackgroundColors) {
				...
				cnbg.push(
					"bgColor bgColor-" +
						(orderGroup.bgColor || group.bgColor || "grey"),
				);
			}
			...
			items.forEach((item: any) => {
				const object = S.Detail.get(subId, item.id, [
						...
						<div className="sides">
							...
							<div className="side right">
								<Icon
									...
									className="more"
									tooltip={translate(
										"blockDataviewBoardColumnSettings",
									)}
									onClick={this.onMore}
								/>
								{isAllowedObject ? (
		...
		load(clear: boolean) {
			...
			const el = block.content.objectOrder.find(
				(it) => it.viewId == view.id && it.groupId == id,
			);
			...
			U.Data.searchSubscribe(
				...
				() => {
					S.Record.recordsSet(
						subId,
						"",
						applyObjectOrder(id, S.Record.getRecordIds(subId, "")),
					);
		...
		getItems() {
			...
			return applyObjectOrder(
				id,
				U.Common.objectCopy(S.Record.getRecordIds(getSubId(), "")),
			).map((id) => ({ id }));
		}
