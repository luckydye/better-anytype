	...
	class Item extends React.Component<Props> {
		...
		render() {
			...
			if (length > LIMIT) {
				more = (
					<div className="item more" onClick={this.onMore}>
						+{length - LIMIT} {translate("commonMore")}{" "}
						{U.Common.plural(
							length,
			...
			const Item = (item: any) => {
				...
				if (!hideIcon) {
					icon = (
						<IconObject object={item} size={16} canEdit={canEdit} />
					);
				}
		...
		getItems() {
			...
			return items.filter(
				(it) =>
					U.Date.date("j-n-Y", it[view.groupRelationKey]) == current,
			);
		}
