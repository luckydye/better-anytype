...
const ViewGallery = observer(
	class ViewGallery extends React.Component<I.ViewComponent> {
		cache: any = {};
		...
		render() {
			...
			for (const id of records) {
				...
				for (const k in item) {
					...
					if (
						!relation ||
						![I.RelationType.Object, I.RelationType.File].includes(
							relation.format,
						)
					) {
						continue;
			...
			if (isInline) {
				...
				content = (
					<WindowScroller
						scrollElement={
							isPopup ? $("#popupPage-innerWrap").get(0) : window
						}
					>
						{({ height }) => (
							<AutoSizer
		...
		setColumnCount() {
			...
			switch (view.cardSize) {
				default:
					size = 224;
					break;
				case I.CardSize.Medium:
					size = 284;
					break;
				case I.CardSize.Large:
					size = 360;
					break;
			}
			...
			this.columnCount = Math.max(
				1,
				Math.floor((this.width - margin) / size),
			);
		}
		...
		getCoverObject(id: string): any {
			...
			return Dataview.getCoverObject(
				subId,
				record,
