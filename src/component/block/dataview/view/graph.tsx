...
const ViewGraph = observer(
	class ViewGraph extends React.Component<I.ViewComponent> {
		node: any = null;
		data: any = {
			nodes: [],
			edges: [],
		...
		unbind() {
			$(window).off(
				`keydown.graphPage updateGraphRoot.graphPage removeGraphNode.graphPage sidebarResize.graphPage`,
			);
		}
		...
		load() {
			const { getView, getSearchIds, getTarget, isCollection } =
				this.props;
			const view = getView();
			if (!view) {
				return;
			...
			const filters = []
				.concat(view.filters)
				.concat(U.Data.graphFilters())
				.map((it) => Dataview.filterMapper(view, it));
			...
			C.ObjectGraph(
				...
				(message: any) => {
					...
					this.data.nodes = message.nodes.map((it) =>
						S.Detail.mapper(it),
					);
					this.forceUpdate();
