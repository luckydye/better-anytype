...
const Head = observer(
	class Head extends React.Component<I.ViewComponent, State> {
		state = {
			isEditing: false,
		};
		_isMounted = false;
		...
		render() {
			const { block, readonly, className, isCollection, getTarget } =
				this.props;
			const { isEditing } = this.state;
			const { targetObjectId } = block.content;
			const object = getTarget();
			const cn = ["dataviewHead"];
			const placeholder = isCollection
			...
			let options: any[] = [
				{
					id: "editTitle",
					icon: "editText",
					name: translate("blockDataviewHeadMenuEdit"),
				},
				{
					id: "sourceChange",
					icon: "source",
					name: U.Common.sprintf(
						translate("blockDataviewHeadMenuChange"),
						sourceName,
					),
					arrow: true,
				},
				{
					...
					icon: "expand",
					name: U.Common.sprintf(
						translate("blockDataviewHeadMenuOpen"),
						sourceName,
					),
				},
			];
					...
					C.ObjectCreate(
						...
						(message: any) => {
							C.BlockDataviewCreateFromExistingObject(
								...
								block.id,
								message.objectId,
								(message: any) => onCreate(message, true),
							);
						},
					);
				};
					...
					menuParam.data = Object.assign(menuParam.data, {
						...
						onSelect: (item: any) => {
							C.BlockDataviewCreateFromExistingObject(
								...
								block.id,
								item.id,
								(message: any) => onCreate(message, false),
							);
							this.menuContext.close();
						},
					});
		...
		onIconUpload(objectId: string) {
			U.Object.setIcon(
				this.props.block.content.targetObjectId,
				"",
				objectId,
			);
		}
