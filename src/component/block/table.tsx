...
const BlockTable = observer(
	class BlockTable extends React.Component<I.BlockComponent> {
		_isMounted = false;
		node = null;
		refTable = null;
		offsetX = 0;
						...
						<div className="inner">
							...
							{!readonly ? (
								<React.Fragment>
									<div
										id="plus-v"
										className="plusButton vertical"
										onClick={this.onPlusV}
									>
										<Icon />
									</div>
									<div
										id="plus-h"
										className="plusButton horizontal"
										onClick={this.onPlusH}
									>
										<Icon />
									</div>
									<div
										id="plus-c"
										className="plusButton circle"
										onClick={this.onPlus}
									>
										<Icon />
									</div>
			...
			switch (type) {
				...
				default: {
					options = options.concat([
						{
							id: "row",
							name: translate("blockTableRow"),
							arrow: true,
						},
						{
							id: "column",
							name: translate("blockTableColumn"),
							arrow: true,
						},
						{ isDiv: true },
					]);
											...
											item.onSwitch(
												...
												() => {
													if (
														...
														menuSubContext.ref.updateOptions(
															this.optionsRow(
																rowId,
																true,
															),
		...
		onSelect(
			...
			switch (item.id) {
				case "columnBefore":
				case "columnAfter": {
					C.BlockTableColumnCreate(
						rootId,
						columnId,
						item.id == "columnBefore"
							? I.BlockPosition.Left
							: I.BlockPosition.Right,
					);
					break;
				}
				...
				case "rowMoveBottom": {
					...
					next = this.getNextRow(
						rowId,
						position == I.BlockPosition.Top ? -1 : 1,
					);
		...
		onOptionsClose() {
			...
			table
				.find(".isHighlightedColumn")
				.removeClass("isHighlightedColumn isFirst isLast");
		...
		onResizeStart(e: any, id: string) {
			...
			win.on(
				"mousemove.table",
				throttle((e) => this.onResizeMove(e, id), 40),
			);
		...
		onResizeEnd(e: any, id: string) {
			...
			C.BlockListSetFields(rootId, [{ blockId: id, fields: { width } }]);
		...
		getColumnWidths(): number[] {
			...
			columns.forEach((it: I.Block) => {
				ret.push(
					this.checkWidth(it.fields.width || J.Size.table.default),
				);
			});
		...
		onDragStartColumn(e: any, id: string) {
			...
			win.on(
				"drag.tableColumn",
				throttle((e) => this.onDragMoveColumn(e, id), 40),
			);
		...
		onDragStartRow(e: any, id: string) {
			...
			win.on(
				"drag.tableRow",
				throttle((e) => this.onDragMoveRow(e, id), 40),
			);
			...
			let options: any[] = [
				{
					...
					name: translate("blockTableOptionsRowHeaderRow"),
					withSwitch: true,
					switchValue: isHeader,
			...
			if (!isHeader) {
				...
				if (nextTop && !nextTop.content.isHeader && idx > 0) {
					moveTop = {
						id: "rowMoveTop",
						icon: "table-move-top",
						name: translate("blockTableOptionsRowRowMoveTop"),
					};
				}
				...
				if (nextBot && !nextBot.content.isHeader && idx < length - 1) {
					moveBot = {
						id: "rowMoveBottom",
						icon: "table-move-bottom",
						name: translate("blockTableOptionsRowRowMoveBottom"),
					};
				}
				...
				options = options.concat([
					{
						id: "rowBefore",
						icon: "table-insert-top",
						name: translate("blockTableOptionsRowRowBefore"),
					},
					{
						id: "rowAfter",
						icon: "table-insert-bottom",
						name: translate("blockTableOptionsRowRowAfter"),
					},
					moveTop,
					moveBot,
					{
			...
			options = options.concat([
				{
					id: "clearContent",
					icon: "clear",
					name: translate("blockTableOptionsClearContent"),
				},
				length > 1
					? {
							id: "rowRemove",
							icon: "remove",
							name: translate("blockTableOptionsRowRowRemove"),
						}
					: null,
		...
		optionsColumn(id: string, isInner?: boolean) {
			...
			const options: any[] = [
				{
					id: "columnBefore",
					icon: "table-insert-left",
					name: translate("blockTableOptionsColumnColumnBefore"),
				},
				{
					id: "columnAfter",
					icon: "table-insert-right",
					name: translate("blockTableOptionsColumnColumnAfter"),
				},
				idx > 0
					? {
							id: "columnMoveLeft",
							...
							name: translate(
								"blockTableOptionsColumnColumnMoveLeft",
							),
						}
					: null,
				idx < length - 1
					? {
							id: "columnMoveRight",
							icon: "table-move-right",
							name: translate(
								"blockTableOptionsColumnColumnMoveRight",
							),
						}
					: null,
				{
				...
				length > 1
					? {
							...
							icon: "remove",
							name: translate(
								"blockTableOptionsColumnColumnRemove",
							),
						}
					: null,
				!isInner ? { isDiv: true } : null,
		...
		optionsColor(cellId: string) {
			...
			const innerBackground = (
				<div
					className={[
						"inner",
						"bgColor bgColor-" + (current?.bgColor || "default"),
					].join(" ")}
				/>
			);
			...
			return [
				{
					id: "color",
					icon: "color",
					name: translate("blockTableOptionsColorColor"),
					inner: innerColor,
					arrow: true,
				},
				{
					id: "background",
					icon: "color",
					name: translate("blockTableOptionsColorBackground"),
				...
				},
				{
					id: "style",
					icon: "paragraph",
					name: translate("blockTableOptionsColorStyle"),
					arrow: true,
				},
				{
					id: "clearStyle",
					icon: "clear",
					name: translate("blockTableOptionsColorClearStyle"),
				},
				{ isDiv: true },
			];
		}
		...
		optionsAlign(cellId: string) {
			...
			return [
				{
					id: "horizontal",
					icon: U.Data.alignHIcon(current?.hAlign),
					name: translate("blockTableOptionsAlignText"),
					arrow: true,
				},
				{
					id: "vertical",
					icon: U.Data.alignVIcon(current?.vAlign),
					name: translate("blockTableOptionsAlignVertical"),
					arrow: true,
				},
			];
		}
		...
		optionsStyle(cellId: string) {
			...
			const ret: any[] = [
				{
					id: I.MarkType.Bold,
					icon: "bold",
					name: translate("commonBold"),
				},
				{
					id: I.MarkType.Italic,
					icon: "italic",
					name: translate("commonItalic"),
				},
				{
					id: I.MarkType.Strike,
					icon: "strike",
					name: translate("commonStrikethrough"),
				},
				{
					id: I.MarkType.Underline,
					icon: "underline",
					name: translate("commonUnderline"),
				},
			];
			...
			return ret.map((it) => {
				it.checkbox = current
					? !!Mark.getInRange(current.content.marks, it.id, {
							from: 0,
							to: length,
						})
					: false;
		...
		resize() {
			...
			String(row.css("grid-template-columns") || "")
				.split(" ")
				.forEach((it) => (width += Number.parseInt(it)));
			...
			if (parent.isPage() || parent.isLayoutDiv()) {
				...
				width > maxWidth
					? wrap.addClass("withScroll")
					: wrap.removeClass("withScroll");
				...
				obj.css({
					...
					marginLeft:
						width >= wrapperWidth
							? Math.min(0, (wrapperWidth - width) / 2)
							: "",
				});
			} else {
