...
const BlockLink = observer(
	class BlockLink extends React.Component<I.BlockComponent> {
		_isMounted = false;
		node: any = null;
		...
		render() {
			...
			const withCover =
				relations.includes("cover") && coverId && coverType;
			...
			if (_empty_) {
				element = (
					<div
						className="loading"
						{...U.Common.dataProps({
							"target-block-id": object.id,
						})}
					>
						<Loader type="loader" />
						<div className="name">
							{translate("blockLinkSyncing")}
						</div>
					</div>
				);
			} else if (isDeleted) {
				...
				if (isArchived) {
					archive = (
						<div className="tagItem isMultiSelect archive">
				...
				if (withIcon) {
					const canEdit =
						!readonly &&
						!isArchived &&
						U.Object.isTaskLayout(object.layout);
					icon = (
						<IconObject
					...
					<div className={cnc.join(" ")} onClick={onCardClick}>
						<div id="sides" className={cns.join(" ")}>
							<div key="sideLeft" className={cnl.join(" ")}>
								<div
									className="relationItem cardName"
									onClick={onNameClick}
								>
									{icon}
									<ObjectName
		...
		onClick(e: any) {
			...
			if (
				...
				(keyboard.withCommand(e) && ids.length)
			) {
				return;
			}
		...
		getIconSize() {
			...
			const content = U.Data.checkLinkSettings(
				block.content,
				object.layout,
			);
			...
			if (
				...
				content.iconSize == I.LinkIconSize.Medium
			) {
				size = 48;
				iconSize = 28;
			}
		...
		resize() {
			window.setTimeout(() => {
				...
				icon.length
					? card.addClass("withIcon")
					: card.removeClass("withIcon");
				node.width() <= mw / 2
					? card.addClass("isVertical")
					: card.removeClass("isVertical");
			});
		}
	},
);
