...
const BlockCover = observer(
	class BlockCover extends React.Component<I.BlockComponent, State> {
		_isMounted = false;
		node: any = null;
		state = {
			isEditing: false,
		...
		render() {
			...
			const object = S.Detail.get(
				rootId,
				rootId,
				["iconImage", "iconEmoji"].concat(J.Relation.cover),
				true,
			);
			...
			if (coverType == I.CoverType.Source) {
				image = S.Detail.get(
					rootId,
					coverId,
					["mediaArtistName", "mediaArtistURL"],
					true,
				);
				author = (
					<Label
						...
						text={U.Common.sprintf(
							translate("unsplashString"),
							`<a href=${image.mediaArtistURL + J.Url.unsplash.utm}>${image.mediaArtistName}</a>`,
							`<a href=${J.Url.unsplash.site + J.Url.unsplash.utm}>Unsplash</a>`,
						)}
					/>
				);
			}
			...
			if (isImage) {
				content = (
					<img
						id="cover"
						src=""
						className={["cover", "type" + coverType, coverId].join(
							" ",
						)}
					/>
				);
				...
				elements = (
					<React.Fragment>
						...
						<div className="dragWrap">
							...
							/>
							<div id="dragValue" className="number">
								100%
							</div>
						</div>
		...
		onIcon(e: any) {
			...
			const cb = () =>
				S.Menu.update("smile", { element: `#block-icon-${rootId}` });
		...
		onDragMove(e: any) {
			...
			const { x, y } = this.setTransform(
				e.pageX - this.rect.x - this.x,
				e.pageY - this.rect.y - this.y,
			);
			this.cx = x;
			this.cy = y;
		}
		...
		onDragEnd(e: any) {
			...
			this.coords = {
				x: this.cx / this.rect.cw,
				y: this.cy / this.rect.ch,
			};
		}
