...
const BlockFeatured = observer(
	class BlockFeatured extends React.Component<Props> {
		...
		render() {
			...
			const short = S.Detail.get(
				rootId,
				storeId,
				["featuredRelations"],
				true,
			);
			const featuredRelations = Relation.getArrayValue(
				short.featuredRelations,
			);
			const object = S.Detail.get(rootId, storeId, featuredRelations);
			const allowedValue = S.Block.checkFlags(rootId, rootId, [
			...
			return (
				...
				>
					...
					{items.map((relationKey: any, i: any) => {
						...
						const canEdit =
							allowedValue && !relation.isReadonlyValue;
						const cn = ["cell", canEdit ? "canEdit" : ""];
						const check = Relation.checkRelationValue(
							relation,
							value,
						);
						...
						return (
							...
							>
								...
								<Cell
									...
									arrayLimit={
										relation.format == I.RelationType.Object
											? 1
											: 2
									}
									textLimit={150}
		...
		renderSetOf() {
			...
			if (tl) {
				setOfString.push(
					U.Common.sprintf(
						"%s: %s",
						U.Common.plural(tl, translate("pluralObjectType")),
						types.slice(0, SOURCE_LIMIT).join(", "),
					),
				);

				if (tl > SOURCE_LIMIT) {
					setOfString.push(
						<div className="more">+{tl - SOURCE_LIMIT}</div>,
					);
				}
			}
			if (rl) {
				setOfString.push(
					`${U.Common.plural(rl, translate("pluralRelation"))}: ${relations.slice(0, SOURCE_LIMIT).join(", ")}`,
				);

				if (rl > SOURCE_LIMIT) {
					setOfString.push(
						<div className="more">+{rl - SOURCE_LIMIT}</div>,
					);
				}
			}
			...
			return (
				<span
					className={["cell", !readonly ? "canEdit" : ""].join(" ")}
				>
					<div className="bullet" />
					<div
		...
		renderIdentity() {
			...
			const object = S.Detail.get(
				rootId,
				storeId,
				U.Data.participantRelationKeys(),
			);
					...
					<div
						...
						onMouseEnter={(e) =>
							this.onMouseEnter(
								e,
								relationKey,
								translate("blockFeaturedIdentity"),
							)
						}
						onMouseLeave={this.onMouseLeave}
					>
		...
		renderLinks(relationKey: string, index: number) {
			...
			const value = object[relationKey];
			const options = Relation.getArrayValue(value)
				.map((it) => S.Detail.get(rootId, it, []))
		...
		checkSource() {
			...
			if (!setOf.length) {
				...
				setOf.length &&
				setOf.length > types.length + relations.length
			) {
		...
		getItems() {
			...
			return (object.featuredRelations || []).filter(
				(it) => S.Record.getRelationByKey(it) && !skipIds.includes(it),
			);
		}
		...
		onType(e: any) {
			...
			if (!typeIsDeleted) {
				options.push({
					id: "open",
					name: translate("blockFeaturedTypeMenuOpenType"),
				});
			}
			...
			if (!readonly) {
				...
				if (!typeIsDeleted && U.Object.isSetLayout(object.layout)) {
					options.push({
						id: "turnCollection",
						name: translate(
							"blockFeaturedTypeMenuTurnSetIntoCollection",
						),
					});
				}
			}
					...
					if (message.records.length == 1) {
						...
						options.push({
							...
							name: U.Common.sprintf(
								translate("blockFeaturedTypeMenuOpenSetOf"),
								type.name,
							),
						});
					} else if (message.records.length == 2) {
						options.push({
							id: "setOpenMenu",
							name: translate("blockFeaturedTypeMenuOpenSet"),
							arrow: true,
						});
					} else if (type && !type.isDeleted) {
						options.push({
							id: "setCreate",
							name: U.Common.sprintf(
								translate("blockFeaturedTypeMenuCreateSetOf"),
								type.name,
							),
						});
					}
				...
				case "change":
					...
					menuParam.data = Object.assign(menuParam.data, {
						...
						onClick: (item: any) => {
							...
							S.Detail.update(
								rootId,
								{ id: item.id, details: item },
								false,
							);

							C.ObjectSetObjectType(
								rootId,
								item.uniqueKey,
								() => {
									if (
										object.internalFlags &&
										object.internalFlags.includes(
											I.ObjectFlag.SelectTemplate,
										)
									) {
										C.ObjectApplyTemplate(
											rootId,
											item.defaultTemplateId ||
												J.Constant.templateId.blank,
											open,
										);
									} else {
										open();
							...
							analytics.event("ChangeObjectType", {
								objectType: item.id,
								count: 1,
								route: analytics.route.featured,
							});
						},
					});
					break;
		...
		onTypeSelect(e: any, item: any) {
			...
			switch (item.id) {
				...
				case "setCreate":
					...
					if (type) {
						details.name = U.Common.sprintf(
							translate("commonSetName"),
							type.name,
						);
				...
				case "turnCollection":
					...
					if (collectionType) {
						S.Detail.update(
							rootId,
							{ id: collectionType.id, details: collectionType },
							true,
						);
					}
					...
					C.ObjectToCollection(rootId, (message: any) => {
						...
						keyboard.disableClose(true);
						U.Object.openAuto(
							{ id: rootId, layout: I.ObjectLayout.Collection },
							{ replace: true },
						);
						window.setTimeout(() => {
							Preview.toastShow({
								text: U.Common.sprintf(
									translate("toastTurnIntoCollection"),
									object.name,
								),
							});
						}, 200);
		...
		onRelation(e: any, relationKey: string) {
			...
			switch (relation.format) {
				...
				default: {
					...
					S.Menu.closeAll(null, () =>
						S.Menu.open("blockRelationView", param),
					);
					break;
				}
			}
				...
				data: {
					...
					onChange: (v: any, callBack?: () => void) => {
						const details = [
							{
								key: relationKey,
								value: Relation.formatValue(relation, v, true),
							},
						];
						C.ObjectListSetDetails([rootId], details);
