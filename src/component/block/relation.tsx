...
const BlockRelation = observer(
	class BlockRelation extends React.Component<I.BlockComponent> {
		refCell: any = null;
		...
		render(): any {
			...
			if (!relation) {
				relation = S.Record.getRelations().find(
					(it) => it.relationKey == relationKey,
				);
			}
		...
		onKeyDown(e: any) {
			...
			keyboard.shortcut(
				`${cmd}+z, ${cmd}+shift+z, ${cmd}+y, ${cmd}+v, ${cmd}+x`,
				e,
				() => {
					ret = true;
				},
