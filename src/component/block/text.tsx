...
const BlockText = observer(
	class BlockText extends React.Component<Props> {
		...
		render() {
			...
			const { text, marks, style, checked, color, iconEmoji, iconImage } =
				content;
			...
			for (const mark of marks) {
				if (
					[I.MarkType.Mention, I.MarkType.Object].includes(mark.type)
				) {
					...
					additional = (
						<React.Fragment>
							<Select
								...
								value={
									fields.lang || J.Constant.default.codeLang
								}
							...
							<div className="buttons">
								<div
									className="btn"
									onClick={this.onToggleWrap}
								>
									<Icon className="codeWrap" />
									<div className="txt">
										{fields.isUnwrapped
											? translate("blockTextWrap")
											: translate("blockTextUnwrap")}
									</div>
								</div>
		...
		renderMentions() {
			...
			items.each((i: number, item: any) => {
				...
				const object = S.Detail.get(rootId, data.param, []);
				const { id, _empty_, layout, done, isDeleted, isArchived } =
					object;
				...
				if (_empty_) {
					...
					icon = (
						<IconObject
							...
							onUpload={(objectId) =>
								this.onMentionUpload(id, objectId)
							}
		...
		getMarksFromHtml(): { marks: I.Mark[]; text: string } {
			const { block } = this.props;
			const value = this.refEditable
				? this.refEditable.getHtmlValue()
				: "";
		...
		onKeyDown(e: any) {
			...
			keyboard.shortcut("backspace", e, () => {
				...
				} else if (!menuOpenAdd && !menuOpenMention && !range.to) {
			...
			if (
				range &&
				(range.from != range.to || block.isTextCode()) &&
				Object.keys(twinePairs).includes(key)
			) {
				e.preventDefault();
		...
		onKeyUp(e: any) {
			...
			const { rootId, block, onMenuAdd, isInsideTable, onKeyUp } =
				this.props;
			...
			const canOpenMenuAdd =
				!menuOpenAdd && oneSymbolBefore == "/" && isAllowedMenu;
			const canOpenMenuMention =
				!menuOpenMention && oneSymbolBefore == "@" && isAllowedMenu;
			const newBlock: any = {
			...
			if (menuOpenAdd || menuOpenMention) {
				...
				this.timeoutFilter = window.setTimeout(() => {
					...
					if (d >= 0) {
						const part = value
							.substring(filter.from, filter.from + d)
							.replace(/^\//, "");
			...
			if (match) {
				...
				newBlock.content.style =
					match[1] == "***" ? I.DivStyle.Dot : I.DivStyle.Line;
				cmdParsed = true;
			}
			...
			if (block.canHaveMarks() && !isInsideTable && !block.isTextCode()) {
				for (const k in Markdown) {
					...
					const isFirstEmoji = Mark.getInRange(
						this.marks,
						I.MarkType.Emoji,
						{
							from: Length[newStyle],
							to: Length[newStyle] + 1,
						},
					);
					if (isFirstEmoji) {
						continue;
			...
			if (!keyboard.isSpecial(e)) {
				...
				if (d > 0) {
					for (let i = 0; i < this.marks.length; ++i) {
						...
						if (
							Mark.needsBreak(mark.type) &&
							mark.range.to == range.to
						) {
							const adjusted = Mark.adjust(
								[mark],
								mark.range.from,
								-d,
							);
		...
		onToggleWrap() {
			...
			C.BlockListSetFields(rootId, [
				{
					blockId: id,
					fields: { ...fields, isUnwrapped: !fields.isUnwrapped },
				},
			]);
		}
		...
		onSelect() {
			...
			const { rootId, block, isPopup, isInsideTable, readonly } =
				this.props;
			...
			this.timeoutContext = window.setTimeout(() => {
				...
				this.setText(this.marks, true, () => {
					...
					window.setTimeout(() => {
						const pageContainer =
							U.Common.getPageContainer(isPopup);

						pageContainer
							.off("mousedown.context")
							.on("mousedown.context", () => {
		...
		checkMarkOnBackspace(value: string) {
			...
			if (mark) {
				...
				this.marks = this.marks.filter((it) => {
					return (
						it.type != mark.type ||
						it.range.from != mark.range.from ||
						it.range.to != mark.range.to ||
						it.param != mark.param
					);
				});

				this.marks = Mark.adjust(
					this.marks,
					mark.range.from,
