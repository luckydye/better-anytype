...
const BlockDataview = observer(
	class BlockDataview extends React.Component<Props, State> {
		...
		render() {
			...
			const className = [
				U.Common.toCamelCase(`view-${I.ViewType[view.type]}`),
			];
			...
			if (loading) {
				body = null;
			} else if (isInline && !targetId) {
				body = this.getEmpty("target");
			} else if (!isCollection && !sources.length) {
				...
				body = (
					<div className="content">
						<ViewComponent
							...
							onRef={(ref: any, id: string) =>
								this.refCells.set(id, ref)
							}
							{...this.props}
							{...dataviewProps}
							pageContainer={U.Common.getCellContainer(
								isPopup ? "popup" : "page",
							)}
		...
		componentDidMount() {
			...
			if (match.params.viewId || viewId) {
				S.Record.metaSet(subId, "", {
					viewId: match.params.viewId || viewId,
				});
			}
			...
			const view = this.getView();
			const eventName = this.isCollection()
				? "ScreenCollection"
				: "ScreenSet";
			...
			if (
				!isInline &&
				Onboarding.isCompleted("mainSet") &&
		...
		init() {
			...
			object.isDeleted
				? head.addClass("isDeleted")
				: head.removeClass("isDeleted");
		}
		...
		rebind() {
			...
			win.on(`updateDataviewData.${ns}`, () =>
				this.loadData(this.getView().id, 0, true),
			);
			win.on(`setDataviewSource.${ns}`, () =>
				this.onSourceSelect(`#block-head-${block.id} #value`, {
					offsetY: 36,
				}),
			);
			win.on(`selectionEnd.${ns} selectionClear.${ns}`, () =>
				this.onSelectEnd(),
			);
		}
		...
		loadData(
			...
			} else if (view.type == I.ViewType.Calendar) {
		...
		reloadData() {
			...
			if (view) {
				S.Record.metaSet(this.getSubId(), "", {
					viewId: view.id,
					offset: 0,
					total: 0,
				});
		...
		getKeys(id: string): string[] {
			...
			if (view) {
				keys = keys.concat(
					(view.relations || []).map((it) => it && it.relationKey),
				);

				if (
					view.coverRelationKey &&
					view.coverRelationKey != J.Relation.pageCover
				) {
		...
		getSubId(groupId?: string): string {
			...
			return groupId
				? S.Record.getGroupSubId(rootId, block.id, groupId)
				: S.Record.getSubId(rootId, block.id);
		}
		...
		getSources(): string[] {
			...
			const types = Relation.getSetOfObjects(
				rootId,
				target.id,
				I.ObjectLayout.Type,
			).map((it) => it.id);
			const relations = Relation.getSetOfObjects(
				rootId,
				target.id,
		...
		getTarget() {
			...
			return S.Detail.get(rootId, isInline ? targetObjectId : rootId, [
				"setOf",
			]);
		}
		...
		getDefaultTemplateId(typeId?: string): string {
			...
			} else if (type && type.defaultTemplateId) {
		...
		recordCreate(e: any, template: any, dir: number, groupId?: string) {
			...
			C.ObjectCreate(
				...
				(message: any) => {
					...
					if (oldIndex < 0) {
						dir > 0
							? records.push(message.objectId)
							: records.unshift(message.objectId);
					} else {
						records = arrayMove(
							records,
							oldIndex,
							dir > 0 ? records.length : 0,
						);
					}

					if (groupId) {
						this.objectOrderUpdate(
							[{ viewId: view.id, groupId, objectIds: records }],
							records,
		...
		onEmpty(e: any) {
			...
			if (isInline) {
				this.onSourceSelect(e.currentTarget, {
		...
		onBookmarkMenu(e: any, dir: number, groupId?: string, param?: any) {
			...
			S.Menu.open("dataviewCreateBookmark", {
				...
				vertical:
					dir > 0 ? I.MenuDirection.Top : I.MenuDirection.Bottom,
				horizontal:
					dir > 0 ? I.MenuDirection.Left : I.MenuDirection.Right,
				offsetX: dir < 0 ? -24 : 0,
				offsetY: 4 * -dir,
				data: {
		...
		onTemplateMenu(e: any, dir: number) {
			...
			S.Menu.open("dataviewTemplateList", {
				...
				offsetY: 10,
				noAnimation: true,
				subIds: J.Menu.dataviewTemplate.concat([
				...
				data: {
					...
					onTypeChange: (id) => {
						if (id != this.getTypeId()) {
							Dataview.viewUpdate(rootId, block.id, view.id, {
								defaultTypeId: id,
								defaultTemplateId: J.Constant.templateId.blank,
							});
					...
					onSelect: (item: any) => {
						...
						if (
							type &&
							type.uniqueKey == J.Constant.typeKey.bookmark
						) {
							menuContext.close();
							this.onBookmarkMenu(e, dir, "", {
								element: `#button-${block.id}-add-record`,
							});
						} else if (item.id == J.Constant.templateId.new) {
		...
		onTemplateAdd(id?: string) {
			...
			C.ObjectCreate(
				...
				(message) => {
					...
					focus.clear(true);
					analytics.event("CreateTemplate", {
						objectType: typeId,
		...
		canCellEdit(relation: any, record: any): boolean {
			...
			if (
				U.Object.isNoteLayout(record.layout) &&
		...
		onCellChange(
			...
			if (
				undefined !== record[relationKey] &&
				!U.Common.compareJSON(record[relationKey], value)
			) {
		...
		onContext(e: any, id: string): void {
			...
			S.Menu.open("dataviewContext", {
				...
				data: {
					...
					isCollection,
					route: this.analyticsRoute(),
					relationKeys: this.getVisibleRelations().map(
						(it) => it.relationKey,
					),
					view,
					allowedLink: true,
		...
		onSourceSelect(element: any, param: Partial<I.MenuParam>) {
			...
			if (isCollection) {
				...
				addParam.nameWithFilter = translate(
				...
				addParam.onClick = (details: any) => {
					C.ObjectCreate(
						...
						S.Common.space,
						(message) => onSelect(message.details, true),
					);
				};
			} else {
				filters = filters.concat([
					{
						...
						relationKey: "layout",
						condition: I.FilterCondition.Equal,
						value: I.ObjectLayout.Set,
					},
					{
						operator: I.FilterOperator.And,
						relationKey: "setOf",
						condition: I.FilterCondition.NotEmpty,
						value: null,
					},
				]);
				...
				addParam.nameWithFilter = translate(
					"blockDataviewCreateNewSetWithName",
				);
				addParam.onClick = (details: any) => {
					C.ObjectCreateSet(
						[],
			...
			const onSelect = (item: any, isNew: boolean) => {
				C.BlockDataviewCreateFromExistingObject(
					rootId,
					block.id,
					item.id,
					(message: any) => {
						...
						if (isInline) {
							Onboarding.start(
								isCollection ? "inlineCollection" : "inlineSet",
								isPopup,
								false,
								{
									parseParam: (param) => ({
				...
				analytics.event("InlineSetSetSource", {
					type: isNew ? "newObject" : "externalObject",
				});
			};
		...
		getVisibleRelations() {
			...
			return view
				.getVisibleRelations()
				.filter((it) => keys.includes(it.relationKey));
		}
			...
			switch (type) {
				case "target": {
					const name = translate(
						isCollection
							? "blockDataviewEmptyTargetCollections"
							: "blockDataviewEmptyTargetSets",
					);
					emptyProps = {
						title: translate("blockDataviewEmptyTargetTitle"),
						description: U.Common.sprintf(
							translate("blockDataviewEmptyTargetDescription"),
							name,
						),
						button: translate("blockDataviewEmptyTargetButton"),
						onClick: () =>
							this.onSourceSelect(
								`#block-${block.id} .dataviewEmpty .button`,
								{},
							),
					};
					break;
				...
				case "source": {
					emptyProps = {
						...
						description: translate(
							"blockDataviewEmptySourceDescription",
						),
				...
				case "view": {
					...
					if (this.isAllowedObject()) {
						emptyProps.description = translate(
							"blockDataviewEmptyViewDescription",
						);
						emptyProps.button = translate("commonCreateObject");
						emptyProps.onClick = (e) =>
		...
		isAllowedObject() {
			...
			let isAllowed =
				!readonly &&
				S.Block.checkFlags(rootId, block.id, [
					I.RestrictionDataview.Object,
				]);
			if (!isAllowed) {
				return false;
			}
			...
			const skipLayouts = [I.ObjectLayout.Participant].concat(
				U.Object.getFileAndSystemLayouts(),
			);
		...
		isAllowedDefaultType(): boolean {
			...
			const { rootId } = this.props;
			return !!Relation.getSetOfObjects(
				rootId,
				this.getTarget().id,
				I.ObjectLayout.Relation,
			).map((it) => it.id).length;
		...
		objectOrderUpdate(
			orders: any[],
			records: any[],
			callBack?: (message0: any) => void,
		) {
			const { rootId, block } = this.props;
			const view = this.getView();

			C.BlockDataviewObjectOrderUpdate(
				rootId,
				block.id,
				orders,
				(message) => {
					...
					orders.forEach((it: any) => {
						const old = block.content.objectOrder.find(
							(item) =>
								view.id == item.viewId &&
						...
						window.setTimeout(
							() => this.applyObjectOrder(it.groupId, records),
							30,
						);
					});
		...
		applyObjectOrder(groupId: string, records: any[]): string[] {
			...
			return Dataview.applyObjectOrder(
				rootId,
				block.id,
				view.id,
				groupId,
			...
			this.timeoutFilter = window.setTimeout(() => {
				...
				if (v) {
					U.Data.search(
						...
						(message: any) => {
							this.searchIds = (message.records || []).map(
								(it) => it.id,
							);
							this.reloadData();
						},
				...
				analytics.event("ScreenSearchDataview", {
					route: this.analyticsRoute(),
				});
		...
		resize() {
			...
			this.frame = raf(() => {
				...
				if (getWrapperWidth) {
					...
					node.width() <= getWrapperWidth() / 2
						? obj.addClass("isVertical")
						: obj.removeClass("isVertical");
				}
