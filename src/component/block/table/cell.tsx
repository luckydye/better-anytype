...
const BlockTableCell = observer(
	class BlockTableCell extends React.Component<Props> {
		...
		render() {
			...
			return (
				<div
					...
					onMouseEnter={(e) =>
						onCellEnter(e, row.id, column.id, cellId)
					}
					onMouseLeave={(e) =>
						onCellLeave(e, row.id, column.id, cellId)
					}
				...
				>
					{!rowIdx ? (
						<Handle
							key={"handle-column-" + cellId}
							type={I.BlockType.TableColumn}
							{...column}
						/>
					) : (
						""
					...
					{!columnIdx ? (
						<Handle
							key={"handle-row-" + cellId}
							type={I.BlockType.TableRow}
							{...row}
						/>
					) : (
						""
					...
					{block ? (
						<Block
							...
							onBlur={(e) =>
								onCellBlur(e, row.id, column.id, cellId)
							}
		...
		onMouseDown(e: any) {
			...
			$(window)
				.off("mousedown.table-cell")
				.on("mousedown.table-cell", () =>
