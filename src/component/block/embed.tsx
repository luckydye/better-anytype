...
const BlockEmbed = observer(
	class BlockEmbed extends React.Component<I.BlockComponent, State> {
		_isMounted = false;
		text = "";
		timeoutChange = 0;
		timeoutScroll = 0;
		...
		render() {
			...
			const menuItem: any = U.Menu.getBlockEmbed().find(
				(it) => it.id == processor,
			) || {
				name: "",
				icon: "",
			...
			if (block.isEmbedLatex()) {
				...
				select = (
					<div
						id="select"
						className="select"
						onMouseDown={this.onLatexTemplate}
					>
						<div className="name">
							{translate("blockEmbedLatexTemplate")}
						</div>
				...
				placeholder = U.Common.sprintf(
					translate("blockEmbedPlaceholder"),
					menuItem.name,
				);
				empty = !text
					? U.Common.sprintf(
							translate("blockEmbedEmpty"),
							menuItem.name,
						)
					: "";
			...
			return (
				...
				>
					<div
						id="valueWrap"
						className="valueWrap resizable"
						style={css}
					>
						{select ? (
							<div className="selectWrap">{select}</div>
						) : (
							""
						)}
						...
						<div
							...
							className={[
								"preview",
								U.Data.blockEmbedClass(processor),
							].join(" ")}
		...
		rebind() {
			...
			if (isEditing) {
				win.on(`mousedown.${block.id}`, (e: any) => {
					if (
						!this._isMounted ||
						S.Menu.isOpenList(["blockLatex", "select"])
					) {
						return;
					}
		...
		onPaste(e: any) {
			...
			const value = U.Common.stringInsert(
				this.getValue(),
				text,
				range.from,
				range.to,
		...
		setContent(text: string) {
			...
			switch (processor) {
				default: {
					...
					const allowIframeResize =
						U.Embed.allowIframeResize(processor);
					...
					const onLoad = () => {
						const iw = (iframe[0] as HTMLIFrameElement)
							.contentWindow;
						...
						const data: any = {
							allowIframeResize,
							insertBeforeLoad:
								U.Embed.insertBeforeLoad(processor),
						...
						if (
							...
							const result = btoa(
								U.Common.uint8ToString(compressed),
							)
								.replace(/\+/g, "-")
								.replace(/\//g, "_");
							const type =
								fields.type || U.Embed.getKrokiOptions()[0].id;
						...
						if (
							U.Embed.allowEmbedUrl(processor) &&
							!text.match(/<(iframe|script)/)
						) {
							text = U.Embed.getHtml(
								processor,
								U.Embed.getParsedUrl(text),
							);
						}

						if (
							block.isEmbedSketchfab() &&
							text.match(/<(iframe|script)/)
						) {
							text =
								text.match(/<iframe.*?<\/iframe>/)?.[0] || "";
						}

						if (block.isEmbedGithubGist()) {
							allowScript = !!text.match(
					...
					if (!iframe.length) {
						iframe = $("<iframe />", {
							id: "receiver",
							src: U.Common.fixAsarPath(
								`./embed/iframe.html?theme=${S.Common.getThemeClass()}`,
							),
							frameborder: 0,
							scrolling: "no",
				...
				case I.EmbedProcessor.Mermaid: {
					const theme = (J.Theme[S.Common.getThemeClass()] || {})
						.mermaid;
					...
					mermaid
						.render(this.getContainerId(), this.text)
						.then((res) => {
		...
		onResizeStart(e: any, checkMax: boolean) {
			...
			win.on(`mousemove.${block.id}`, (e) =>
				this.onResizeMove(e, checkMax),
			);
