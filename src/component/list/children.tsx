...
const ListChildren = observer(
	class ListChildren extends React.Component<Props> {
		...
		render() {
			...
			const className = String(this.props.className || "").replace(
				/first|last/g,
				"",
			);
			...
			if (isRow) {
				ColResize = (item: any) => (
					<div
						className={["colResize", "c" + item.index].join(" ")}
						onMouseDown={(e) => onResizeStart(e, item.index)}
					>
						<div className="inner">
							<div className="line" />
						</div>
			...
			return (
				...
				>
					{children.map((item: any, i: number) => {
						...
						if (isRow) {
							css.width =
								(item.fields.width || 1 / length) * 100 + "%";
						}
