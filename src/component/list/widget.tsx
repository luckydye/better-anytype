...
const ListWidget = observer(
	class ListWidget extends React.Component<{}, State> {
		state: State = {
			isEditing: false,
			previewId: "",
		};
		...
		render(): React.ReactNode {
			...
			if (previewId) {
				...
				const blocks = S.Block.getChildren(
					widgets,
					widgets,
					(block: I.Block) => {
						...
						if (
							Object.values(J.Constant.widgetId).includes(target)
						) {
							return true;
						}
					...
					buttons = buttons.concat([
						{
							id: "widget-list-add",
							className: "grey c28",
							text: translate("widgetAdd"),
							onMouseDown: this.onAdd,
						},
						{
							id: "widget-list-edit",
							className: "grey c28",
							text: translate("widgetEdit"),
							onMouseDown: this.onEdit,
						},
					]);
				}
				...
				data: {
					filters: [
						...
						},
						{
							...
							value: S.Record.getTemplateType()?.id,
						},
					],
					canAdd: true,
					dataChange: (context: any, items: any[]) => {
						...
						const fixed: any[] = U.Menu.getFixedWidgets().filter(
							(it) => it.name.match(reg),
						);
					...
					onSelect: (target: any, isNew: boolean) => {
						const limitOptions = U.Menu.getWidgetLimitOptions(
							I.WidgetLayout.Link,
						);
						const layoutOptions = U.Menu.getWidgetLayoutOptions(
							target.id,
							target.layout,
						);
						const layout = layoutOptions.length
							? layoutOptions[0].id
							: I.WidgetLayout.Link;
						...
						C.BlockCreateWidget(
							...
							(message: any) => {
								...
								analytics.event("AddWidget", {
									type: I.WidgetLayout.Link,
								});
		...
		onDragOver(e: React.DragEvent, blockId: string) {
			...
			this.frame = raf(() => {
				...
				target.addClass(
					[
						"isOver",
						this.position == I.BlockPosition.Top ? "top" : "bottom",
					].join(" "),
				);
			});
		}
