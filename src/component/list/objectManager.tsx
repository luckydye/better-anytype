...
const ListObjectManager = observer(
	class ListObjectManager extends React.Component<Props, State> {
		_isMounted = false;
		state = {
			isLoading: false,
		};
		...
		render() {
			...
			const { buttons, rowHeight, iconSize, info, isReadonly } =
				this.props;
			...
			if (this.selected.length) {
				...
				buttonsList.push({
					...
					text: translate("commonDeselectAll"),
				...
				buttonsList.push({
					icon: "checkbox",
					text: translate("commonSelectAll"),
					onClick: this.onSelectAll,
				});
			}
			...
			const Info = (item: any) => {
				...
				switch (info) {
					...
					case I.ObjectManagerItemInfo.FileSize: {
						itemInfo = (
							<Label
								text={String(U.File.size(item.sizeInBytes))}
							/>
						);
						break;
					}
			...
			if (!items.length && !isLoading) {
				if (!filter) {
					...
					textEmpty = U.Common.sprintf(
						translate("popupSearchNoObjects"),
						filter,
					);
				}
		...
		onClick(e: React.MouseEvent, item: any) {
			...
			if (e.shiftKey) {
				...
				if (idx >= 0 && this.selected.length > 0) {
					const indexes = this.getSelectedIndexes().filter(
						(i) => i != idx,
					);
		...
		getSelectionRange(index1: number, index2: number) {
			const [start, end] =
				index1 >= index2 ? [index2, index1] : [index1 + 1, index2 + 1];
			return [start, end];
		}
		...
		load() {
			const { subId, sources, withArchived, collectionId, onAfterLoad } =
				this.props;
			const filter = this.getFilterValue();
