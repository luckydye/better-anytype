...
class Notification implements I.Notification {
	...
	fillContent() {
		...
		const identityName = U.Common.shorten(
			String(this.payload.identityName || translate("defaultNamePage")),
			32,
		);
		const spaceName = U.Common.shorten(
			String(this.payload.spaceName || translate("defaultNamePage")),
			32,
		);
		const permissions = translate(
			`participantPermissions${this.payload.permissions}`,
		);

		this.title = translate(
			U.Common.toCamelCase(`notification-${et}-${lang}-title`),
		);
		this.text = translate(
			U.Common.toCamelCase(`notification-${et}-${lang}-text`),
		);
		...
		switch (this.type) {
			case I.NotificationType.Import: {
				if (
					...
					errorCode == J.Error.Code.NO_OBJECTS_TO_IMPORT
				) {
					this.title = translate(
