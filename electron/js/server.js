			...
			this.stop().then(() => {
				...
				try {
					if (!process.stdout.isTTY) {
						env["GOLOG_FILE"] = path.join(
							logPath,
							`anytype_${Util.dateForFile()}.log`,
						);
					}
				...
				this.cp.stderr.on("data", (data) => {
					...
					if (!this.lastErrors) {
						...
						this.lastErrors.length >= maxStdErrChunksBuffer
					) {
				...
				this.cp.on("exit", () => {
					...
					dialog.showErrorBox(
						...
						"You will be redirected to the crash log file. You can send it to Anytype developers by creating issue at https://community.anytype.io",
					);
