...
class Util {
	...
	printHtml(win, exportPath, name, options) {
		...
		win.webContents
			.savePage(exportName, "HTMLComplete")
			.then(() => {
	...
	printPdf(win, exportPath, name, options) {
		win.webContents
			...
			.then((data) => {
				fs.writeFile(
					path.join(exportPath, this.fileName(name)),
					data,
					(error) => {
	...
	translate(key) {
		...
		const defaultData = require(`../../packages/locales/lang/en-US.json`);

		let data = {};
		try {
			data = require(`../../packages/locales/lang/${lang}.json`);
		} catch (e) {}

		return data[key] || defaultData[key] || `!${key}!`;
	}
