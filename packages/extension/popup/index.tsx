...
import { type I, C, S, U, J, Storage } from "Lib";
...
const Index = observer(
	class Index extends React.Component<I.PageComponent, State> {
		state = {
			error: "",
		};
		interval: any = 0;
			...
			Util.sendMessage({ type: "getPorts" }, (response) => {
				Util.sendMessage({ type: "checkPorts" }, (response) => {
					if (!response.ports || !response.ports.length) {
						this.setState({
							error: "Automatic pairing failed, please open the app",
						});
		...
		login() {
			...
			if (appKey) {
				...
				C.AccountLocalLinkNewChallenge(
					manifest.name,
					(message: any) => {
