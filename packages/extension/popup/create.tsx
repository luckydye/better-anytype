...
const Create = observer(
	class Create extends React.Component<I.PageComponent, State> {
		details: any = {
			type: "",
			tag: [],
		};
					...
					<form onSubmit={this.onSubmit}>
						<div className="rows">
							...
							<div className="row">
								...
								<div
									id="select-tag"
									className="box cell isEditing c-select"
									onClick={this.focus}
								>
		...
		componentDidMount(): void {
			U.Data.createAllSubscriptions(() => {
				...
				this.setState({
					withContent: Boolean(Storage.get("withContent")),
				});
			});
		}
		...
		initType() {
			const options = this.getTypes().map((it) => ({
				...it,
				id: it.uniqueKey,
			}));
			...
			this.details.type =
				this.details.type || J.Constant.typeKey.bookmark;
		...
		getSpaces() {
			return U.Space.getList()
				.filter(
					(it) =>
						it && U.Space.canMyParticipantWrite(it.targetSpaceId),
				)
		...
		getTypes() {
			...
			return this.getObjects(J.Constant.subId.type)
				...
				.filter(
					(it) =>
						layouts.includes(it.recommendedLayout) &&
						it.spaceId == S.Common.space,
				)
